...
`@logicn/core-sentinel-io` (LSIO) is intentionally pure TypeScript over
...
## (b) `logicn-tower-citizen` wiring deferred

Deeper integration into `logicn-tower-citizen` — loading real model weights via
...
`../logicn-core-compiler` (there is no local `tsc`). That compiler package does
