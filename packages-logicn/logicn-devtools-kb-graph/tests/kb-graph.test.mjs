// @ts-check
// =============================================================================
// kb-graph.test.mjs — unit tests for the LogicN KB Graph scanner/builder
// =============================================================================
...
describe("scanner — metadata extraction", () => {
  ...
  test("extracts LLN codes from governance-rules doc (>=20 codes)", () => {
    const gov = scanResult.docs.find(d => d.id.includes("governance-rules"));
    if (!gov) {
      // Non-fatal: doc might not be present
      console.log("  (skipped — logicn-governance-rules.md not found)");
      return;
    }
    assert.ok(gov.lnlCodes.length >= 20,
      `expected >=20 LLN codes, got ${gov.lnlCodes.length}`);
  });

  test("LLN codes match expected pattern LLN-XXX-NNN", () => {
    const re = /^LLN-[A-Z]+-\d+$/;
    for (const doc of scanResult.docs) {
      for (const code of doc.lnlCodes) {
        assert.ok(re.test(code), `unexpected LLN code format: "${code}" in ${doc.id}`);
      }
    }
  });
...
describe("reporter — output formats", () => {
  ...
  test("generateMarkdownReport contains expected sections", () => {
    ...
    assert.ok(md.includes("# LogicN KB Graph Report"),       "missing title");
