...
function nodeLabel(doc: KBDocNode): string {
  ...
  if (doc.lnlCodes.length > 0) {
    label += `\\n${doc.lnlCodes.length} LLN code${doc.lnlCodes.length !== 1 ? "s" : ""}`;
  }
  return label;
}
...
export function generateMarkdownReport(graph: KBGraph, generatedAt: string): string {
  ...
  const lines: string[] = [
    ...
    "",
    "## Stats",
    `- Docs: ${stats.totalDocs} | Edges: ${stats.totalEdges} | Orphans: ${stats.orphanCount} | Stale links: ${stats.staleLinkCount} | LLN codes: ${stats.totalLlnCodes}`,
    "",
