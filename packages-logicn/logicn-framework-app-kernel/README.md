...
> `logicn-framework-app-kernel` is one of three **app-layer framework templates**
> (`logicn-framework-app-kernel`, `logicn-framework-api-server`,
> `logicn-framework-example-app`). It sits **above** the LogicN language + core
> runtime and exists to scaffold how an app *would* run as a governed service — it
> is **not** part of the LogicN language or compiler, and it is **not** the
