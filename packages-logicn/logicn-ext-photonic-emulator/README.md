...
A **physics-faithful (Rung-2) photonic-PPU backend** for LogicN: a noise-carrying MZI-mesh /
...
(`@logicn/inference-bridge-contract`) — the same contract the stub ternary bridge and the native
...
- **D1 emulator** — `LogicN-R-AND-D/scripts/rd-photonic-ppu-emulator-proof.mjs` (18/18): converges to
...
- **D2 router** — `LogicN-R-AND-D/scripts/rd-photonic-ppu-cost-model-proof.mjs` (25/25): an
