/**
 * @logicn/devtools-pci — Black Box Compliance Ledger Tests (#146)
 *
 * Verifies the compliance-ledger module that consumes the audit-egress ledger
 * (from @logicn/core-sentinel-egress) and emits a structured, append-only,
...
function freshDir(tag) {
  return mkdtempSync(join(tmpdir(), `lln-compliance-${tag}-`));
}
...
describe("empty ledger handling", () => {
  it("a missing egress dir yields an empty report with genesis chain head", () => {
    const report = buildComplianceReportFromDir(join(tmpdir(), `lln-compliance-missing-${process.pid}-${Date.now()}`), FIXED_TS);
...
describe("auditor-consumable JSON", () => {
  it("serialises to valid JSON with who/what/effect/decision/timestamp on every entry", () => {
    ...
    assert.equal(parsed.schemaVersion, "lln.compliance-ledger.v1");
