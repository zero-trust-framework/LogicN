# Getting Started with LogicN

This guide explains how to try **LogicN / LogicN** from this repository.
...
LogicN is currently a concept, documentation and v0.1 prototype project.
...
runnable .lln examples
a Node.js prototype CLI in compiler/logicn.js
...
node compiler/logicn.js tokens examples/hello.lln
...
```bash
node compiler/logicn.js run my-logicn-app
```

The generated `boot.lln` includes target declarations, global registry entries,
...
`source-map-error.lln` intentionally contains invalid compute-block I/O so the
prototype can demonstrate source-mapped diagnostics.

```bash
node compiler/logicn.js explain examples/source-map-error.lln
node compiler/logicn.js explain examples/source-map-error.lln --for-ai
```
...
LogicN is based on these rules:
...
Source files use the `.lln` extension. The recommended project entry file is
`boot.lln`; simple scripts can expose `secure flow main()`.
