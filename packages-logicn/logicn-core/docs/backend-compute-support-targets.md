# LogicN Backend Compute Targets and Runtime Planning
...
This document defines how LogicN should support backend compute planning without
...
planning is a valid LogicN compiler/runtime concern.
...
LogicN should support compute target planning at the language/compiler/runtime
level.

LogicN should not hard-code vendor-specific hardware behaviour.
...
LogicN compiler/runtime:
  target planning
  capability detection
  fallback selection
  memory/interconnect planning

LogicN target plugins:
  CPU backend
...
LogicN should select the best safe target
fallback is allowed
...
LogicN should provide general compute planning primitives:
...
LogicN may support broad target categories:
...
These should not be hard-coded into LogicN core.

LogicN core defines the target model. Plugins implement hardware-specific
behaviour.
...
LogicN should never make CPU optional.

Every LogicN program should have a safe CPU fallback unless the project explicitly
...
LogicN should support GPU as a broad target category. Vendor/runtime-specific
...
LogicN should avoid unnecessary movement such as:
...
LogicN core should not assume a specific photonic chip exists.
...
LogicN should include this in target reports.
...
This should be a target-plugin capability, not a mandatory LogicN feature.
...
```text
Photonic compute must return to strict LogicN values before affecting business or
security decisions.
```
...
LogicN should support memory/interconnect planning as part of `compute auto`.
...
LogicN should support cloud compute profiles without hard-coding cloud providers
...
LogicN should support planning for:
...
LogicN should support precision types for compute planning:
...
Before `main()` runs, LogicN should validate compute capabilities.
...
Compute targets must respect LogicN's security model.
...
LogicN may eventually consider:
...
## Reports LogicN Should Generate
...
not mandatory LogicN core features.

---

## Non-Goals

LogicN compute support should not:
...
Should LogicN support vendor plugins for hardware backends?
...
This document keeps compute planning as a valid LogicN compiler/runtime concern, but
removes wording that makes LogicN sound like it directly implements hardware
...
Kept as LogicN compiler/runtime concerns:
...
LogicN should support modern and future compute without making code difficult to
...
LogicN should make compute safer, clearer and easier to optimise.

LogicN should not become a vendor-specific hardware SDK.
