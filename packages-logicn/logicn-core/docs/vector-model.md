# LogicN Hybrid Scalar + Vector Model

Ownership note: `logicn-core` may document vector syntax and compiler checks, but
vector values, lanes, dimensions, operations and vector reports belong in
`packages-logicn/logicn-core-vector/`.

This document defines a security-first design for adding vector-aware execution to **LogicN / LogicN**.

The goal is not to make LogicN a pure vector language. The goal is to let LogicN support normal readable application logic while giving developers and the compiler a safe way to optimise repeated, data-heavy work.
...
LogicN should use a hybrid model:
...
Recommended LogicN position:

```text
LogicN is scalar-first and vector-aware.
```
...
LogicN should avoid forcing everything into vector form.
...
```text
logicn-ERR-VECTOR-SECURITY-001: Side effect "saveToDatabase" is not aLOwed inside vector block.
```
...
Therefore LogicN should keep these actions in scalar code unless the developer uses a specialised, explicit construct.
...
If vector acceleration is disabled, LogicN should compile it as safe scalar code:
...
```json
{
  "vector": {
    ...
    "fallback": "scalar",
    "notes": [
      "Vector block in src/checkout.lln:5 compiled to scalar fallback because vector execution is disabled for this target."
    ]
  }
}
```
...
```text
logicn-ERR-VECTOR-REQUIRED-001: Vector execution was required, but this target does not support it.
```
...
LogicN should support clear project-level modes.
...
| `aggressive` | Optimise heavily with fewer restrictions | Not recommended for secure LogicN apps |

Recommended LogicN default:
...
LogicN may auto-vectorise scalar code only when it can prove safety.
...
LogicN should provide a command that suggests vector syntax without changing files automatically.

Example command:

```bash
LogicN suggest vector
```
...
```bash
LogicN refactor vector --interactive
```
...
The hybrid model is good for memory usage only if LogicN avoids hidden copies.
...
Before compiling vector code, LogicN should check:
...
If any check fails, LogicN should either:
...
For security-first LogicN, fail closed by default.
...
```text
logicn-WARN-VECTOR-MEMORY-004: Vector pipeline may aLOcate a large intermediate result.
```
...
LogicN should avoid hidden partial failure.
...
LogicN should provide precision controls.
...
For money, LogicN should avoid unsafe floating-point maths.
...
```text
logicn-WARN-MONEY-PRECISION-001: Float type used inside financial vector calculation.
```
...
If CPU worker nodes are enabled, LogicN may chunk work across workers.

If workers are disabled, LogicN falls back to scalar.

---

## Primary Lane and Offload Integration

The vector model should integrate with LogicN's primary lane/offload nodes idea.
...
Do not add these as executable `.lln` examples until parser support exists for `vector` blocks.

Recommended future examples:

```text
examples/vector/checkout-vector.lln
examples/vector/vector-disabled-scalar-fallback.lln
examples/vector/vector-memory-safe-pipeline.lln
examples/vector/vector-security-blocked-side-effect.lln
```
...
LogicN should include tests for:
...
LogicN should adopt a hybrid scalar + vector model.
