...
Startup validation is part of that model. A LogicN app should validate
`boot.lln`, imports, packages, globals, secrets, security policy, routes,
...
LogicN should be able to request and report supported hardware-assisted security
...
LogicN should treat API input as unsafe until it has been decoded into strict
...
LogicN should provide typed primitives and reports for:
...
LogicN should detect duplicate API structure at check/build time and help prevent
...
LogicN should support standard auth and verification workflows safely without
