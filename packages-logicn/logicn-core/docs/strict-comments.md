# LogicN Strict Comments

This document describes the proposed **Strict Comments** feature for **LogicN / LogicN**.
...
LogicN should support normal comments, but also introduce **strict comments**.
...
Original source:
  src/services/order-service.lln:12:1
...
However, LogicN strict comments are different because they are designed to combine:
...
So LogicN strict comments are not just documentation.
...
LogicN should support several comment levels.
...
LogicN should support a small, clear set of strict comment tags.
...
LogicN should provide checks for strict comments.

Possible commands:

```bash
LogicN lint --comments
LogicN check --strict-comments
...
Original source:
  src/services/order-service.lln:8:1
```
...
Original source:
  src/webhooks/payment-webhook.lln:1:1
...
```json
{
  ...
  "source": "src/webhooks/payment-webhook.lln:1"
}
```
...
Strict comment checks should map back to original `.lln` files.
...
Original source:
  src/services/order-service.lln:14:1
```
...
```json
{
  "strictComments": {
    "mismatches": [
      {
        "file": "src/webhooks/payment-webhook.lln",
        "line": 1,
        "tag": "@security",
...
LogicN should foLOw these rules:
...
## Strict Comments and `LogicN explain --for-ai`

When an error occurs, `LogicN explain --for-ai` should include relevant strict comment context.
...
For early LogicN, strict comments should begin as a linter and report feature.
...
add dedicated LogicN lint --comments command
```
...
Strict comments should make LogicN easier to understand and safer to change.
...
LogicN should treat strict comments as a bridge between:
