# LogicN Specification

This document is the root normative specification for **LogicN / LogicN**.

LogicN is still a concept and prototype project. This specification therefore
...
LogicN specification text uses these status words:

```text
required     = mandatory for a stable LogicN implementation
...
The LogicN specification covers:
...
The following rules define the official LogicN language contract at a root level.
...
`boot.lln` is the preferred full-project entry and configuration file.
Single-file scripts are allowed for small examples and learning material.

### Rule 2: Practical CPU Baseline

LogicN must remain useful on normal binary CPU systems.
...
for ordinary LogicN development.

### Rule 3: Strict Types

LogicN is strictly typed. It must reject:
...
LogicN must distinguish between:
...
Normal LogicN code must be memory-safe by default. The language must prevent or
...
LogicN must default to secure behaviour. At minimum, the language and toolchain
...
LogicN is JSON-native, but JSON handling must still be typed, bounded and safe.
...
If a requested target cannot be used, LogicN must report:
...
LogicN is not only a language; it is also an explainable build system.
...
LogicN may generate AI-oriented outputs such as `ai-context`, AI guides and
`LogicN explain --for-ai`, but those outputs must stay compact, deterministic and
...
A conforming LogicN implementation must aim to provide:
...
Stable LogicN releases should reject unsafe or ambiguous programs before target
generation whenever possible.

## Runtime and Build Semantics

LogicN supports both checked source execution and compiled/build output generation.
...
LogicN diagnostics and generated artefacts are part of the language contract, not
...
Until LogicN reaches a stable release, it is useful to distinguish conformance
levels:
...
Changes to the LogicN language contract should update, at minimum:
...
LogicN is defined by five non-negotiable ideas:
...
Any future LogicN implementation should be judged against those rules first.
