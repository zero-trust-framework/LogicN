# LogicN Prototype Compiler

This folder contains the first runnable LogicN prototype CLI.

It is not a production compiler. It is a practical v0.1 scaffold that can:

- discover `.lln` source files
- lex `.lln` source into source-mapped tokens
- format `.lln` files with stable two-space indentation
...
- report planned development serve mode through `LogicN serve --dev`
- generate JSON Schema drafts and OpenAPI drafts from LogicN declarations
...
node compiler/logicn.js serve examples --dev
node compiler/logicn.js init my-logicn-app
...
`LogicN verify` checks this metadata alongside required files, output hashes and
JSON report parsing, so placeholder artefacts cannot be mistaken for runnable
platform binaries in a valid build manifest.

Builds clean known logicn-generated outputs before writing new artefacts. This
...
`generatedOutputNaming` in `app.build-manifest.json` and checked by `LogicN verify`.
...
`sourceFiles` list with one hash per `.lln` source file.
...
Diagnostics now include standard LogicN codes, levels, categories, recovery
...
Development output commands now include `LogicN generate`, `LogicN dev`,
`LogicN dev --watch` and `LogicN run --generate`. They write reports and docs to
...
```bash
node compiler/logicn.js fmt examples --check
node compiler/logicn.js fmt examples
```

The first command reports files that would change. The second rewrites `.lln`
...
```bash
node compiler/logicn.js test examples
```

It checks that `hello.lln` parses as a secure `main` flow, `boot.lln` parses the
...
```bash
node compiler/logicn.js schema examples/api-orders.lln
node compiler/logicn.js schema examples/api-orders.lln --type CreateOrderRequest
node compiler/logicn.js openapi examples/api-orders.lln
```

Builds write `app.schemas.json` and `app.openapi.json`. The JSON Schema output
is a draft contract for LogicN type declarations and includes custom types in
`$defs`.
...
node compiler/logicn.js verify build/examples/app.build-manifest.json
```
...
The `boot.lln` `build` block may declare `require_outputs`; when
...
`runtime` blocks in `boot.lln` are extracted into `app.runtime-report.json` and
`docs/runtime-guide.md`.
...
The v0.1 `LogicN run` command executes simple checked `main` scripts with
`print("...")` or `console.log("...")` output. The v0.1 `LogicN serve --dev`
command reports the planned development runtime state; it does not start a
production HTTP server.

## Global Report

`globals` blocks in `boot.lln` are extracted into `app.global-report.json` and
`docs/global-registry-guide.md`.
...
`LogicN targets` emits the target compatibility report used by
`app.target-report.json`.

```bash
node compiler/logicn.js targets examples
```
...
`LogicN ai-context` writes compact JSON and Markdown summaries for AI tools:
