// @logicn/devtools-flowgraph — comprehensive test suite
...
function meta(name, qualifier, effects = []) {
  return {
    ...
    location: { line: 1, column: 1, file: "test.lln" },
  };
}
...
test("flowGraphToJson produces valid JSON", () => {
  ...
  assert.equal(parsed.schemaVersion, "lln.flowgraph.v1");
...
test("001: simple A→B→A cycle detected", () => {
  ...
  assert.equal(diags[0].code, "LLN-GRAPH-001");
...
test("002: flow not reachable from any route is dead", () => {
  ...
  assert.ok(diags.length > 0);
  assert.equal(diags[0].code, "LLN-GRAPH-002");
...
// ── LLN-GRAPH-003: Authority escalation ──────────────────────────────────────
...
test("003: plain flow calling database.write flow triggers escalation", () => {
  ...
  assert.ok(diags.length > 0);
  assert.equal(diags[0].code, "LLN-GRAPH-003");
  assert.equal(diags[0].severity, "error");
});
...
// ── LLN-GRAPH-004: PII leakage paths ─────────────────────────────────────────
...
test("004: pii flow → network.outbound without audit triggers leakage", () => {
  ...
  assert.ok(diags.length > 0);
  assert.equal(diags[0].code, "LLN-GRAPH-004");
  assert.equal(diags[0].severity, "error");
});
...
// ── LLN-GRAPH-005: Missing audit coverage ────────────────────────────────────
...
test("005: database.write without audit.write triggers warning", () => {
  ...
  assert.ok(diags.length > 0);
  assert.equal(diags[0].code, "LLN-GRAPH-005");
  assert.equal(diags[0].severity, "warning");
});
...
test("006: caller invoking same callee 3 times triggers warning", () => {
  ...
  assert.ok(diags.length > 0);
  assert.equal(diags[0].code, "LLN-GRAPH-006");
  assert.equal(diags[0].severity, "warning");
});
...
test("checkFlowGraph aggregates multiple diagnostic codes", () => {
  ...
  assert.ok(codes.includes("LLN-GRAPH-002"), "Expected dead flow diag");
  assert.ok(codes.includes("LLN-GRAPH-005"), "Expected missing audit diag");
});
