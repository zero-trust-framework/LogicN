...
LogicN does not claim to make Ethernet hardware faster.

LogicN's responsibility is governed network behavior:
...
`logicn-framework-app-kernel` owns request lifecycle enforcement.
`logicn-core-security` owns permission and secret-safe sink policy.
...
## Diagnostic Codes (LLN-NETWORK-001–008)
...
| `LLN-NETWORK-008` | runtime network policy unavailable, replay violation or governance metadata missing |
...
logicn-core-security decides permissions and secret-safe policy.
