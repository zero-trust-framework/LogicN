// =============================================================================
// //lln: source WRITER (R&D 0045, decision #3: silently overwrite the machine-owned tier)
// rewriteGeneratedComments() must touch ONLY //lln: lines — never a human // line or any code.
// =============================================================================
...
const gen = new Map([
  ["leaf", ["//lln: USEDBY: (1) top", "//lln: IMPACT: (1)", "//lln: COMPLEXITY: 3"]],
  ["top", ["//lln: USES: (1) leaf", "//lln: IMPACT: (0) — safe to delete"]],
]);

describe("rewriteGeneratedComments — safe //lln: overwrite", () => {
  it("inserts the //lln: block immediately above each flow, preserving human // + code", () => {
    const src = [
      "// HUMAN: stable core",
    ...
    assert.deepEqual(out.slice(1, 4), ["//lln: USEDBY: (1) top", "//lln: IMPACT: (1)", "//lln: COMPLEXITY: 3"]);
  ...
  it("OVERWRITES a stale //lln: block (does not append a second one)", () => {
    const stale = [
      "//lln: USEDBY: (99) wrong",
      "//lln: IMPACT: (99)",
      "pure flow leaf(x: Int) -> Int { return x }",
    ...
    assert.equal((out.match(/\/\/lln: USEDBY/g) || []).length, 1, "exactly one USEDBY line (old replaced, not duplicated)");
    assert.ok(out.includes("//lln: USEDBY: (1) top"), "the fresh value replaced the stale (99)");
  ...
  it("preserves the flow's indentation on the inserted //lln: lines", () => {
    const src = "  pure flow leaf(x: Int) -> Int { return x }";
    const out = rewriteGeneratedComments(src, gen).split("\n");
    assert.ok(out[0].startsWith("  //lln:"), "inserted lines match the flow's indentation");
