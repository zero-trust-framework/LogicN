...
function parseAndCheck(source) {
  const parsed = parseProgram(source, "financial.lln");
...
function parse(source) {
  return parseProgram(source, "financial.lln");
}
...
describe("Financial — Money type declarations", () => {
  it("accepts Money<GBP> as a parameter type without LLN-TYPE-001", () => {
    const result = parseAndCheck(`
pure flow getPrice(amount: Money<GBP>) -> Money<GBP> {
  return amount
}
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-001"), "Money<GBP> should be a known type");
  });

  it("accepts Money<USD> as a parameter type without LLN-TYPE-001", () => {
    const result = parseAndCheck(`
pure flow convert(amount: Money<USD>) -> Money<USD> {
  return amount
}
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-001"), "Money<USD> should be a known type");
  });

  it("accepts Money<EUR> as a parameter type without LLN-TYPE-001", () => {
    const result = parseAndCheck(`
pure flow price(amount: Money<EUR>) -> Money<EUR> {
  return amount
}
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-001"), "Money<EUR> should be a known type");
  });

  it("accepts Money<JPY> as a parameter type without LLN-TYPE-001", () => {
    const result = parseAndCheck(`
pure flow jpy(amount: Money<JPY>) -> Money<JPY> {
  return amount
}
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-001"), "Money<JPY> should be a known type");
  });
...
`);
    const typeErrors = diagsWithCode(result, "LLN-TYPE-001");
    assert.equal(typeErrors.length, 0, "No unknown-type errors for all four currency Money types");
  });

  it("does not emit LLN-TYPE-009 for Money<GBP> (correct arity)", () => {
    const result = parseAndCheck(`
pure flow gross(price: Money<GBP>) -> Money<GBP> {
  return price
}
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-009"), "Money<GBP> has arity 1 — no arity error");
  });
});
...
describe("Financial — valid same-currency arithmetic (type check)", () => {
  it("does not emit LLN-TYPE-004 for Money<GBP> + Money<GBP>", () => {
    const result = parseAndCheck(`
flow test() -> String {
...
`);
  ...
  it("does not emit LLN-TYPE-004 for Money<USD> + Money<USD>", () => {
    const result = parseAndCheck(`
flow test() -> String {
...
`);
  ...
  it("does not emit LLN-TYPE-004 for Money<EUR> - Money<EUR>", () => {
    const result = parseAndCheck(`
flow test() -> String {
...
// ── 3. Invalid cross-currency: LLN-TYPE-004 ──────────────────────────────────

describe("Financial — cross-currency rejection (LLN-TYPE-004)", () => {
  it("emits LLN-TYPE-004 for Money<GBP> + Money<USD>", () => {
    const result = parseAndCheck(`
flow test() -> String {
...
`);
    assert.ok(
      ...
      "Expected LLN-TYPE-004 for Money<GBP> + Money<USD>",
    );
    const diag = diagsWithCode(result, "LLN-TYPE-004").find((d) => d.message.includes("currency"));
    assert.ok(diag !== undefined, "LLN-TYPE-004 message must mention 'currency'");
  });

  it("emits LLN-TYPE-004 for Money<GBP> + Money<EUR>", () => {
    const result = parseAndCheck(`
flow test() -> String {
...
`);
    assert.ok(hasDiag(result, "LLN-TYPE-004"), "Expected LLN-TYPE-004 for GBP + EUR");
  });

  it("emits LLN-TYPE-004 for Money<USD> - Money<JPY>", () => {
    const result = parseAndCheck(`
flow test() -> String {
...
`);
    assert.ok(hasDiag(result, "LLN-TYPE-004"), "Expected LLN-TYPE-004 for USD - JPY");
  });
...
`, "takeHome");
  ...
  it("Decimal type is recognised without LLN-TYPE-001", () => {
    const result = parseAndCheck(`
...
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-001"), "Decimal must be a known type");
  });
});
...
describe("Financial — brand types: AccountId, TransactionId, CardNumber", () => {
  it("type checker accepts AccountId as a Brand<String, ...> alias (alias itself not flagged)", () => {
    // The string literal "AccountId" inside Brand<> may emit LLN-TYPE-001 — that is expected.
...
`);
    ...
    assert.ok(
      ...
      "AccountId alias itself must not appear in LLN-TYPE-001 messages",
    );
  });

  it("type checker accepts TransactionId as a Brand<String, ...> alias (alias itself not flagged)", () => {
    // The string literal "TransactionId" inside Brand<> may emit LLN-TYPE-001 — that is expected.
    const result = parseAndCheck(`
...
`);
    ...
    assert.ok(
      ...
      "TransactionId alias itself must not appear in LLN-TYPE-001 messages",
    );
  });

  it("type checker accepts CardNumber as a Brand<String, ...> alias (alias itself not flagged)", () => {
    // The string literal "CardNumber" inside Brand<> may emit LLN-TYPE-001 — that is expected.
    const result = parseAndCheck(`
...
`);
    ...
    assert.ok(
      ...
      "CardNumber alias itself must not appear in LLN-TYPE-001 messages",
    );
  });

  it("emits LLN-TYPE-003 when a string literal is directly assigned to AccountId", () => {
    const result = parseAndCheck(`
type AccountId = Brand<String, "AccountId">
...
`);
    assert.ok(
      ...
      "Assigning a raw string to AccountId (Brand) must emit LLN-TYPE-003",
    );
  });

  it("emits LLN-TYPE-003 when a string literal is directly assigned to TransactionId", () => {
    const result = parseAndCheck(`
type TransactionId = Brand<String, "TransactionId">
...
`);
    assert.ok(
      hasDiag(result, "LLN-TYPE-003"),
      "Assigning a raw string to TransactionId must emit LLN-TYPE-003",
    );
  });

  it("emits LLN-TYPE-003 when a string literal is assigned to CardNumber", () => {
    const result = parseAndCheck(`
type CardNumber = Brand<String, "CardNumber">
...
`);
    assert.ok(
      hasDiag(result, "LLN-TYPE-003"),
      "Assigning a raw card number string to CardNumber brand must emit LLN-TYPE-003",
    );
  });

  it("LLN-TYPE-003 diagnostic for CardNumber includes a suggested validate gate", () => {
    const result = parseAndCheck(`
type CardNumber = Brand<String, "CardNumber">
...
`);
    ...
    assert.ok(diag !== undefined, "Expected LLN-TYPE-003 for unsafe CardNumber assignment");
    assert.ok(
  ...
  it("does not emit LLN-TYPE-003 for a plain String binding (no brand)", () => {
    const result = parseAndCheck(`
flow test() -> String {
...
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-003"), "Plain String binding must not emit LLN-TYPE-003");
  });
});
