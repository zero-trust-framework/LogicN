...
// ---------------------------------------------------------------------------
// Test 1: request_time 0.001s triggers LLN-RUNTIME-006
// ---------------------------------------------------------------------------

describe("R4+R6 complete: full enforcement", () => {

  it("1. Flow with very short request_time limit reports LLN-RUNTIME-006", async () => {
...
`;
    ...
    const has006 = result.diagnostics.some((d) => d.code === "LLN-RUNTIME-006");
    assert.ok(has006, `Expected LLN-RUNTIME-006 in diagnostics; got: ${JSON.stringify(result.diagnostics)}`);
  });
  ...
  it("2. CapabilityHost with networkCallLimit 0 denies network call with LLN-RUNTIME-006", async () => {
    ...
    assert.ok(
      reason.includes("LLN-RUNTIME-006"),
      `Expected LLN-RUNTIME-006 in denial reason; got: "${reason}"`,
    );
  });
  ...
  it("5. manifest.requiresAudit = true triggers LLN-RUNTIME-007 when AuditLog.write not called", async () => {
    ...
    const has007 = result.diagnostics.some((d) => d.code === "LLN-RUNTIME-007");
    assert.ok(
      has007,
      `Expected LLN-RUNTIME-007 diagnostic when requiresAudit=true and AuditLog.write not called; got: ${JSON.stringify(result.diagnostics)}`,
    );
  });
