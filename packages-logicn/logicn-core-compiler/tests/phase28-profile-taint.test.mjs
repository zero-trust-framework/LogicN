 ...
 *
 * Tests:
 *   - checkProfiles: LLN-PROFILE-001 (recursion), -002 (unbounded loop), -006 (budget)
 *   - checkTaint:    LLN-TAINT-001 (tainted→sink), -003 (wrong context), -004 (discouraged)
 */
...
import {
  parseProgram,
  checkProfiles, LLN_PROFILE_001, LLN_PROFILE_002, LLN_PROFILE_006,
  checkTaint, LLN_TAINT_001, LLN_TAINT_003, LLN_TAINT_004,
...
describe("Phase 28: profile enforcement", () => {
  ...
  it("LLN-PROFILE-001: recursion in strict profile", () => {
    assert.ok(profileCodes(recursiveFib, ["strict"]).includes("LLN-PROFILE-001"));
  });

  it("LLN-PROFILE-001: recursion in high_integrity profile", () => {
    assert.ok(profileCodes(recursiveFib, ["high_integrity"]).includes("LLN-PROFILE-001"));
  });
  ...
  it("LLN-PROFILE-002: unbounded loop in strict profile", () => {
    const src = [
      "pure flow s(n: Int) -> Int contract { effects {} }",
      "{ mut t: Int = 0  mut i: Int = 0  while i < n { t = t + i  i = i + 1 } return t }",
    ].join("\n");
    assert.ok(profileCodes(src, ["strict"]).includes("LLN-PROFILE-002"));
  });
  ...
  it("LLN-PROFILE-006: high_integrity without runtime budget warns", () => {
    const src = "secure flow f(n: Int) -> Int contract { effects { audit.write } } { return n }";
    assert.ok(profileCodes(src, ["high_integrity"]).includes("LLN-PROFILE-006"));
  });

  it("LLN_PROFILE constants have correct codes", () => {
...
describe("Phase 28: taint tracking", () => {
  it("LLN-TAINT-001: tainted request reaches SQL sink", () => {
    const src = [
  ...
  it("LLN-TAINT-003: value cleaned for HTML used at SQL sink", () => {
    const src = [
      "secure flow q(req: Request) -> Response contract { effects { database.read } }",
  ...
  it("LLN-TAINT-004: discouraged Sql.escape warns", () => {
    const src = [
      "secure flow q(req: Request) -> Response contract { effects { database.read } }",
