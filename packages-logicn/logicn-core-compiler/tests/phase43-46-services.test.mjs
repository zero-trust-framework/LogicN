 ...
 *
 * Tests four new governed HTTP services that form the runtime-in-LogicN
 * introspection and audit layer:
 *
 *   Phase 43 — governanceService.lln      POST /governance/verify  (port 3920)
 *   Phase 44 — auditChainService.lln      POST /audit/chain        (port 3921)
 *   Phase 45 — proofVerifierService.lln   POST /proof/verify       (port 3922)
 *   Phase 46 — typeRegistryService.lln    POST /types/resolve      (port 3923)
 */
...
// ── Phase 43: governanceService.lln ──────────────────────────────────────────

describe("Phase 43: governanceService.lln", () => {
  let server;
  const PORT = 3920;
  const url = `http://127.0.0.1:${PORT}/governance/verify`;

  before(async () => {
    const prog = loadService("governanceService.lln");
  ...
  it("governanceService.lln parses with zero errors", () => {
    const prog = loadService("governanceService.lln");
...
// ── Phase 44: auditChainService.lln ──────────────────────────────────────────

describe("Phase 44: auditChainService.lln", () => {
  let server;
  const PORT = 3921;
  const url = `http://127.0.0.1:${PORT}/audit/chain`;

  before(async () => {
    const prog = loadService("auditChainService.lln");
    server = await startServer(prog.ast, prog.flows, { port: PORT });
  });
  ...
  it("auditChainService.lln parses with zero errors", () => {
    const prog = loadService("auditChainService.lln");
    const errs = (prog.diagnostics ?? []).filter(d => d.severity === "error");
    assert.equal(errs.length, 0, errs.map(e => e.message).join(", "));
  });
...
// ── Phase 45: proofVerifierService.lln ───────────────────────────────────────

describe("Phase 45: proofVerifierService.lln", () => {
  let server;
  const PORT = 3922;
  const url = `http://127.0.0.1:${PORT}/proof/verify`;

  before(async () => {
    const prog = loadService("proofVerifierService.lln");
    server = await startServer(prog.ast, prog.flows, { port: PORT });
  });
  ...
  it("proofVerifierService.lln parses with zero errors", () => {
    const prog = loadService("proofVerifierService.lln");
    const errs = (prog.diagnostics ?? []).filter(d => d.severity === "error");
    assert.equal(errs.length, 0, errs.map(e => e.message).join(", "));
  });
...
// ── Phase 46: typeRegistryService.lln ────────────────────────────────────────

describe("Phase 46: typeRegistryService.lln", () => {
  let server;
  const PORT = 3923;
  const url = `http://127.0.0.1:${PORT}/types/resolve`;

  before(async () => {
    const prog = loadService("typeRegistryService.lln");
    server = await startServer(prog.ast, prog.flows, { port: PORT });
  });
  ...
  it("typeRegistryService.lln parses with zero errors", () => {
    const prog = loadService("typeRegistryService.lln");
    const errs = (prog.diagnostics ?? []).filter(d => d.severity === "error");
    assert.equal(errs.length, 0, errs.map(e => e.message).join(", "));
  });
