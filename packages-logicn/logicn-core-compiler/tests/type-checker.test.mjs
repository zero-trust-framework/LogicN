...
// ── LLN-TYPE-023: mandatory wildcard arm (supersedes LLN-TYPE-021) ────────────
...
describe("Type checker — LLN-TYPE-023 mandatory wildcard arm", () => {
  it("emits LLN-TYPE-023 when Option match has no wildcard", () => {
    const result = parseAndCheck(`
...
`);
    assert.ok(hasDiag(result, "LLN-TYPE-023"), "Expected LLN-TYPE-023 for Option match without _");
  });

  it("emits LLN-TYPE-023 when Result match has no wildcard", () => {
    const result = parseAndCheck(`
...
`);
    assert.ok(hasDiag(result, "LLN-TYPE-023"), "Expected LLN-TYPE-023 for Result match without _");
  });

  it("emits LLN-TYPE-023 when enum match has no wildcard", () => {
    const result = parseAndCheck(`
enum Status {
  Active
  Suspended
...
`);
    assert.ok(hasDiag(result, "LLN-TYPE-023"), "Expected LLN-TYPE-023 for enum match without _");
  });

  it("emits LLN-TYPE-023 even when all variants are present (wildcard is mandatory)", () => {
    const result = parseAndCheck(`
flow test(x: Option<String>) -> String {
  match x {
    Some(v) => v
...
`);
    assert.ok(hasDiag(result, "LLN-TYPE-023"), "Wildcard mandatory even when Some+None both present");
  });

  it("does not emit LLN-TYPE-023 when wildcard _ arm is present", () => {
    const result = parseAndCheck(`
flow test(x: Option<String>) -> String {
  match x {
    Some(v) => v
...
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-023"), "Unexpected LLN-TYPE-023 when _ wildcard present");
  });

  it("does not emit LLN-TYPE-023 when else arm is present", () => {
    const result = parseAndCheck(`
flow test(x: Option<String>) -> String {
  match x {
    Some(v) => v
...
`);
    assert.ok(!hasDiag(result, "LLN-TYPE-023"), "else arm counts as the wildcard catch-all");
  });
});
...
// (LLN-TYPE-023 mandatory-wildcard coverage is consolidated into the suite above;
//  the former duplicate LLN-TYPE-021 non-exhaustive-match block was removed when
