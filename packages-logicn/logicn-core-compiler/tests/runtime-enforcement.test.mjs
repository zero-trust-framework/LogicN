...
`;
    const result = await run(source, "test.lln", "add", new Map([
      ["a", { __tag: "int", value: 3 }],
      ["b", { __tag: "int", value: 4 }],
    ]));
...
`;
    ...
    const result = await run(source, "test.lln", "doWork", new Map(), {
      deadlineMs: 10_000,
    });
...
`;
    const result = await run(source, "test.lln", "identity", new Map([
      ["x", { __tag: "int", value: 99 }],
    ]), { deadlineMs: 60_000 });

    assert.equal(result.ok, true);
    assert.ok(
      !result.diagnostics.some((d) => d.code === "LLN-RUNTIME-DEADLINE"),
...
`;
    ...
    const result = await run(source, "test.lln", "slowFlow", new Map(), {
      deadlineMs: 1,
    });
    ...
    if (result.ok === false || result.value?.__tag === "err") {
      ...
      if (errValue.__tag === "err") {
        assert.ok(
          ...
          errValue.error.value.includes("LLN-TIMEOUT"),
          `expected LLN-TIMEOUT in error message, got: ${JSON.stringify(errValue.error)}`,
        );
      }
    }
...
`;
    ...
    const resultPromise = run(source, "test.lln", "quick", new Map(), {
      deadlineMs: 1,
    });
