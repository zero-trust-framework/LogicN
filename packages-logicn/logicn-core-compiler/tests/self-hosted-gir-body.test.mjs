/**
 * Self-hosted GIR body emitter (gir-emitter.lln, M-B body section) — execution tests.
 *
...
const GIR_LLN = join(__dir, "..", "src", "self-hosted", "gir-emitter.lln");
const program = parseProgram(readFileSync(GIR_LLN, "utf8"), "gir-emitter.lln");
...
describe("gir-emitter.lln (body GIR) — parses clean", () => {
...
// ── FLOW TABLE: parse real source via lexer.lln + parser.lln, feed parseFlows'
//    output straight into buildFlowTable, read back the FlowEntry records. ──
const SH_DIR = join(__dir, "..", "src", "self-hosted");
const lexer = parseProgram(readFileSync(join(SH_DIR, "lexer.lln"), "utf8"), "lexer.lln");
const parser = parseProgram(readFileSync(join(SH_DIR, "parser.lln"), "utf8"), "parser.lln");
...
describe("gir-emitter.lln — buildFlowTable (cross-flow table for the runtime)", () => {
...
describe("gir-emitter.lln — expression lowering (real lowerExpr inside statements)", () => {
...
describe("gir-emitter.lln — statement lowering", () => {
