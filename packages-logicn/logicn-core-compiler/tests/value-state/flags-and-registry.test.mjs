...
// ---------------------------------------------------------------------------
// LLN_GATE_001 constant
// ---------------------------------------------------------------------------

describe("LLN_GATE_001: constant shape", () => {
  it("has correct code and name", () => {
    assert.equal(LLN_GATE_001.code, "LLN-GATE-001");
    assert.equal(LLN_GATE_001.name, "GateAnnotationRequired");
    assert.equal(LLN_GATE_001.severity, "error");
  });

  it("has why and suggestedFix", () => {
    assert.ok(typeof LLN_GATE_001.why === "string");
    assert.ok(typeof LLN_GATE_001.suggestedFix === "string");
    assert.ok(LLN_GATE_001.suggestedFix.includes("?"), "suggestedFix must include ? operator");
  });

  it("suggestedFix uses canonical 'safe mut' form", () => {
    assert.ok(LLN_GATE_001.suggestedFix.includes("safe mut"), "Must use 'safe mut' form");
  });
});
