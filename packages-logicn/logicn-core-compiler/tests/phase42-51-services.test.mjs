 ...
 * Phase 51 (manifest verification / v1.0 RC). Each service is a .lln file
...
describe("Phase 42: capabilityHostService.lln", () => {
  let server;
  const PORT = 3930;
  before(async () => { server = await startServer(loadService("capabilityHostService.lln").ast, loadService("capabilityHostService.lln").flows, { port: PORT }); });
...
describe("Phase 47: routingPolicyService.lln", () => {
  let server;
  const PORT = 3931;
  const prog = loadService("routingPolicyService.lln");
...
describe("Phase 48: economicsService.lln", () => {
  let server;
  const PORT = 3932;
  const prog = loadService("economicsService.lln");
  before(async () => { server = await startServer(prog.ast, prog.flows, { port: PORT }); });
  after(async () => server?.close());
...
describe("Phase 49: valueClassificationService.lln", () => {
  let server;
  const PORT = 3933;
  const prog = loadService("valueClassificationService.lln");
  before(async () => { server = await startServer(prog.ast, prog.flows, { port: PORT }); });
  after(async () => server?.close());
...
describe("Phase 50: runtimeProfileService.lln", () => {
  let server;
  const PORT = 3934;
  const prog = loadService("runtimeProfileService.lln");
  before(async () => { server = await startServer(prog.ast, prog.flows, { port: PORT }); });
  after(async () => server?.close());
...
describe("Phase 51: manifestVerificationService.lln", () => {
  let server;
  const PORT = 3935;
  const prog = loadService("manifestVerificationService.lln");
  before(async () => { server = await startServer(prog.ast, prog.flows, { port: PORT }); });
  after(async () => server?.close());
  ...
  it("valid signed manifest with flags → accepted, trustLevel=4", async () => {
    const r = await post({ schemaVersion: "lln.runtime.v1", verified: true, hasSignature: true, governanceFlagsMask: 7 });
  ...
  it("valid unsigned manifest → trustLevel=3, accepted", async () => {
    const r = await post({ schemaVersion: "lln.runtime.v1", verified: true, hasSignature: false, governanceFlagsMask: 3 });
    assert.equal(r.trustLevel, 3);
    assert.equal(r.accepted, true);
  });

  it("unverified manifest → trustLevel=0, rejected", async () => {
    const r = await post({ schemaVersion: "lln.runtime.v1", verified: false, hasSignature: false, governanceFlagsMask: 0 });
    assert.equal(r.trustLevel, 0);
    assert.equal(r.accepted, false);
  });

  it("all .lln service files from Phase 42-51 parse with zero errors", () => {
    const services = [
      "capabilityHostService.lln",
      "routingPolicyService.lln",
      "economicsService.lln",
      "valueClassificationService.lln",
      "runtimeProfileService.lln",
      "manifestVerificationService.lln",
    ];
