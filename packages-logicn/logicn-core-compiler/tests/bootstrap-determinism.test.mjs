...
//
// Proves that the LogicN compiler produces stable, deterministic output when
// given the same input — the foundation of `logicn verify-selfhost`.
//
...
describe("bootstrap-determinism: idempotent hash", () => {
  it("canonicalHash of same object twice → identical strings", () => {
    const obj = { compiler: "logicn", phase: 16, stable: true };
...
describe("bootstrap-determinism: different inputs produce different hashes", () => {
  it("canonicalHash of different objects → different strings", () => {
    const h1 = canonicalHash({ compiler: "logicn", version: 1 });
    const h2 = canonicalHash({ compiler: "logicn", version: 2 });
...
`;
    const parsed = parseProgram(source, "hashme.lln");
...
`;
    const parsed = parseProgram(source, "stable.lln");
    const errors = parsed.diagnostics.filter((d) => d.severity === "error");
    assert.equal(errors.length, 0, `Parse errors: ${errors.map((e) => e.message).join(", ")}`);
