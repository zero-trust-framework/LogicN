...
//   - LLN-SYNTAX-LEGACY-001 fires for 'with effects [...]'
...
`;
  ...
  it("offset and endOffset are non-negative integers", () => {
    ...
    const result = parseProgram(source, "add.lln");
...
describe("parser spans: ParseDiagnostic carries byteSpan", () => {
  it("error diagnostic from bad syntax has byteSpan [start, end]", () => {
    ...
    const result = parseProgram(source, "bad.lln");
    const errors = result.diagnostics.filter((d) => d.severity === "error");
...
`;
    const result = parseProgram(source, "canonical.lln");
    const legacyWarnings = result.diagnostics.filter((d) => d.code === "LLN-SYNTAX-LEGACY-001");
    assert.equal(
      legacyWarnings.length,
      0,
      "Canonical contract.effects must not trigger LLN-SYNTAX-LEGACY-001",
    );
  });

  it("LLN_SYNTAX_LEGACY_001 constant has correct shape", () => {
    assert.equal(LLN_SYNTAX_LEGACY_001.code, "LLN-SYNTAX-LEGACY-001");
    assert.equal(LLN_SYNTAX_LEGACY_001.name, "LegacyEffectsSyntax");
    assert.equal(LLN_SYNTAX_LEGACY_001.severity, "warning");
    assert.ok(typeof LLN_SYNTAX_LEGACY_001.message === "string");
    assert.ok(typeof LLN_SYNTAX_LEGACY_001.suggestedFix === "string");
  });
});
...
describe("parser: suggestedFix uses 'request' not 'req'", () => {
  it("LLN-SYNTAX-008 suggestedFix says 'readonly request: Request'", () => {
    // Trigger LLN-SYNTAX-008: unsafe let at top level
    const source = `unsafe let rawBody = readBody()\n`;
    const result = parseProgram(source, "unsafe-top.lln");
    const diag = result.diagnostics.find((d) => d.code === "LLN-SYNTAX-008");
    assert.ok(diag !== undefined, "Expected LLN-SYNTAX-008");
...
// ---------------------------------------------------------------------------
// Recovery: single error must not cascade into many LLN-PARSE-001 errors
// ---------------------------------------------------------------------------
...
`;
    const result = parseProgram(source, "recovery.lln");
...
`;
    const result = parseProgram(source, "multi-error.lln");
    const errors = result.diagnostics.filter((d) => d.severity === "error");
