...
function parseOk(source, label) {
  const result = parseProgram(source, label ?? "test.lln");
...
const getPatientSource = readFileSync(
  join(__dirname, "../../../examples/healthcare/getPatient.lln"),
  "utf8",
);

describe("Phase 26: getPatient example parses correctly", () => {
  it("produces 0 parse errors", () => {
    parseOk(getPatientSource, "getPatient.lln");
  });

  it("effects include phi.read and audit.write", () => {
    const result = parseOk(getPatientSource, "getPatient.lln");
