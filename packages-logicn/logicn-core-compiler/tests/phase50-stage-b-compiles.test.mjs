...
//
// Milestone: LogicN code (lexer.lln + parser.lln) compiles LogicN code to a
// parsed AST that the Stage A executor can run. The result must match Stage A.
//
// This is the first time "LogicN compiles LogicN" produces something executable.
// Not a complete compiler — a bootstrap milestone.
//
// Pipeline:
//   1. Stage B lexer (lexer.lln) tokenises the source
//   2. Stage B parser (parser.lln) produces ParseResult (FlowDecl records)
...
const lexerSrc  = readFileSync(join(SB_DIR, "lexer.lln"),  "utf8");
const parserSrc = readFileSync(join(SB_DIR, "parser.lln"), "utf8");

const lexerProg  = parseProgram(lexerSrc,  "lexer.lln");
const parserProg = parseProgram(parserSrc, "parser.lln");
...
describe("Phase 50: Stage B pipeline (lexer.lln → parser.lln)", () => {

  it("lexer.lln and parser.lln parse with zero Stage A errors", () => {
    assert.equal(lexErrors.length, 0,
      `lexer.lln Stage A errors: ${lexErrors.map(e=>e.message.slice(0,60)).join(", ")}`);
    assert.equal(parseErrors.length, 0,
      `parser.lln Stage A errors: ${parseErrors.map(e=>e.message.slice(0,60)).join(", ")}`);
  });
  ...
  it("Stage B and Stage A agree on flow name and return type", async () => {
    ...
    // Stage A
    const stageA = parseProgram(src, "test.lln");
  ...
  it("Stage B output can be fed into Stage A executor (the milestone)", async () => {
    ...
    // This is the 'LogicN compiles LogicN' proof-of-concept.
