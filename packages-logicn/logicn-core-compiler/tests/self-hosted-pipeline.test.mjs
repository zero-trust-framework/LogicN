 ...
 *
 * Proves the Stage-B compiler composes end-to-end IN LogicN: a source string is
 * lexed by lexer.lln, the token stream is parsed by parser.lln, and the resulting
 * flow records are type-checked by type-checker.lln — with the LogicNValue outputs
...
before(() => {
  lexer = load("lexer.lln");
  parser = load("parser.lln");
  checker = load("type-checker.lln");
  effect = load("effect-checker.lln");
  govern = load("governance-verifier.lln");
  gir = load("gir-emitter.lln");
  runtime = load("runtime.lln");
});

const vStr = (s) => ({ __tag: "string", value: s });

async function pipeline(source) {
  // 1. lexer.lln: source -> Result<Array<Token>>
  ...
  // 2. parser.lln: tokens -> ParseResult { flows, errors }
  ...
  // 3. type-checker.lln: return-expr check (checkFlows) + full body check (checkFlowBodies)
  const readDiags = (res) =>
...
describe("self-hosted pipeline — lexer → parser → type-checker (M-A/M-B)", () => {
  ...
  it("a String returned where Int is declared is caught as LLN-TYPE-002 by the self-hosted checker", async () => {
    ...
    assert.deepEqual(diags, [{ code: "LLN-TYPE-002", flowName: "bad" }]);
  });
...
describe("self-hosted pipeline — full body AST + body type-check (M-A fold → M-B)", () => {
  ...
  it("checkFlowBodies catches a bad let binding in the body via the real parser", async () => {
    ...
    assert.deepEqual(bodyDiags, [{ code: "LLN-TYPE-002", flowName: "bad" }]);
  });
...
describe("self-hosted pipeline — effect + governance over the parsed body AST (M-B)", () => {
  it("effect-checker derives undeclared effect use from the body's calls", async () => {
    ...
    assert.deepEqual(codesOn(r), [{ code: "LLN-EFFECT-001", flowName: "charge" }]);
  });
  ...
  it("governance flags a secure flow that never audits in its body", async () => {
    ...
    assert.deepEqual(codesOn(r), [{ code: "LLN-VAL-001", flowName: "charge" }]);
  });
...
describe("self-hosted pipeline — new grammar forms reach all downstream stages", () => {
  ...
  it("govern: a secure flow with no audit in either branch is flagged", async () => {
    ...
    const r = await executeFlow("checkBodyGovernance", new Map([["flows", flows]]), govern.ast);
    assert.deepEqual(codesOn(r), [{ code: "LLN-VAL-001", flowName: "bad" }]);
  });
});

// ── The full chain: source → lex → parse → emit GIR → EXECUTE, all in LogicN (S7 / M-C) ──
describe("self-hosted pipeline — execute the emitted GIR (S7: source→run, in LogicN)", () => {
...
// ── M-C: a recursive, MULTI-FLOW program compiles AND runs in LogicN ──
describe("self-hosted pipeline — multi-flow + recursion (M-C: LogicN runs LogicN)", () => {
  const vList = (items) => ({ __tag: "list", items });
  const vRec = (o) => { const f = new Map(); for (const [k, v] of Object.entries(o)) f.set(k, v); return { __tag: "record", fields: f }; };
  ...
  it("recursion: fib(10) → 55, computed entirely in LogicN", async () => {
