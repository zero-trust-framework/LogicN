...
// fail-open / monotonicity-violating declarations are rejected (LLN-FAULT-003 / LLN-FAULT-001).
...
describe("0017: fail-closed default + monotonicity diagnostics", () => {
  it("undeclared faults never fail open — secure default is halt (no diagnostic, clean)", () => {
    ...
    assert.ok(!has(g, "LLN-FAULT-003"), codes(g));
    assert.ok(!has(g, "LLN-FAULT-001"), codes(g));
  });

  it("LLN-FAULT-003: a fail-OPEN `log` outside on_rotation_fault is rejected", () => {
    const g = gov(withResilience("    on_timeout_fault log"));
    assert.ok(has(g, "LLN-FAULT-003"), codes(g));
  });

  it("`log` IS allowed on on_rotation_fault (back-compat opt-in) — no LLN-FAULT-003", () => {
  ...
  it("LLN-FAULT-001: on_denial_fault retry is rejected (deny-only monotonicity)", () => {
  ...
  it("a disallowed/fail-open action is COERCED to halt in the matrix (fail-closed by construction)", () => {
    ...
    // even though LLN-FAULT-003 fires, the materialised handler is the safe halt, never log
