// =============================================================================
// LogicN — epilogue {} auto-policy / EpilogueReceipt tests
//
// Tests:
...
//   7. invalid strategy → LLN-GOV-015 fired, no receipt
...
/**
 * Build a minimal governed LogicN program with an optional epilogue block.
...
describe("epilogue-receipt", () => {
  ...
  // -------------------------------------------------------------------------
  // Test 7: invalid strategy → LLN-GOV-015 fired, no receipt
  // -------------------------------------------------------------------------
  it("7. invalid strategy → LLN-GOV-015 diagnostic, no receipt generated", () => {
    const { govResult } = parseAndVerify(
    ...
    // LLN-GOV-015 should have been emitted
    const gov015 = govResult.diagnostics.find((d) => d.code === "LLN-GOV-015");
    assert.ok(gov015, "LLN-GOV-015 should be fired for invalid strategy");
  });
