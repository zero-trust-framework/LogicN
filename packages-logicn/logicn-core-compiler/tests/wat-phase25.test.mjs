...
//   25B. verifyPassword.lln: parses with 0 errors
//   25B. verifyPassword.lln: emitGIR produces correct declared effects
//   25B. verifyPassword.lln: buildWATModule produces WAT with host:* imports
...
const VERIFY_PASSWORD_PATH = join(__dir, "../../../examples/auth-service/verifyPassword.lln");
...
// ---------------------------------------------------------------------------
// 25B — verifyPassword.lln: parse + GIR effects + WAT imports
// ---------------------------------------------------------------------------

describe("Phase 25B: verifyPassword.lln parses and emits correct GIR + WAT", () => {
  let vpSource;
  try {
  ...
  it("verifyPassword.lln file exists and is readable", () => {
    assert.ok(
      vpSource !== null,
      `verifyPassword.lln not found at: ${VERIFY_PASSWORD_PATH}`,
    );
  });

  it("verifyPassword.lln parses with 0 errors", () => {
    if (!vpSource) return assert.ok(true, "skipped — file missing");
    const parsed = parseProgram(vpSource, "verifyPassword.lln");
    const errors = parsed.diagnostics.filter((d) => d.severity === "error");
    assert.equal(
      errors.length,
      0,
      `verifyPassword.lln parse errors (${errors.length}):\n${errors.map((e) => e.message).join("\n")}`,
    );
  });

  it("verifyPassword.lln emitGIR: declares database.read effect", () => {
    if (!vpSource) return assert.ok(true, "skipped — file missing");
    const parsed = parseProgram(vpSource, "verifyPassword.lln");
  ...
  it("verifyPassword.lln emitGIR: declares crypto.verify effect", () => {
    if (!vpSource) return assert.ok(true, "skipped — file missing");
    const parsed = parseProgram(vpSource, "verifyPassword.lln");
    const eff = checkEffects(parsed.flows, parsed.ast);
    const gir = emitGIR(parsed.ast, parsed.flows, eff);
  ...
  it("verifyPassword.lln emitGIR: declares audit.write effect", () => {
    if (!vpSource) return assert.ok(true, "skipped — file missing");
    const parsed = parseProgram(vpSource, "verifyPassword.lln");
    const eff = checkEffects(parsed.flows, parsed.ast);
    const gir = emitGIR(parsed.ast, parsed.flows, eff);
  ...
  it("verifyPassword.lln buildWATModule: WAT contains host:* imports", () => {
    if (!vpSource) return assert.ok(true, "skipped — file missing");
    const parsed = parseProgram(vpSource, "verifyPassword.lln");
    const eff = checkEffects(parsed.flows, parsed.ast);
    const gir = emitGIR(parsed.ast, parsed.flows, eff);
  ...
  it("verifyPassword.lln buildWATModule: WAT includes host:audit.write import", () => {
    if (!vpSource) return assert.ok(true, "skipped — file missing");
    const parsed = parseProgram(vpSource, "verifyPassword.lln");
    const eff = checkEffects(parsed.flows, parsed.ast);
    const gir = emitGIR(parsed.ast, parsed.flows, eff);
  ...
  it("verifyPassword.lln buildWATModule: WAT includes host:db.* import", () => {
    if (!vpSource) return assert.ok(true, "skipped — file missing");
    const parsed = parseProgram(vpSource, "verifyPassword.lln");
    const eff = checkEffects(parsed.flows, parsed.ast);
    const gir = emitGIR(parsed.ast, parsed.flows, eff);
  ...
  it("verifyPassword.lln buildWATModule: WAT includes host:crypto.verify import", () => {
    if (!vpSource) return assert.ok(true, "skipped — file missing");
    const parsed = parseProgram(vpSource, "verifyPassword.lln");
    const eff = checkEffects(parsed.flows, parsed.ast);
    const gir = emitGIR(parsed.ast, parsed.flows, eff);
