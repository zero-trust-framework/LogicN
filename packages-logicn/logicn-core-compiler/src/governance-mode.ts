 ...
 *
 * Design: docs/Knowledge-Bases/logicn-governance-mode-ladder.md (R&D job 0011).
 */
...
export function resolveGovernanceMode(input: GovernanceModeInputs): GovernanceModeResolution {
  ...
  if (STRICTNESS[flowRequest] < STRICTNESS[projectDefault]) {
    ...
    diagnostics.push(
      `LLN-CONFIG-GOV-001: flow requested governance:${flowRequest} but the project ceiling is ${projectDefault}; opt-down past the ceiling is rejected — using ${projectDefault}.`
    );
  }
  ...
  if (effectiveMode === "full") {
    ...
    if (leanEligible) {
      ...
      diagnostics.push(
        `LLN-CONFIG-GOV-002: flow requested governance:lean but has a declared/inferred effect or taint-to-sink; forced to full (lean cannot relax a gate that exists).`
      );
    }
  }
