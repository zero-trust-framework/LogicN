 ...
 * LogicN's UInt64 overflow / underflow / divide-by-zero TRAP semantics. Sibling of i64-arith.ts.
 *
 * Owner decision (Fork A = TRAP) carried to unsigned 64-bit: arithmetic must NEVER silently wrap. Native
 * WebAssembly `i64.add/sub/mul` wrap mod 2^64 (a lying abstraction in a governed system), so LogicN
 ...
 *
 * Why a layer SEPARATE from i64-arith (the split LLN-NUMERIC-001 deliberately preserved): an unsigned
 ...
 * which is the precise fail-open (silent precision loss) that gating scalar UInt64 behind LLN-NUMERIC-001
 ...
 * UInt64 stays fail-closed under LLN-NUMERIC-001 until the interpreter dispatch + gate-lift land. Building
