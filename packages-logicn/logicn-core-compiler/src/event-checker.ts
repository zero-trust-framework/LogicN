...
//
// Phase 9B+ additions:
//   LLN-EVENT-003: contract.events lists "emits X" but no global "event X" exists
//   LLN-EVENT-004: same event emitted more than once in a single flow body
//   LLN-EVENT-005: emit X in flow body but X not listed in contract.events
//
 ...
 * - LLN-EVENT-003: contract.events lists "emits X" but no global "event X" exists
 * - LLN-EVENT-004: same event emitted twice in a single flow without a match arm
 * - LLN-EVENT-005: emit X in a flow body but X not listed in contract.events
 */
export function checkEvents(ast: AstNode): EventCheckResult {
  ...
  // (used for LLN-EVENT-001 and LLN-EVENT-002)
  ...
  // ── Step 3: LLN-EVENT-001 — emitted name not globally declared ───────────
  ...
  // ── Step 4: LLN-EVENT-002 — globally declared name never emitted ─────────
  ...
  // ── Step 5: Per-flow checks (LLN-EVENT-003, 004, 005) ────────────────────
  for (const topLevelNode of ast.children ?? []) {
    ...
    // LLN-EVENT-003: contract lists emits X but no global event X
    for (const name of contractEmitNames) {
      if (!declaredEvents.has(name)) {
        diagnostics.push({
          code: LLN_EVENT_003.code,
          name: LLN_EVENT_003.name,
          severity: LLN_EVENT_003.severity,
    ...
    for (const [name, count] of seenInBody) {
      if (count > 1) {
        diagnostics.push({
          code: LLN_EVENT_004.code,
          name: LLN_EVENT_004.name,
          severity: LLN_EVENT_004.severity,
    ...
    // LLN-EVENT-005: emit X in body but X not listed in contract.events
    ...
    if (contractEmitNames.size > 0) {
      ...
      for (const { name, location } of bodyEmits) {
        if (!contractEmitNames.has(name) && !reported005.has(name)) {
          ...
          diagnostics.push({
            code: LLN_EVENT_005.code,
            name: LLN_EVENT_005.name,
            severity: LLN_EVENT_005.severity,
