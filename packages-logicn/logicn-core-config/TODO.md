...
[x] Implement EnvironmentMode unknown-mode diagnostic (LLN-CONFIG-001, LLN-CONFIG-002) — resolveEnvironmentMode() returns INVALID_ENVIRONMENT_MODE or MISSING_ENVIRONMENT_MODE
...
[ ] Implement loadEnvironmentConfig(input): Promise<{config, diagnostics}> with LLN-CONFIG-001, LLN-CONFIG-002
...
[x] Implement host package manifest boundary diagnostic (LLN-CONFIG-010) — validateHostPackageManifestBoundary() rejects LogicN keys from package.json; diagnostic rename pass complete — all codes now use LLN-CONFIG-001…027 format with {code, name, message} metadata (2026-05-26)
[x] Define ConfigVaultEntry<T>, ConfigVaultSchema, ConfigVaultResult, getVaultEntry<T>() (2026-05-26)
[x] Define LLN-VAULT-001 through LLN-VAULT-005 diagnostic codes and constructors (2026-05-26)
