...
`logicn-core-config` is the package for LogicN project configuration, environment mode and
...
- `HostPackageManifestBoundaryPolicy` - validation that keeps LogicN package graph
...
such as `logicn-tools-benchmark` and `logicn-devtools-*` are disabled by default in
production profiles.
...
It must not define LogicN package graph keys, runtime profiles, compiler target
...
```text
logicn-core-config describes configuration safely.
logicn-core-security protects sensitive values.
