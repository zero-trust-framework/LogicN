/**
 * @logicn/devtools-context — Integration Tests
 *
 * Tests the Context Receipt generator against both inline fixtures and
 * real auth-service .lln examples.
 */
...
describe("context-receipt: basic generation", () => {
  it("generates receipt for a clean pure flow", () => {
    const receipts = generateReceipts(CLEAN_PURE_FLOW, { fileName: "test.lln" });
    assert.equal(receipts.schemaVersion, "lln.context-receipt.v1");
...
describe("context-receipt: contract extraction", () => {
  ...
  it("flags hasSecrets: true when secrets{} block present", () => {
    const receipts = generateReceipts(FLOW_WITH_SECRETS_BLOCK, { fileName: "creds.lln" });
  ...
  it("flags hasEpilogue: true when epilogue{} block present", () => {
    const receipts = generateReceipts(FLOW_WITH_EPILOGUE_BLOCK, { fileName: "payment.lln" });
    assert.ok(receipts.receipts.length > 0, "Should have at least one receipt");
    const receipt = receipts.receipts[0];
    assert.ok(receipt !== undefined);
    assert.equal(
...
describe("context-receipt: body omission", () => {
  ...
  it("receipt JSON is substantially smaller than full source", () => {
    ...
    const receipts = generateReceipts(source, { fileName: "auth.lln" });
    const receipt = receipts.receipts[0];
    assert.ok(receipt !== undefined);
...
describe("context-receipt: token reduction on real files", () => {
  it("achieves >50% token reduction on verifyPassword.lln", () => {
    let source;
    try {
      source = readFileSync(join(AUTH_SERVICE_DIR, "verifyPassword.lln"), "utf8");
    } catch {
  ...
  it("each individual flow receipt is smaller than the full source file", () => {
    ...
    let source;
    try {
      source = readFileSync(join(AUTH_SERVICE_DIR, "auditWriterService.lln"), "utf8");
    } catch {
      return;
    }
    const receipts = generateReceipts(source, { fileName: "auditWriterService.lln" });
  ...
  it("handles multi-flow file: produces correct flow count", () => {
    ...
    const receipts = generateReceipts(source, { fileName: "auditWriterService.lln" });
    // auditWriterService.lln has 3 flows: classifyAuditSeverity, validateAuditEvent, writeAuditEvent
    assert.ok(receipts.flowCount >= 2,
      `Expected at least 2 flows in auditWriterService.lln, got ${receipts.flowCount}`
    );
  });
});
...
describe("context-receipt: flow filter", () => {
  ...
  it("generateFlowReceiptByName returns a single named flow", () => {
    const receipt = generateFlowReceiptByName(MULTI_FLOW_SOURCE, "square", "multi.lln");
  ...
  it("generateFlowReceiptByName returns undefined for unknown flow", () => {
    const receipt = generateFlowReceiptByName(MULTI_FLOW_SOURCE, "doesNotExist", "multi.lln");
  ...
  it("--flow filter on real file returns correct single receipt", () => {
    ...
    const receipt = generateFlowReceiptByName(source, "classifyAuditSeverity", "auditWriterService.lln");
...
describe("context-receipt: JSON output", () => {
  it("JSON output is valid and round-trips through JSON.parse", () => {
    ...
    assert.equal(parsed.schemaVersion, "lln.context-receipt.v1");
    assert.ok(Array.isArray(parsed.receipts));
  });

  it("single-flow JSON output has all required fields", () => {
    const receipt = generateFlowReceiptByName(SECURE_FLOW_WITH_CONTRACT, "verifyPassword", "auth.lln");
    assert.ok(receipt !== undefined);
...
describe("context-receipt: taint detection", () => {
  ...
  it("detects multiple taint sources in economics service", () => {
    let source;
    try {
      source = readFileSync(join(AUTH_SERVICE_DIR, "economicsService.lln"), "utf8");
    } catch {
      return;
    }
    const receipt = generateFlowReceiptByName(source, "estimateCost", "economicsService.lln");
...
describe("context-receipt: governance code inference", () => {
  it("infers LLN-GOV-010 for secure flow without intent", () => {
    const receipts = generateReceipts(SECURE_FLOW_MISSING_INTENT, { fileName: "missing.lln" });
    assert.ok(receipts.receipts.length > 0);
    const receipt = receipts.receipts[0];
    assert.ok(receipt !== undefined);
    const codes = receipt.governance.governanceCodes;
    assert.ok(
      codes.includes("LLN-GOV-010"),
      `Expected LLN-GOV-010 for secure flow without intent, got: ${codes.join(", ")}`,
    );
  });

  it("does NOT infer LLN-GOV-010 when intent is present", () => {
    const receipts = generateReceipts(SECURE_FLOW_WITH_CONTRACT, { fileName: "auth.lln" });
    const receipt = receipts.receipts[0];
    assert.ok(receipt !== undefined);
    const codes = receipt.governance.governanceCodes;
    assert.ok(
      !codes.includes("LLN-GOV-010"),
      `Should not have LLN-GOV-010 when intent is declared, got: ${codes.join(", ")}`,
    );
  });
});
