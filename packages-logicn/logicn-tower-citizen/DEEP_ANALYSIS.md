# Deep Analysis: logicn-tower-citizen

Date: 2026-06-06

Scope:
- `packages-logicn/logicn-tower-citizen`
- `packages-logicn/logicn-ext-bridge-cpp`
- `logicn.mjs` runtime wiring
- related Tower/Sentinel tests

Verification run:
- `packages-logicn/logicn-tower-citizen`: 71/71 tests passing
- `packages-logicn/logicn-ext-bridge-cpp`: 13/13 tests passing

## Executive Finding

`logicn-tower-citizen` is the current source of truth for LogicN's governed inference
...
- Brawn: `InferenceBridge` implementations, currently stub/simulator in
  `tower-citizen` and CPU/GPU BitNet bridge seams in `logicn-ext-bridge-cpp`
...
The Tower implements LogicN's core governance model for inference:
...
not for the whole LogicN language. Syntax, diagnostics, and manifest generation
...
- `BitNetCpuBridge`: implements the Tower bridge contract and uses the simulator
  ...
  (`../logicn-ext-bridge-cpp/src/bitnet-cpu-bridge.ts:29`)
- `BitNetGpuBridge`: detects NVIDIA hardware but currently uses the simulator
  because `cudaKernelReady` is false
  (`../logicn-ext-bridge-cpp/src/bitnet-gpu-bridge.ts:24`)

`createCppBridgeRegistry()` is the current Brain-to-Brawn wiring point
(`../logicn-ext-bridge-cpp/src/index.ts:48`). The CLI uses that registry when
running governed inference from a flow's `ai {}` contract (`../../logicn.mjs:514`).
...
- the GPU BitNet bridge detects NVIDIA hardware through `nvidia-smi`, but
  ...
  (`../logicn-ext-bridge-cpp/src/hardware-detect.ts:49`)
...
`logicn-ext-bridge-cpp` is the production bridge seam:
- `BitNetCpuBridge` loads `bitnet_addon.node` when present
  (`../logicn-ext-bridge-cpp/src/addon-loader.ts:52`)
- the native addon contract exposes `tmac(...)`
  (`../logicn-ext-bridge-cpp/src/addon-loader.ts:34`)
- the CPU bridge cross-checks native results against the simulator for the first
  eight calls (`../logicn-ext-bridge-cpp/src/bitnet-cpu-bridge.ts:40`,
  `../logicn-ext-bridge-cpp/src/bitnet-cpu-bridge.ts:109`)
...
(`../../logicn.mjs:476`, `../../logicn.mjs:488`, `../../logicn.mjs:499`). The engine
...
The package comments refer to `governance.lln`, but no package-local manifest or
...
In `logicn-ext-bridge-cpp`, `canCommit()` exists but is not called by `execute()`
before native or simulator execution (`../logicn-ext-bridge-cpp/src/bitnet-cpu-bridge.ts:66`,
`../logicn-ext-bridge-cpp/src/bitnet-cpu-bridge.ts:74`). That makes the Governance
