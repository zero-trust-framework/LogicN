...
//
// Spec + proofs: docs/Knowledge-Bases/logicn-three-valued-governance.md
...
describe("indeterminate at a trust boundary → deny + LLN-GOV-3VL-001 (never silent)", () => {
  ...
  it("an ordinary DENY at the boundary is NOT a 3VL diagnostic (it is a policy denial)", () => {
    ...
    assert.equal(d.decision, "deny");
    assert.equal(d.authorized, false);
    assert.equal(d.diagnostic, null, "DENY is definite — no LLN-GOV-3VL-001");
...
// Deterministic PRNG (no Math.random — reproducible, per LogicN determinism discipline).
...
// exactly classical Boolean — and NO LLN-GOV-3VL-001 is ever emitted.
