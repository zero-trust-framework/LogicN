...
//
// Spec + proofs: docs/Knowledge-Bases/logicn-substrate-failure-model.md
...
describe("acceptance #1: tolerance unachievable under noise is flagged", () => {
  ...
  it("met=false and exactly one LLN-SUBSTRATE-002, payload epsilonModeled > epsilonDeclared", () => {
    ...
    assert.equal(prod.diagnostic.code, "LLN-SUBSTRATE-002");
    assert.equal(prod.diagnostic.severity, "error");
  });
  it("dev profile downgrades LLN-SUBSTRATE-002 to warning but changes ONLY severity (verdict/code/check invariant)", () => {
    ...
    assert.equal(dev.diagnostic.code, "LLN-SUBSTRATE-002");
...
describe("acceptance #2: raising redundancy clears the diagnostic, monotonically", () => {
  ...
  it("pBad≥0.5: redundancyHelps=false, the trace NEVER clears (non-decreasing), and LLN-SUBSTRATE-003 fires", () => {
    const eps = 1e-3;
    ...
    assert.equal(d.diagnostic.code, "LLN-SUBSTRATE-003");
...
describe("no-regression: a noiseless substrate is a perfect pass-through", () => {
  ...
  it("crypto on a CLEAN lane does NOT trip LLN-SUBSTRATE-001 (only noisy lanes do)", () => {
    const g = { resultId: "r", epsilonDeclared: 0, redundancyN: 1, mustCommit: true };
...
describe("crypto-on-noisy and unvoted-into-deterministic are fail-closed", () => {
  ...
  it("LLN-SUBSTRATE-001: crypto effect on a noisy lane is forbidden regardless of tolerance", () => {
    ...
    assert.equal(d.diagnostic.code, "LLN-SUBSTRATE-001");
    assert.equal(d.diagnostic.severity, "error");
  });
  it("LLN-SUBSTRATE-004: un-voted (N=1) noisy result into a deterministic sink is denied", () => {
    ...
    assert.equal(d.verdict, Verdict.DENY);
    assert.equal(d.diagnostic.code, "LLN-SUBSTRATE-004");
  });
