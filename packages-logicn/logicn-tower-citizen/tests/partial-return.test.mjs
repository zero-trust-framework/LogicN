...
// fold of actor caps; LLN-GOV-3VL-001 carried + sunk on every INDETERMINATE collapse; and the
...
test("maskByVerdict: ALLOW keeps (null); DENY and INDETERMINATE withhold", () => {
  ...
  assert.equal(i.diagnostic?.code, "LLN-GOV-3VL-001", "INDETERMINATE carries the audit diagnostic");
});
...
test("partialReturn: keep-the-rest — allowed fields pass through, others masked", () => {
  ...
  assert.equal(out.diagnostics.length, 1, "one LLN-GOV-3VL-001 (the INDETERMINATE field)");
  assert.equal(out.diagnostics[0].field, "note");
  assert.equal(out.diagnostics[0].diagnostic.code, "LLN-GOV-3VL-001");
});
...
test("partialReturn: onDiagnostic sink fires once per INDETERMINATE field", () => {
  ...
  assert.deepEqual(seen, [["b", "LLN-GOV-3VL-001"]], "only the INDETERMINATE collapse is audited (DENY is ordinary)");
});
