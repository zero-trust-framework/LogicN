// Runner tests for @logicn/test.
//
// Hermetic: every runner is exercised against a CRAFTED tmp workspace with FAKE
// targets (a fake run-all-tests.cjs, a fake logicn.mjs, fake node:test corpora),
...
const FAKE_CLI = [
  // A fake `logicn` CLI: exits non-zero iff any arg path mentions "bad".
  `const args = process.argv.slice(2);`,
...
function fullWorkspace() {
  const root = mkdtempSync(join(tmpdir(), "lln-test-full-"));
  after(() => { try { rmSync(root, { recursive: true, force: true }); } catch { /* best effort */ } });
  w(root, "logicn.workspace.json", JSON.stringify({ name: "fixture", packages: [] }));
  w(root, "scripts/run-all-tests.cjs", FAKE_UNIT);
  w(root, "logicn.mjs", FAKE_CLI);
  w(root, "tests/r6-corpus/r6-parity.test.mjs", PASSING_TEST);
  w(root, "packages-logicn/logicn-core-compiler/tests/fidelity-differential.test.mjs", PASSING_TEST);
  w(root, "packages-logicn/logicn-core-compiler/dist/index.js", "export {};\n");
  w(root, "examples/good.lln", "pure flow main() -> Int { return 0 }\n");
  w(root, "examples/bad.lln", "pure flow main() -> Int { return 0 }\n");
  return root;
}

/** A tmp workspace with ONLY the marker — every target is absent. */
function bareWorkspace() {
  const root = mkdtempSync(join(tmpdir(), "lln-test-bare-"));
  after(() => { try { rmSync(root, { recursive: true, force: true }); } catch { /* best effort */ } });
  w(root, "logicn.workspace.json", JSON.stringify({ name: "bare", packages: [] }));
  return root;
}
...
test("runE2e: passes when every example compiles clean", async () => {
  const root = fullWorkspace();
  const res = await runE2e({ rootDir: root, examples: ["examples/good.lln"] });
  assert.equal(res.ok, true);
...
test("runE2e: one failing example fails the whole check", async () => {
  ...
  const res = await runE2e({
    rootDir: root,
    examples: ["examples/good.lln", "examples/bad.lln"],
  });
  assert.equal(res.ok, false);
  assert.equal(res.exitCode, 1);
...
test("runE2e: a missing example file fails closed (not skipped)", async () => {
  const root = fullWorkspace();
  const res = await runE2e({ rootDir: root, examples: ["examples/nope.lln"] });
  assert.equal(res.ok, false);
});
...
test("runE2e: --build uses the build verb", async () => {
  const root = fullWorkspace();
  const res = await runE2e({ rootDir: root, examples: ["examples/good.lln"], build: true });
  assert.equal(res.ok, true);
...
test("runFidelity: fail-closed prerequisite when the compiler dist is not built", async () => {
  ...
  rmSync(join(root, "packages-logicn/logicn-core-compiler/dist"), { recursive: true, force: true });
  const res = await runFidelity({ rootDir: root });
  assert.equal(res.ok, false);
...
test("runAll: aggregates green children into a single pass", async () => {
  ...
  const res = await runAll({ rootDir: root, examples: ["examples/good.lln"] });
