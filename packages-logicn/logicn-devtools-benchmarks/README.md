...
Runtime comparison benchmarks for the LogicN language runtime. Three compute-heavy workloads run across six runtimes, measuring raw throughput and the overhead introduced by LogicN's governance and manifest verification layers.
...
| **LogicN (manifest)** | LogicN source compiled to an AST, effect-checked, governance-verified, then executed. The manifest layer adds static proof checks before execution begins. |
| **LogicN (governed)** | Same as manifest but the runtime enforces governance contracts continuously during execution. The strictest and most overhead-heavy mode. |
...
The final column — **Node/LogicN** — shows how many times faster Node.js is compared to the LogicN governed runtime. This ratio is the primary signal for LogicN runtime optimisation work.

Typical result shape on a mid-range desktop:

- Rust and C++ are 2x–5x faster than Node.js
- Node.js is 10x–40x faster than Python
- LogicN (manifest) is slower than Node.js due to parse + effect-check overhead per run
- LogicN (governed) is slower still due to continuous contract enforcement during execution
...
LogicN's governed runtime deliberately trades throughput for safety guarantees:

**Manifest mode** adds a static pre-flight phase before execution:
- Parse the `.lln` source and build an AST
...
This overhead is intentional. LogicN programs operating under governance get proofs that would otherwise require external audit tools.

The benchmark numbers make the cost of that guarantee visible: if LogicN governed runs at 1/20th of Node.js throughput, that gap is the price of continuous governance enforcement on this hardware.

---

## Phase 25: closing the gap with WASM

The current LogicN runtime is a tree-walking interpreter running inside Node.js. Phase 25 of the LogicN roadmap introduces a WASM compilation target:

1. The LogicN compiler emits a WebAssembly module from the typed, effect-annotated AST.
...
Expected outcome: the Node/LogicN ratio should fall from ~10x–20x toward ~2x–3x for compute-bound workloads. Governance overhead becomes a static compile-time cost rather than a per-operation interpreted cost.

A new runtime column — **LogicN (wasm)** — will be added to the benchmark suite when Phase 25 lands. The `results/latest.json` schema is forward-compatible: the runner skips any runtime whose binary or module is not present.
...
src/
  ...
  logicn-runner.mjs   LogicN interpreter bridge
results/
