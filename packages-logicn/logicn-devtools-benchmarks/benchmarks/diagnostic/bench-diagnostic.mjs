 ...
 *   - governance check pass rate (logicn check must exit 0)
 ...
 *   All tests use `logicn check` (governance static analysis path).
 ...
 *   governancePass        — logicn check exits 0 (boolean)
...
const BENCH_LLN = join(__dir, "benchmark.lln");
const BENCH_LLN_REL = "packages-logicn/logicn-devtools-benchmarks/benchmarks/diagnostic/benchmark.lln";

// Run governance check on the diagnostic benchmark file
function runGovernanceCheck() {
  const result = spawnSync(
    "node",
    [join(ROOT, "logicn.mjs"), "check", BENCH_LLN_REL],
