...
In OOP that is `Controller → Service.method() → util()`. In LogicN every layer is
...
LogicN governed flows are expensive to call (each call binds args into tagged
...
LogicN: 50,000 outer chains; runner normalises throughput to ops/second.
