 ...
 *   2. LogicN governed endpoint (route-dispatcher → governance → response)
 *
 * Each test:
...
// ── Benchmark: LogicN governed server ────────────────────────────────────────
async function benchmarkLogicNGoverned(requests = 200) {
  const latencies = [];

  // Start the LogicN server via the serve-logicn.mjs shim
  const shimPath = join(__dir, "serve-logicn.mjs");
  ...
  try {
    ...
    serverProc.kill();
    return { runtime: "logicn-governed", error: true, reason: String(e) };
  }
  ...
  return {
    runtime: "logicn-governed",
    server: "LogicN route-dispatcher → governance → audit → response",
    requests,
    errors,
    elapsedMs: Number(elapsedMs.toFixed(1)),
...
const skipLogicN = args.includes("--skip-logicn");

const nodeResult = await benchmarkNodeRaw(requests);
const logicnResult = skipLogicN ? { runtime: "logicn-governed", skipped: true }
                                : await benchmarkLogicNGoverned(Math.min(requests, 200));
