/**
 * LogicN Diagnostic Benchmark Runner
 *
...
// Track which lln files have been built in this session (avoid redundant rebuilds per benchmark)
const _builtFiles = new Set();

// Build a .lln file to register a fresh manifest before running
function buildFile(llnFile) {
  if (_builtFiles.has(llnFile)) return;
  spawnSync(
    "node",
    [join(ROOT, "logicn.mjs"), "build", llnFile],
    { cwd: ROOT, encoding: "utf-8", timeout: 30_000 }
  );
  _builtFiles.add(llnFile);
}

// Run governance check on a .lln file — measures static analysis speed
// Returns { success, flowCount, violations, diagnostics }
function runGovernanceCheck(llnFile) {
  const result = spawnSync(
    "node",
    [join(ROOT, "logicn.mjs"), "check", llnFile],
  ...
  const trapCount = (output.match(/LLN-INV|trap/g) ?? []).length;
  return {
...
// Note: logicn run --invoke is not used by the diagnostic runner in Stage A.
// All measurements use governance-check (logicn check) — the authoritative
...
//   All scenarios use `logicn check` (governance static analysis path).
...
//
//   Trap coverage is verified via static trap declarations in the .lln source,
...
async function runDiagnostic(benchId, llnFile, scenarios) {
  ...
  for (const [scenarioName, { flow, args, expectTrap, iterations = 100, altFile }] of Object.entries(scenarios)) {
    ...
    // Governance compliance: for all diagnostic benchmarks, the .lln file must pass
...
// Scenarios use gov-check mode: each measures `logicn check` latency for the benchmark file.
// trap-declarations-in-source confirms structural trap coverage (compile-time audit proxy).
const DIAGNOSTICS = [
  {
    id: "toxic-input",
    llnFile: "packages-logicn/logicn-devtools-benchmarks/benchmarks/toxic-input/benchmark.lln",
    scenarios: {
  ...
  },
  {
    ...
    llnFile: "packages-logicn/logicn-devtools-benchmarks/benchmarks/governance-violation/benchmark.lln",
    scenarios: {
  ...
  },
  {
    ...
    llnFile: "packages-logicn/logicn-devtools-benchmarks/benchmarks/resource-exhaustion/benchmark.lln",
    scenarios: {
  ...
  },
  {
    ...
    llnFile: "packages-logicn/logicn-devtools-benchmarks/benchmarks/logging-throughput/benchmark.lln",
    scenarios: {
...
async function main() {
  ...
  console.log("\n+==================================================================+");
  console.log("|  LogicN Diagnostic Benchmarks  *  Resilience + Audit Fidelity   |");
  console.log("+==================================================================+");
