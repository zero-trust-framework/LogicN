# Migration: extracting `@logicn/inference-bridge-contract`
...
Out of `logicn-tower-citizen` and into this neutral, **zero-dependency** package:
...
- `precision-strategy.ts` imports the three precision types from the contract and
  **re-exports** them, so `@logicn/tower-citizen` consumers are unaffected.
- `logicn-ext-bridge-cpp` now imports the **contract** for `InferenceBridge` /
...
into its own package (e.g. `@logicn/tpl-oracle`) so `ext-bridge-cpp` imports NO
