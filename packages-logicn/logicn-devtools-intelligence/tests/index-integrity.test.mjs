// index-integrity.test.mjs — LLN-INTEL-001 (poisoned-index) + LLN-INTEL-002 (unsandboxed write).
//
...
async function tmpWorkspace() {
  const dir = await mkdtemp(join(tmpdir(), "lln-intel-"));
  await writeFile(join(dir, "pay.lln"), SAMPLE, "utf8");
  return dir;
}
...
test("verifyIndexIntegrity: any field mutation breaks the tag; a missing tag fails closed", async () => {
  const base = { version: 1, builtAt: "2026-06-24T00:00:00Z", workspaceDir: "/w", flows: [{ flowName: "a", filePath: "/w/a.lln" }], fileHashes: { "/w/a.lln": "abc" }, skippedFiles: 0 };
  const sealed = { ...base, integrity: computeIndexIntegrity(base) };
  assert.equal(verifyIndexIntegrity(sealed), true);
  assert.equal(verifyIndexIntegrity({ ...sealed, flows: [{ flowName: "b", filePath: "/w/a.lln" }] }), false, "mutating a flow breaks the tag");
  assert.equal(verifyIndexIntegrity({ ...sealed, fileHashes: { "/w/a.lln": "DEAD" } }), false, "mutating a fileHash breaks the tag");
  assert.equal(verifyIndexIntegrity(base), false, "no integrity tag → fail-closed");
});

test("HMAC mode: with LOGICN_INDEX_HMAC_KEY the tag is unforgeable without the key", async () => {
  const base = { version: 1, builtAt: "t", workspaceDir: "/w", flows: [], fileHashes: {}, skippedFiles: 0 };
  const prev = process.env.LOGICN_INDEX_HMAC_KEY;
  try {
    process.env.LOGICN_INDEX_HMAC_KEY = "super-secret-key";
    ...
    delete process.env.LOGICN_INDEX_HMAC_KEY;
    ...
    process.env.LOGICN_INDEX_HMAC_KEY = "different-key";
    assert.equal(verifyIndexIntegrity({ ...base, integrity: keyed }), false, "a tag from a different key is rejected");
  } finally {
    if (prev === undefined) delete process.env.LOGICN_INDEX_HMAC_KEY; else process.env.LOGICN_INDEX_HMAC_KEY = prev;
  }
});

test("LLN-INTEL-002: an indexDir with a '..' path-traversal segment is refused", async () => {
  const dir = await tmpWorkspace();
  try {
