/**
 * @logicn/devtools-security — Integration Tests
 */
...
describe("SecurityAuditRunner: full pipeline", () => {
  ...
  it("catches SQL injection (LLN-TAINT-001)", async () => {
    const report = await runSecurityAudit([
  ...
  it("catches recursion in strict profile (LLN-PROFILE-001)", async () => {
    const report = await runSecurityAudit(
      "pure flow fib(n: Int) -> Int contract { effects {} } { if n <= 1 { return n } return fib(n-1) + fib(n-2) }",
      { profiles: ["strict"] }
    );
    assert.ok(report.findings.some(f => f.code === "LLN-PROFILE-001"));
    assert.ok(!report.passed);
  });

  it("catches dynamic regex in strict profile (LLN-PROFILE-005B)", async () => {
    const report = await runSecurityAudit(
      "pure flow f(p: String, s: String) -> Bool contract { effects {} } { return s.matchesPattern(p) }",
      { profiles: ["strict"] }
    );
    assert.ok(report.findings.some(f => f.code === "LLN-PROFILE-005B"));
  });

  it("schemaVersion is lln.security-audit.v1", async () => {
