// =============================================================================
// @logicn/devtools-provenance — Report Renderer
//
 ...
 * @param fileCount     - Total number of .lln files scanned
 */
export function renderTextReport(graph: ProvenanceGraph, fileCount: number): string {
  ...
  if (flowGroups.size === 0) {
    lines.push("  (no flows found — check that the directory contains .lln files)");
    lines.push("");
  }
...
export function renderJsonReport(graph: ProvenanceGraph, fileCount: number): string {
  const output = {
    schemaVersion: "lln.provenance.v1",
    fileCount,
    ...graph,
  };
