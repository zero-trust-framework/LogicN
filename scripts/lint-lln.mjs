#!/usr/bin/env node
// lint-lln.mjs — `.lln` source-quality gate (owner rules, 2026-06-23).
//
...
//   1. FLOW COMMENTS        — every flow has a human `//` comment directly above it (the `//lln:`
//                             generated provenance lines do NOT count).            → LLN-LINT-COMMENT
...
//                             omitted (let the runtime infer them):                → LLN-LINT-CONTRACT
//                             secrets · economics · epilogue · cyber_physical_hardening · liability
//                                                                                  → LLN-LINT-INTENT / LLN-LINT-AUTO-SETTING
...
//   - default file `governance/lln-lint-allow.json` (auto-loaded if present),
//   - `--whitelist <file>` (merged on top),
//   - inline `//lln-allow: CODE[,CODE]` on the offending line OR the line directly above a flow.
//
// Flags:  --json   --soft (always exit 0)   --summary   --whitelist <f>   --no-default-whitelist
//         [paths…]  (default scan roots: packages-logicn, examples, tests)
...
const SCAN_ROOTS = paths.length > 0 ? paths : ["packages-logicn", "examples", "tests"];
...
// throughout LogicN comments and are NOT slop.
...
function loadWhitelist() {
  ...
  const sources = [];
  if (!noDefaultWl) sources.push(join(ROOT, "governance", "lln-lint-allow.json"));
  ...
  for (const src of sources) {
    ...
    try {
      ...
      console.error(`lint-lln: could not parse whitelist ${src}: ${e.message}`);
    }
  }
  return wl;
}
...
function* walk(dir) {
  ...
  for (const e of entries) {
    ...
    else if (e.name.endsWith(".lln")) yield p;
  }
}
function discover() {
  const out = [];
  for (const r of SCAN_ROOTS) {
    ...
    if (statSync(abs).isFile()) { if (abs.endsWith(".lln")) out.push(abs); }
...
function lintFile(abs, wl, findings) {
  ...
  const inlineAllow = (lineNo) => {
    const m = (lines[lineNo - 1] ?? "").match(/\/\/lln-allow:\s*([A-Z0-9,\- ]+)/);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/\t/.test(ln.match(/^\s*/)[0])) add("LLN-SYNTAX-TAB", no, "tab in indentation — use spaces");
    if (/[ \t]+$/.test(ln)) add("LLN-SYNTAX-TRAILING-WS", no, "trailing whitespace");
    if (EMOJI_RE.test(ln)) add("LLN-SLOP-EMOJI", no, "emoji in source");
    const cm = ln.match(/\/\/\s?(.*)$/);
    if (cm && !ln.includes("//lln:") && !ln.includes("//lln-allow:")) {
      const body = cm[1].trim();
      const low = body.toLowerCase();
      if (body && FILLER_COMMENT.some((re) => re.test(low))) add("LLN-SLOP-FILLER-COMMENT", no, `filler/hedging comment: "${body.slice(0, 50)}"`);
      if (AI_ARTIFACT.some((re) => re.test(body))) add("LLN-SLOP-AI-ARTIFACT", no, `AI-assistant artifact in source: "${body.slice(0, 50)}"`);
    }
  }
  ...
  if (bad || depth !== 0) add("LLN-SYNTAX-BRACE", 1, `unbalanced braces (net ${depth}) — likely a syntax error`);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // placeholder name
    if (PLACEHOLDER_NAME.has(name.toLowerCase())) add("LLN-SLOP-PLACEHOLDER-NAME", flowLine, `placeholder flow name "${name}"`);

    // rule 1 — human comment directly above (skip //lln: provenance; a blank gap breaks association)
    let hasComment = false;
    for (let j = i - 1; j >= 0; j--) {
      const t = lines[j].trim();
      if (t === "") break;                       // blank gap → not attached
      if (t.startsWith("//lln:")) continue;      // generated provenance — not a human comment
      if (t.startsWith("//")) { const c = t.slice(2).trim(); if (c !== "") { hasComment = true; } continue; }
      break;                                     // hit code → stop
    }
    if (!hasComment) add("LLN-LINT-COMMENT", flowLine, `flow "${name}" has no human comment (rule 1)`);
    ...
    if (!hasContract) {
      add("LLN-LINT-CONTRACT", flowLine, `flow "${name}" declares no contract {} (rule 2)`);
    } else {
      ...
      if (braceOpen >= 0 && braceClose > braceOpen) {
        ...
        if (!clauses.has("intent")) add("LLN-LINT-INTENT", flowLine, `flow "${name}" contract has no intent {} (rule 2)`);
        for (const c of clauses) if (AUTO_SETTINGS.has(c)) add("LLN-LINT-AUTO-SETTING", lineOf(braceOpen + 1 + inner.indexOf(c)), `contract declares the AUTO-by-default setting "${c}" — omit it (let the runtime infer; rule 2 exception)`);
        // vacuous intent
        const im = inner.match(/\bintent\s*\{\s*"([^"]*)"/);
        if (im) { const s = im[1].trim(); if (s.length < 12 || /^todo$/i.test(s) || s.toLowerCase() === name.toLowerCase()) add("LLN-SLOP-VACUOUS-INTENT", flowLine, `intent string is vacuous/too short: "${s}"`); }
        ...
        if (bodyBeforeContract >= 0) add("LLN-SYNTAX-CONTRACT-IN-BODY", flowLine, `flow "${name}" puts contract {} INSIDE the body — it must sit between signature and body`);
      }
    }

    // NOTE: match exhaustiveness (the mandatory `_ =>` / `else =>` catch-all, LLN-TYPE-023) is
...
const findings = [];
for (const f of files) { try { lintFile(f, wl, findings); } catch (e) { findings.push({ file: relative(ROOT, f), line: 1, code: "LLN-LINT-ERROR", msg: `lint crashed: ${e.message}` }); } }
...
if (asJson) {
  console.log(JSON.stringify({ tool: "lint-lln", scanned: files.length, skippedFixtures: skippedCount, violations: findings.length, byCode, findings }, null, 2));
} else if (summary) {
  const codes = Object.entries(byCode).sort((a, b) => b[1] - a[1]).map(([c, n]) => `${c}:${n}`).join(" · ");
  console.log(`lint-lln: ${files.length} .lln scanned (${skippedCount} fixtures skipped) · ${findings.length} finding(s)${findings.length ? " — " + codes : ""}`);
  console.log(`VIOLATIONS: ${findings.length}`);
} else {
  const out = ["# lint-lln — .lln source-quality gate (rules: flow comments · contracts · no slop)\n"];
  for (const f of findings) out.push(`${f.file}:${f.line}  ${f.code}  ${f.msg}`);
  out.push(`\nScanned ${files.length} .lln file(s) (${skippedCount} fixtures skipped via whitelist) · ${findings.length} finding(s)`);
  if (Object.keys(byCode).length) out.push(Object.entries(byCode).sort((a, b) => b[1] - a[1]).map(([c, n]) => `  ${c}: ${n}`).join("\n"));
  out.push(findings.length === 0 ? "CLEAN ✓ — every flow is commented + contracted and no slop detected." : "Whitelist false positives in governance/lln-lint-allow.json or with //lln-allow: <CODE>.");
