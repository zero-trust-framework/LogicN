...
| Typed content blocks: html / dom / script / css heredoc syntax | ✅ | `logicn-core-syntax-typed-content-blocks.md` — `ContentBlockType`, `TypedContentBlockExpression` in `@logicn/core`; `LLN-BLOCK-001..004` constants, `LLN_BLOCK_DIAGNOSTICS`; content block state tracking (brace depth suspended inside blocks), `LLN-BLOCK-001` unknown type, `LLN-BLOCK-002` unclosed block detection live in `validateCoreSyntaxSafety()`; `validateTypedContentBlock()` stub; `typedContentBlockExpr` added to `AstNodeKind`; 17/17 tests passing (2026-05-26) |
...
| Runtime audit log format | ✅ | `runtime-audit-log-format.md` — schemaVersion resolved: `"lln.runtime.audit.v1"` canonical; `ExecutionProofHashes` field names confirmed (`manifestSha256`, `auditSha256`, `evidenceSha256`, `denialSha256`, `artefactSha256`); `JsonlAuditWriter` interface + 7-rule contract added; `ExecutionProofV1`/`V2`, `ExecutionProofSection`, `upgradeExecutionProofV1ToV2()` added; `RuntimeAuditEvent` extended with `category` and `message` fields; `DenialReport.schemaVersion` → `"lln.denial.report.v1"` (2026-05-26) |
| Effect checker and boundary checker | ✅ | `effect-checker-and-boundary-checker.md` |
| Formal type system specification — all built-in types, generic arity, null policy, 22 LLN-TYPE-* diagnostic codes, 14-step checker execution order | ✅ | `formal-type-system-spec.md` — Phase 5 prerequisite; nominal typing, value-state interaction, SecureString restrictions, pipeline order (symbol resolution → type checker → value-state checker → effect checker → governance verifier) (2026-05-28) |
| Value-state annotation grammar and checker — unsafe/safe/validated/tainted/secret/protected; gate functions; LLN-VALUESTATE-001..005; LLN-SECRET-001..003 | ✅ | `value-state-annotations.md` + `value-state-checker.md` — Phase 5 prerequisite; EBNF grammar, state machine, taint propagation, audit proof format (2026-05-28) |
| Operator precedence and Pratt parser — binding power table, INFIX/PREFIX tables, governance implications, LLN-EXPR-001..007 | ✅ | `operator-precedence.md` — Phase 5 prerequisite; table-driven Pratt decision, precedence examples, no ++ / --, no operator overloading, pipeline operator reserved (2026-05-28) |
...
| IHSA (Infrequent Hard Storage Access) storage policy — `storage.read.infrequent` effect, streaming-first, memory pressure policy, LLN-STORAGE-001 | ✅ | `ihsa-storage-policy.md` — post-v1 runtime enforcement; `spill.enabled false` default; governance policy block syntax (2026-05-28) |
| @state() lifecycle system — Layer C runtime orchestration, session/gpu/context/distributed categories, lifecycle phases, LLN-STATE-001..006 | ✅ | `lifecycle-state-system.md` — **Phase 6+ / Layer C only — do not implement in v1**; design intent preserved; reactivity, state dependency graph, mutation policies (2026-05-28) |
...
| CLI build / verify / deploy / explain / plan (governance) | ✅ | `logicn-core-cli-deploy-explain-plan.md` |
...
| Phase 16-17 — NodeFlags and ValueStateFlags | ✅ | `logicn-roadmap.md` — NodeFlags bitmask for AST node properties; ValueStateFlags bitmask for value state tracking (unsafe/safe/validated/tainted/secret/protected); integrated into compiler pipeline (2026-05-31) |
...
| Phase 22 — Register VM types | ✅ | `runtime-interpreter-roadmap.md` — Register-based VM type definitions (`register-vm.ts`); instruction set planning for LogicN bytecode execution (2026-05-31) |
...
| Environment config and secret reference model | ✅ | `logicn-core-config-environment-secrets.md` |
...
| CLI commands — v0.2 formal spec | ✅ | `logicn-core-cli-v02.md` |
| Reports/audit — v0.2 formal spec | ✅ | `logicn-core-reports-v02.md` |
| Compute backends — v0.2 formal spec | ✅ | `logicn-core-compute-v02.md` |
| Config — v0.2 formal spec | ✅ | `logicn-core-config-v02.md` |
| Network governance — v0.2 formal spec | ⚠️ | `logicn-core-network-v02.md` ⚠️ **Update status: superseded** — uses `sharedSecret` field, `ws/wss` protocols, `ReplayStore {has/store}` without `IdempotencyRecord`; superseded by `logicn-core-network-webhook.md` canonical names |
| API server — v0.2 formal spec | ✅ | `logicn-framework-api-server-v02.md` |
| Compiler manifest generation — pass 14 | ✅ | `logicn-core-compiler-manifest-generation-pass-14.md` |
| Online Safety Act age assurance policy | ✅ | `logicn-core-policy-online-safety-act.md` |
| Security secret reference model — architecture spec | ✅ | `logicn-core-security-secret-reference-model.md` |
| Photonic governance architecture | ✅ | `logicn-core-photonic-governance-architecture.md` |
| Photonic backend architecture | ⚠️ | `logicn-core-photonic-backend-architecture.md` ⚠️ **Update status: legacy/historical** — contains prior 3-value OpticalTransportMode string union and original LLN-PHOTONIC-001–006 meanings; use as historical context only until logicn-core-photonic reconciles the canonical enum and diagnostic table across all specs |
| Logic types — v0.2 formal spec | ⚠️ | `logicn-core-logic-v02.md` ⚠️ **Update status: formal spec (type: discriminant, 3-state Decision)** — superseded for runtime-facing docs by package README and `logicn-core-logic-tri-decision-bool.md`; current canonical uses kind: discriminant and 4-state Decision (allow\|deny\|review\|unknown) |
| Security — v0.2 formal spec | ✅ | `logicn-core-security-v02.md` **Update status: conflict resolved (2026-05-26)** — canonical public API is `unwrapForApprovedSink(sink)`; `revealUnsafeForRuntimeOnly()` is private internal only; KB updated to match |
| Signed Attestation — Ed25519 artifact signing | ✅ | `logicn-signed-attestation.md` — attestation pipeline, YAML format, key_id rotation, Stage 2 post-quantum ML-DSA/SLH-DSA plan (2026-05-30) |
| Governed Memory Blocks — runtime integrity | ✅ | `logicn-governed-memory-blocks.md` — GMB structure, three-layer enforcement, ARM MTE/CHERI future hardware, contract rules syntax (2026-05-30) |
| Flow Contract Full Model — canonical section reference | ✅ | `logicn-contract-full-model.md` — all 16 sections incl. errors/timeouts/retries/limits/privacy/observability, named result types, canonical order (2026-05-30) |
| Contract errors {} section | ✅ | `logicn-contract-errors.md` — error mapping, expose/redact/audit, connection to Result<T,E> (2026-05-30) |
| Contract operational constraints — timeouts/retries/limits | ✅ | `logicn-contract-operational-constraints.md` — deadline, retry policy, resource limits, unlimited prohibition (2026-05-30) |
| Contract privacy and observability sections | ✅ | `logicn-contract-privacy-observability.md` — PII rules, retention, deny patterns, observability sees execution not data (2026-05-30) |
| Naming conventions — request not req, rawX for unsafe, FlowNameResult | ✅ | `logicn-naming-conventions.md` — accepted style direction, full param names, local binding rules (2026-05-30) |
| Phase 11 design decisions — loops, mut, CEC CI, graph module, result-of, contract runtime | ✅ | `logicn-phase-11-decisions.md` — 6 recorded decisions with rationale (2026-05-30) |
| Phase 10 Roadmap — 10A/10B/10C plan | ✅ | `logicn-phase-10-roadmap.md` — attestation module, governance enforcement, governed memory blocks (2026-05-30) |
| Photonic — v0.2 formal spec | ⚠️ | `logicn-core-photonic-v02.md` ⚠️ **Update status: not implementation-ready** — OpticalTransportMode enum (Waveguide\|Coherent\|Mesh\|FreeSpace\|Hybrid\|Experimental) conflicts with governance spec (DIRECT\|WAVELENGTH\|PACKETIZED\|HYBRID\|EMULATED\|SIMULATED) and vector spec (electrical\|hybrid\|photonic\|waveguide\|plasmonic\|coherent); reconciliation required before any implementation |
| Tri Logic developer guide — v0.2 | ⚠️ | `logicn-core-logic-tristate-developer-guide.md` ⚠️ **Update status: predates current v0.2 shape** — Decision shown as 3-state (allow/deny/unknown), BoolBoundaryResult lacks diagnostics[]; use package README and `logicn-core-logic-tri-decision-bool.md` as canonical; kind: discriminant is standard |
| API server full implementation spec | ⚠️ | `logicn-framework-api-server-implementation.md` ⚠️ **Update status: adapter-level names** — `ReplayStore.exists()/save()` are adapter-level; canonical network contract is `has()/put(ttlSeconds)` from `logicn-core-network-webhook.md`; mapping or name adoption required before implementation |
| Vector photonic governance (proposed) | ⚠️ | `logicn-core-vector-photonic-governance.md` ⚠️ **Update status: proposal/reference note only** — logicn-core-photonic owns final definitions after reconciliation |
...
| docs/tri-logic.md | ✅ | Tri / Bool / Decision logic in depth — ownership note: older examples are language-explanatory only; `logicn-core-logic` README and `logicn-core-logic-tri-decision-bool.md` are canonical |
| docs/omni-logic.md | ✅ | Ownership note: semantics owned by `logicn-core-logic`; uncertain Omni states must map to review() |
...
| docs/README.md | ✅ | Coverage Update Status section added — canonical owners: `logicn-core-logic` (v0.2 TriState/Decision/BoolBoundaryResult/Omni); `logicn-core-network` (webhook HMAC/replay/idempotency); `logicn-core-security` (protected secret); `logicn-core-photonic` (photonic runtime target — ownership/diagnostic-code conflicts unresolved) |
| docs/backend-compute-support-targets.md | ✅ | Ownership note: compute planning in `logicn-core-compute`, target selection in `logicn-target-cpu/native/gpu/photonic` |
...
| Secret reference model | ⚠️ | Fully specified in README and `logicn-core-security-secret-reference-model.md`; not yet implemented |
...
| Effect checker | ⚠️ | Fully specified (Effect interface, EffectCategory, CheckedFunction, EffectGraphNode, EffectGraph, inferExpressionEffects, propagateEffects, LLN-EFFECT-001–004); not yet implemented |
| Boundary checker | ⚠️ | Fully specified (Boundary, BoundaryRequirement, BoundaryEdge, BoundaryGraph, CheckedCallExpression, LLN-BOUNDARY-001–004); not yet implemented |
...
| tests/ | ✅ | 21 tests passing (2026-05-26) — syntax safety suite (17 original) + `validateIntentEffects` stub shape (returns correct flowName/safetyLevel/intent/effects/mismatches/diagnostics fields; omits `intent` key when `undefined`); `String/Char/Byte diagnostic constants` (LLN-STRING-001–004, LLN-CHAR-001–004, LLN-BYTE-001–005: codes, names, severities, array lengths/prefixes); `Memory diagnostic constants` (LLN-MEMORY-001–008: codes, names, all errors, array length 8, prefix check) |
...
| logicn build | ⚠️ | Partial — artefact generation not complete; fully specified (BuildArtefact, BuildResult, buildWorkspace, 14-pass pipeline, LLN-BUILD-001–005) |
| logicn fmt | ✅ | Prototype implemented |
| logicn verify | ⚠️ | Partial — hash checks only; fully specified (VerifiedArtefact, VerificationResult, verifyHash, LLN-VERIFY-001–005) |
| logicn deploy | ⚠️ | Not yet implemented — fully specified (DeploymentTarget 7 values, DeploymentResult, ValidateEffectsInput, validateEffects, exit codes 0–7, LLN-DEPLOY-001–005) |
| logicn explain | ⚠️ | Not yet implemented — fully specified (ExplainTrace, ExplainResult, buildTrace, LLN-EXPLAIN-001–004) |
| logicn plan | ⚠️ | Not yet implemented — fully specified (ComputePlan with GpuPlan/OpticalPlan/CompatibilityReport, estimateTarget, LLN-PLAN-001–004) |
...
| Decision logic | ✅ | v0.2 implemented in `src/decision/` — 4-state `Decision`, `allow/deny/review/unknownDecision` constructors, `decisionToRuntimeBool`, `combineDecisions` (deny>review>unknown>allow), `evaluateCapability`, `LLN-DECISION-001–005` diagnostics; 2286 tests passing (2026-05-31, total workspace) |
...
| WASM target | ⚠️ | Fully specified (WasmTarget extended with runtime type + forbiddenEffects[], DEFAULT_WASM_FORBIDDEN_EFFECTS, BROWSER_WASM_FORBIDDEN_EFFECTS, validateWasmEffect, validateWasmTarget, wasm/ dir, LLN-WASM-001–004); not yet implemented |
| Target compatibility report | ⚠️ | Fully specified (CompatibilityLevel, CompatibilityResult extended, TargetProfile, CompatibilityReport, buildCompatibilityReport, compatibility/ dir, LLN-COMPAT-001–004); not yet implemented |
...
| Governance model | ⚠️ | Fully specified v0.2 (NetworkProtocol 7 values incl. tcp/udp, NetworkPolicy with default/allowDestinations/denyDestinations, productionNetworkPolicy SSRF deny list, GovernedNetworkRuntime, safeHttpRequest, AiProviderNetworkPolicy, LLN-NETWORK-001–008); not yet implemented |
| Webhook HMAC / replay / idempotency | ✅ | Fully specified v0.2 in `logicn-core-network-webhook.md` — canonical names: `secret: string\|Uint8Array`, `ReplayStore.has/put`, `IdempotencyStore.get/put(IdempotencyRecord)`; not yet implemented |
...
| Runtime audit log format (JSONL) | ⚠️ | **Conflict resolved (2026-05-26)** — `runtime-audit-log-format.md` updated: schemaVersion canonical `"lln.runtime.audit.v1"`, `ExecutionProofHashes` field names confirmed, `JsonlAuditWriter` + 7-rule contract added, `category`/`message` fields added to `RuntimeAuditEvent`, `DenialReport.schemaVersion` → `"lln.denial.report.v1"`. Specified but not yet implemented — `JsonlAuditWriter.append`, `serializeAuditEvent`, `validateAuditSafety` remain stubs |
| Execution proof format | ⚠️ | **Updated (2026-05-26)** — `ExecutionProofV1` (schemaVersion `"lln.execution.proof.v1"`, fixed 5 hashes), `ExecutionProofV2` (extensible sections), `ExecutionProofSection`, `upgradeExecutionProofV1ToV2()` all added to `runtime-audit-log-format.md`. Specified but not yet implemented |
| Denial report | ⚠️ | Fully specified v0.2 (DenialReport schemaVersion "logicn.denial.v1", 6-category union, denials/ dir, LLN-DENIAL-001–004); not yet implemented |
| Capability / effect / runtime evidence | ⚠️ | Fully specified v0.2 (CapabilityEvidence, EffectEvidence with declared/inferred/transitive fields, RuntimeEvidence with arrays, buildRuntimeEvidence async, evidence/ dir, LLN-EVIDENCE-001–004); not yet implemented |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope documented — Boundary conflict note and **Coverage Reconciliation Status** added: vector does not own photonic runtime target semantics; those belong in logicn-core-photonic once ownership reconciled |
| TODO.md | ✅ | Work tracking; **first item**: "Treat vector photonic governance notes as proposal-only until logicn-core-photonic reconciles ownership" |
| src/ | ⚠️ | Implementation stubs |
...
| Photonic governance (proposed) | ⚠️ | Specified in `logicn-core-vector-photonic-governance.md` — BOUNDARY CONFLICT: existing README says photonic belongs in logicn-core-photonic; must resolve before implementation |

### logicn-core-photonic

| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Governance-First Architecture section added; v0.2 Architecture Depth section added; **Coverage Reconciliation Status**: unresolved conflicts — OpticalTransportMode enum (3 incompatible 6-value variants), LLN-PHOTONIC-001–006 diagnostic table (prior vs v0.2 meanings differ), boundary overlap with logicn-core-vector/logicn-core-compute/logicn-target-photonic; do not implement until all three conflicts resolved |
| TODO.md | ✅ | All governance types, functions, diagnostic codes, planned sub-packages added; **first three items**: "Reconcile canonical OpticalTransportMode enum", "Reconcile canonical LLN-PHOTONIC-001–006 diagnostic table", "Confirm photonic ownership before implementation" |
...
| Governance architecture (prior KB) | ⚠️ | Specified in `logicn-core-photonic-backend-architecture.md` (OpticalTransportMode prior meanings, LLN-PHOTONIC-001–006 prior meanings); not yet implemented |
| Governance architecture (v0.2 formal spec) | ⚠️ | Specified in `logicn-core-photonic-v02.md` (OpticalTransportMode 6-value enum, all validation functions, PhotonicCapability, topologies, LLN-PHOTONIC-001–006 v0.2 meanings); not yet implemented |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Updated with v0.2 implementation spec (section 30) — all types, 10-step pipeline, security rules, HTTP status contract, may/must-not rules; **Coverage Reconciliation Status**: webhook HMAC, replay, and idempotency semantics must map to `logicn-core-network-webhook.md` v0.2 canonical; `ReplayStore.exists()/save()` are adapter-level names — canonical is `has()/put(ttlSeconds)` |
| TODO.md | ✅ | Architecture Depth items marked complete; full implementation items added for all 13 src files; **first two items**: "Map API-server ReplayStore.exists/save adapter names to logicn-core-network has/put", "Align webhook/idempotency implementation docs with logicn-core-network-webhook.md" |
| src/ | ❌ | Not yet implemented |
| API manifest schema | ✅ | v0.2: `LogicnApiManifest {schemaVersion:"logicn.api.manifest.v2", api, version, generatedAt, routes[]}` |
...
| Implementation spec | ✅ | Fully specified in `logicn-framework-api-server-implementation.md` (all types, all src files, 10-step pipeline, security rules) |
...
| Position | ✅ | Documented: sits between LogicN Core and full frameworks; enforces language-described safety boundaries at runtime |
...
| src/ | ⚠️ | Boundary contracts; implementation not started |
| Dependencies | ✅ | Consumes vector/matrix/tensor from `logicn-core-vector`, compute planning from `logicn-core-compute`, AI safety from `logicn-ai`, low-bit references from `logicn-ai-lowbit` |
...
| src/ | ⚠️ | Boundary contracts; implementation not started |
| Distinction | ✅ | Separate from neural networks: `logicn-ai-neural` = tensors/weights/layers/inference/training; `logicn-ai-neuromorphic` = spikes/events/event-driven spiking models |

### logicn-data

| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Umbrella package — coordinates logicn-data-html, logicn-data-search, logicn-data-archive, logicn-data-json, logicn-data-database, logicn-data-pipeline, logicn-data-reports; shared data-processing vocabulary, pipeline policy, security boundaries, memory limits, archive integrity references, report index contracts |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope: typed query contracts consumed by database adapters (`logicn-db-*`) |
| src/ | ⚠️ | Boundary contracts; implementation not started |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope: `SafeHtml` contracts, HTML sanitization, unsafe HTML reports; consumed by `logicn-web` packages |
| src/ | ⚠️ | Boundary contracts; implementation not started |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope: typed search contracts; consumed by `logicn-db-opensearch` and similar |
| src/ | ⚠️ | Boundary contracts; implementation not started |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope: JSON validation and decoding contracts; owns typed JSON access and decode policy; consumed by `logicn-web` packages |
| src/ | ⚠️ | Boundary contracts; implementation not started |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope: database orchestration contracts that compose logicn-data-db, logicn-data-model, logicn-data-query and logicn-data-response |
| src/ | ⚠️ | Boundary contracts; implementation not started |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Umbrella — browser-safe LogicN web contracts: browser runtime profile boundaries, typed rendering pipeline ownership, shared web package policy, browser-safe imports and effects; coordinates logicn-web-render, -state, -components, -router, -events |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope: future PostgreSQL adapter contract — must consume logicn-data-db, logicn-data-model, logicn-data-query and logicn-data-response; must not bypass typed models, validation, parameterisation, permissions, safe response mapping or reports |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope: future MySQL adapter contract; same constraints as logicn-db-postgres |
| src/ | ❌ | Not yet implemented — future adapter |
...
| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope: future OpenSearch adapter contract; consumes logicn-data-search contracts |
| src/ | ❌ | Not yet implemented — future adapter |
...
| src/ | ⚠️ | Boundary contracts; implementation not started |
| Boundary | ✅ | Does not implement kernels directly; optimized CPU kernel descriptions belong in `logicn-cpu-kernels`; AI model adapters belong in `logicn-ai` and `logicn-ai-lowbit` |
...
| src/ | ⚠️ | Boundary contracts; implementation not started |
| Boundary | ✅ | Describes CPU kernels and constraints; does not own AI model metadata, target selection policy or LogicN language syntax |
...
| src/ | ❌ | Not a v1 milestone; planning only — v1 milestone is the secure web runtime and `logicn serve` |
...
| src/ | ⚠️ | Boundary contracts; WasmTarget extended contract specified in `logicn-core-compute` KB; implementation not started |
...
| src/ | ⚠️ | Boundary contracts; GpuPlan contract specified in `logicn-core-compute` KB; implementation not started |
| Boundary | ✅ | Consumes compute plans from `logicn-core-compute`; does not own vector semantics, compute target selection or application runtime policy |
...
| Status | ⚠️ | Boundary conflict with `logicn-core-photonic` (owns governance) and `logicn-core-vector` (proposed photonic contracts); see photonic ownership conflict in Version Conflict Notes |

### logicn-tools-benchmark

| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope documented — development and diagnostics package for testing LogicN logic, compute targets and runtime behaviour across machine profiles: Bool/Tri/Decision logic, Result/Option handling, CPU scalar/SIMD execution, GPU, low-bit AI, future accelerator targets, optical I/O interconnect targets |
| src/ | ⚠️ | Boundary contracts; implementation not started |
| Goal | ✅ | Not to find the fastest computer — to understand how LogicN behaves correctly and predictably across execution targets |

### logicn-devtools-project-graph

| Area | Status | Notes |
| --- | --- | --- |
| README.md | ✅ | Scope documented — LogicN project knowledge graph contracts: project graph nodes, relationships, package ownership maps, document and decision links, policy and unsafe feature classification, report output manifests, AI assistant map files, graph query/path/explain request contracts, backend selection policy, workspace package/doc scanner, Markdown report and AI map rendering |
| dist/ | ✅ | CLI compiled and functional — generates `build/graph/LogicN_GRAPH_REPORT.md` |
| Graph CLI | ✅ | `logicn graph --out build/graph` generates graph report; stable LogicN surface: `LogicN graph`, project graph nodes and edges |
...
| Boundary | ✅ | Must not contain LogicN language documentation, package design notes or reusable framework features |
...
| --- | --- | --- |
| `logicn-core-compiler` effect/boundary | Reconcile effect/boundary model and diagnostic-code meanings before implementation | README/TODO and KB specs use different effect/boundary shapes and code ranges |
| `logicn-core-compiler` manifest pass 14 | Bring package README/TODO up to pass-14 integrity/stable-serialization contract, then implement | `ManifestIntegrity`, `serializeManifestStable()`, and `buildManifestIntegrity()` are in pass-14 KB, not package README/TODO |
| `logicn-core-cli` build/verify/deploy/explain/plan | Finish build/verify and implement deploy/explain/plan command paths | None documented |
| `logicn-core-reports` audit/proof/denial/evidence/trace | Finalise package contracts and implementation for report schemas; add FlowTraceEvent contract and JSONL writer (LLN-TRACE-001–005) | None documented |
| `logicn-core-compute` GPU/photonic/WASM/compat | Keep as planning until backend and target compatibility implementation starts | Photonic ownership and enum conflicts affect photonic plan output |
| `logicn-core-logic` Tri/Decision/Bool/Omni | Implement v0.2 canonical runtime contracts — TODO items corrected to canonical README shapes (2026-05-26) | KB alignment complete; v0.2 TODO items now use `kind:` discriminant, `value` fields, `UnknownReason`, 4-state Decision + evidence, correct OmniState snake_case literals |
| `logicn-core-config` environment/secrets/vault | Implement EnvironmentConfig v0.2, SecretEnvironmentReference loading/reporting, ConfigVault (LLN-VAULT-001–005) — SecretConfigSource naming resolved (2026-05-26) | Canonical `SecretConfigSource`: `env\|vault\|kms\|runtime`; Config Vault and .env trust model KBs added |
| `logicn-core-security` secret/taint | Implement v0.2 secret reference model — ProtectedSecret unwrap API resolved (2026-05-26) | Canonical: `unwrapForApprovedSink(sink)`; `private revealUnsafeForRuntimeOnly()` internal only |
| `logicn-core-network` governance/webhook | Implement v0.2 contracts — legacy name audit complete (2026-05-26) | TODO corrected: `ReplayStore.insertOnce()` → `has/put(key,ttlSeconds)`, `IdempotencyStore.getOrSet()` → `get(key)/put(record,ttlSeconds?)` |
| `logicn-core-photonic` governance | Reconcile enum, diagnostic table and ownership before implementation | Three incompatible `OpticalTransportMode` forms and diagnostic tables |
| `logicn-core-vector` photonic proposal | Keep proposal-only until photonic ownership is resolved | Boundary conflict with `logicn-core-photonic` |
| `logicn-framework-api-server` implementation | Map adapter replay/idempotency names to network canonical contracts | `exists()/save()` differs from network `has()/put()` |

---

**logicn-core-compiler: effect checker and boundary checker**

- KB: `effect-checker-and-boundary-checker.md`, `logicn-core-effect-checker-v02.md`
...
- Manual check: package README/TODO use the newer `EffectCategory` model
  ...
  an expanded boundary model; `logicn-core-effect-checker-v02.md` still uses the
...
- KB: `logicn-core-manifest-generation-v02.md`, `logicn-core-compiler-manifest-generation-pass-14.md`
...
- Manual check: package README/TODO include `RuntimeManifest`, route/function/
  ...
  `logicn-core-compiler-manifest-generation-pass-14.md`, not in the package
  README/TODO yet.
...
**logicn-core-cli: logicn build, verify, deploy, explain, plan**

- KB: `logicn-core-cli-deploy-explain-plan.md`, `logicn-core-cli-v02.md`
...
**logicn-core-reports: runtime audit log / execution proof / denials / evidence**

- KB: `runtime-audit-log-format.md`, `logicn-core-reports-v02.md`
...
**logicn-core-compute: GPU / photonic backends; WASM target; target compatibility**

- KB: `logicn-core-compute-gpu-and-photonic-backends.md`, `logicn-core-compute-v02.md`
...
**logicn-core-logic: Tri logic, Decision logic, Bool boundary rules, Omni logic**

- KB (canonical): `logicn-core-logic-tri-decision-bool.md`, `logicn-core-logic-omni-logic.md`
- KB (superseded): `logicn-core-logic-v02.md` — ⚠️ Update status: type: discriminant, 3-state Decision; use package README as canonical
- KB (superseded): `logicn-core-logic-tristate-developer-guide.md` — ⚠️ Update status: predates v0.2; Decision shown as 3-state; use package README as canonical
...
**logicn-core-config: environment config model; secret reference model**

- KB: `logicn-core-config-environment-secrets.md`, `logicn-core-config-v02.md`
...
- Manual check (2026-05-26): SecretConfigSource naming conflict resolved — canonical `env|vault|kms|runtime`; `logicn-core-config-environment-secrets.md` updated; TODO updated. ConfigDiagnostic `name` field added to KB and implementation. Diagnostic codes renamed from `LogicN_CONFIG_*` to `LLN-CONFIG-001…027` with `{code, name, message}` metadata. Config keeps `ProtectedSecret` as a simple interface; `logicn-core-security` owns the full class.
...
- KB (canonical): `logicn-core-security-secret-reference-model.md`
- KB (updated): `logicn-core-security-v02.md` — ✅ **conflict resolved (2026-05-26)** — `unwrapForApprovedSink(sink)` canonical; `private revealUnsafeForRuntimeOnly()` internal only; SecretSource renamed from `environment/file/secretStore/runtimeInjected` to `env/vault/kms/runtime|oauth|token`
...
- KB (canonical): `logicn-core-network-governance.md`, `logicn-core-network-webhook.md`
- KB (superseded): `logicn-core-network-v02.md` — uses sharedSecret field and has()/store(); superseded by webhook KB canonical names
...
- Manual check: package README and `logicn-core-network-webhook.md` agree on
  `ReplayStore.has/put` and `IdempotencyStore.get/put(IdempotencyRecord)`.
  `logicn-core-network-v02.md` still uses legacy protocol/secret/store names,
  and `logicn-core-network-governance.md` also differs from the webhook KB for
...
- KB (legacy/historical): `logicn-core-photonic-backend-architecture.md` — ⚠️ Update status: prior 3-value OpticalTransportMode and original LLN-PHOTONIC-001–006 meanings; historical context only
- KB (conflicting v0.2): `logicn-core-photonic-v02.md` — ⚠️ Update status: not implementation-ready; OpticalTransportMode 6-value enum conflicts with governance spec and vector spec; reconciliation required
- KB (governance spec): `logicn-core-photonic-governance-architecture.md` — DIRECT|WAVELENGTH|PACKETIZED|HYBRID|EMULATED|SIMULATED — also conflicts; see version conflict notes
...
- TODO first three items: "Reconcile canonical OpticalTransportMode enum", "Reconcile canonical LLN-PHOTONIC-001–006 table", "Confirm ownership before implementation"
...
**logicn-core-vector: photonic governance proposal**

- KB: `logicn-core-vector-photonic-governance.md` — OpticalTransportMode prior 3-value + v0.2 6-value "electrical"|"hybrid"|"photonic"|"waveguide"|"plasmonic"|"coherent", PhotonicCapability 6-value, PhotonicTopology 6-value, PhotonicRuntimeTarget/PhotonicExecutionPlan v0.2, all validation functions, LLN-PHOTONIC-001–006 prior+v0.2, deny-by-default fallback, BOUNDARY CONFLICT documented
- ⚠️ Update status: proposal/reference note only — logicn-core-photonic owns final definitions after reconciliation
...
**logicn-framework-api-server: full API server implementation**

- KB (prior): `logicn-api-boundary-architecture.md`
- KB (v0.2 formal spec): `logicn-framework-api-server-v02.md` (ReplayStore {exists/save} — differs from network package)
- KB (full impl spec): `logicn-framework-api-server-implementation.md` — ⚠️ Update status: ReplayStore.exists()/save() are adapter-level names; canonical contract is logicn-core-network has()/put(ttlSeconds)
...
- TODO first two items: "Map API-server ReplayStore.exists/save adapter names to logicn-core-network has/put", "Align webhook/idempotency implementation docs with logicn-core-network-webhook.md"
...
interpreter, runtime kernel — is implemented as JavaScript. `logicn-core/compiler/logicn.js`
...
execute LogicN programs, the runtime itself will be rewritten in LogicN's own
...
stage plan (Node.js bootstrap → Neutral IR → Rust/WASM → LogicN self-hosted).

**Existing TypeScript files:** `logicn-core-cli`, `logicn-core-config` and
`logicn-core-logic` contain TypeScript files from earlier contract and tooling work. These
are the tooling and contract layer — not the runtime engine itself. They remain where they
are for now; the Stage 1 runtime engine grows from `compiler/logicn.js` outward. Whether
...
No compilation. `logicn-core/compiler/logicn.js` is the existing foundation and the model
for all Stage 1 runtime additions.

| Package | Decision | Stage 1 notes |
| --- | --- | --- |
| `logicn-core` | **Runtime foundation** | `compiler/logicn.js` is the Stage 1 root — parser, type checker, formatter prototype already in JavaScript; runtime engine grows from here; no `src/` directory needed |
| `logicn-core-cli` | **Include** | CLI tooling layer — 10 TypeScript files present from earlier contract work; `check` and `fmt` working; `build`, `verify`, `deploy`, `explain`, `plan` command bodies are the remaining gaps; language decision (keep TypeScript or migrate to `.js`) made when those commands are implemented |
| `logicn-core-config` | **Include (scope-limited)** | Config and policy layer — TypeScript implemented (`ProjectConfig`/`EnvironmentConfig`/`RuntimeConfigHandoff`/`ProductionStrictnessPolicy`/`loadConfigFromObjects`/`validateHostPackageManifestBoundary`); diagnostic codes renamed to LLN-CONFIG-001…027; SecretConfigSource conflict resolved (2026-05-26) — v0.2 secret config surface now unblocked |
| `logicn-core-logic` | **Include (v0.1 extensions)** | Runtime logic layer — 418 lines TypeScript v0.1 numeric Tri (`Tri = -1\|0\|1`, `TriBoolPolicy`, generic `LogicState<N>`/`LogicDefinition<N>` framework); extend v0.1 with missing operations; v0.2 discriminated union rewrite is a separate milestone after self-hosting is achievable |
| `logicn-core-network` | **Add** | Runtime network layer — canonical v0.2 contracts in `logicn-core-network-webhook.md` are clean and unblocked (`WebhookRequest`/`WebhookResponse`, `IdempotencyRecord`, `ReplayStore.has/put`, `IdempotencyStore.get/put(IdempotencyRecord)`); high-value dependency for `logicn-framework-api-server`; no blocking conflicts |

### Surfaces Deferred from Phase 1

| Surface | Package | Reason |
| --- | --- | --- |
| `SecretConfigSource` / `SecretEnvironmentReference` / `loadEnvironmentConfig` | `logicn-core-config` | **Resolved (2026-05-26)** — canonical: `env\|vault\|kms\|runtime`; KB and TODO updated; implementation unblocked |
| `ProtectedSecret<T>` unwrap API | `logicn-core-security` | **Resolved (2026-05-26)** — canonical: `unwrapForApprovedSink(sink)`; `private revealUnsafeForRuntimeOnly()` internal only; KB and TODO updated |
| `OpticalTransportMode` / LLN-PHOTONIC diagnostic codes | `logicn-core-photonic` | Three incompatible 6-value enums; diagnostic code meanings differ across prior KB, v0.2 formal and vector spec |
| v0.2 `TriState` / `Decision` / `BoolBoundaryResult` / `OmniState` discriminated unions | `logicn-core-logic` | Phase 1 ships v0.1 extensions; v0.2 rewrite is a separate milestone |
| `ReplayStore.exists/save` adapter name mapping | `logicn-framework-api-server` | Must be mapped to canonical `has/put(ttlSeconds)` before implementation can proceed |
| `src/` contracts | `logicn-core` | No `src/` directory; no shared base contracts defined; deferred to a later pass |
...
```text
logicn-core-vector README says photonic belongs in logicn-core-photonic
28.txt proposes OpticalTransportMode/PhotonicRuntimeTarget/PhotonicExecutionPlan in logicn-core-vector
...
24.txt/28.txt v0.2   = secret: string | Uint8Array  ← canonical (logicn-core-network-webhook.md)
```
...
24.txt/28.txt v0.2   = has()/put(key, ttlSeconds)  ← canonical (logicn-core-network-webhook.md)
```
...
logicn-core-security-v02.md     updated — unwrapForApprovedSink(sink) canonical
logicn-core-security/README.md  updated — Coverage Reconciliation Status resolved
logicn-core-security/TODO.md    updated — decision item checked off
```

**SecretSource / SecretConfigSource** ✅ resolved (2026-05-26)

```text
Canonical SecretConfigSource (logicn-core-config):
    "env" | "vault" | "kms" | "runtime"                                        (4 values)

Canonical SecretSource (logicn-core-security adds auth-layer sources):
    "env" | "vault" | "kms" | "runtime" | "oauth" | "token"                   (6 values)

logicn-core-config-environment-secrets.md  updated — SecretConfigSource env|vault|kms|runtime
logicn-core-config/TODO.md                 updated — SecretConfigSource line corrected
logicn-core-security-v02.md               updated — "environment" renamed to "env"; file/secretStore/runtimeInjected removed
logicn-core-security/README.md             updated — SecretSource canonical names
logicn-core-security/TODO.md               updated — SecretSource item corrected
```
...
- `packages-logicn/logicn-core/docs/README.md`
- `packages-logicn/logicn-core/docs/package-boundaries.md`
- `packages-logicn/logicn-core/docs/webhooks.md`
- `packages-logicn/logicn-core/docs/tri-logic.md`
- `packages-logicn/logicn-core/docs/omni-logic.md`
- `packages-logicn/logicn-core/docs/backend-compute-support-targets.md`
- `packages-logicn/logicn-core-photonic/README.md`
- `packages-logicn/logicn-core-vector/README.md`
- `packages-logicn/logicn-core-logic/README.md`
- `packages-logicn/logicn-core-network/README.md`
- `packages-logicn/logicn-framework-api-server/README.md`
- `packages-logicn/logicn-core-security/README.md`
- `packages-logicn/logicn-core-photonic/TODO.md`
- `packages-logicn/logicn-core-vector/TODO.md`
- `packages-logicn/logicn-core-logic/TODO.md`
- `packages-logicn/logicn-core-network/TODO.md`
- `packages-logicn/logicn-framework-api-server/TODO.md`
- `packages-logicn/logicn-core-security/TODO.md`
...
- `logicn-core-network-v02.md` — superseded; uses sharedSecret/ws/wss/has()/store(); replaced by logicn-core-network-webhook.md canonical names
- `logicn-core-logic-v02.md` — formal spec (type: discriminant); superseded by package README
- `logicn-core-logic-tristate-developer-guide.md` — predates v0.2 kind: form; use package README as canonical
- `logicn-core-photonic-v02.md` — not implementation-ready; OpticalTransportMode conflict
- `logicn-core-photonic-backend-architecture.md` — legacy/historical; prior 3-value enum + prior diagnostic codes
- `logicn-core-vector-photonic-governance.md` — proposal/reference note; logicn-core-photonic owns final definitions
- `logicn-core-security-v02.md` — **updated (2026-05-26)**: canonical `unwrapForApprovedSink(sink)`; SecretSource canonical names; no longer stale
- `logicn-framework-api-server-implementation.md` — adapter-level ReplayStore names; canonical is network has/put

### Summary of changes

- Added coverage reconciliation status to 6 package READMEs and 6 package TODOs.
- Added ⚠️ Update status annotations to 8 KB files with conflicting or outdated content (including logicn-core-network-v02.md as superseded).
- Added logicn-core/docs/* ownership and canonical-contract sections (6 files).
...
- Marked logicn-core-logic README and tri-decision-bool KB as current runtime-facing canonical shape.
...
### Workspace stats (build/graph/LogicN_GRAPH_REPORT.md, generated 2026-05-25)
