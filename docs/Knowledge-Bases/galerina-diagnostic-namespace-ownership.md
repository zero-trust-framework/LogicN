...
**Status:** convention + enforcement (2026-06-16). **Why:** with ~180 emitted `FUNGI-*` codes across ~20
...
`docs/Knowledge-Bases/compiler-diagnostics.md` is the **canonical** diagnostic registry (every `FUNGI-*` code).
...
> **Every `FUNGI-*` code emitted in the compiler source (`code: "FUNGI-..."` literals) is registered in
...
- **Reserved family prefix + owner.** Each family (`FUNGI-SECRET-*`, `FUNGI-PRIVACY-*`, `FUNGI-CRYPTO-*`,
  `FUNGI-SUBSTRATE-*`, `FUNGI-EFFECT-*`, `FUNGI-VALUESTATE-*`, …) has a documented meaning in the registry.
- **Mechanism tag.** Each code states its mechanism so same-family codes can't blur:
  ...
  governance-verifier`. (This is exactly what distinguishes `FUNGI-PRIVACY-001` *declarative-clause* from
  `FUNGI-PRIVACY-002` *dataflow-taint*.)
- **Numbering.** Next free number within a family; **never reuse a retired number** (e.g. `FUNGI-TYPE-021` was
  retired in favour of `FUNGI-TYPE-023` — do not recycle 021).

## 4. How to add a diagnostic (the workflow)
1. Pick the family + next free number; choose the mechanism tag.
2. Emit `code: "FUNGI-FAMILY-NNN"` in the source.
...
This session's new codes (`FUNGI-CRYPTO-PQ-001`, …) are reconciled into the registry, not left on the allowlist.
