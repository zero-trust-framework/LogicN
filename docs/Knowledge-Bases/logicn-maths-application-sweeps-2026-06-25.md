...
Two adversarially-verified sweeps asking "where else can the LogicN ternary-governance maths
...
|---|---|---|---|---|
| .tmf | tensor batch-segment verify (Freivalds) | **refute** | 10 | Freivalds (float, probabilistic) on bit-exact .tmf bytes violates LLN-SUBSTRATE-001; no O(n³) to amortise; min-fold is O(N²), SLOWER than the linear hash walk. **DO NOT BUILD.** |
...
(`logicn-rd-62-maths-expansion-2026-06-25.md`).*
