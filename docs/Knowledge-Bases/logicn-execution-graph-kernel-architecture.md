...
LogicN should increasingly execute proofs.
```
...
**Connection to existing code:**
- `LLN-EFFECT-005` (BroadAliasUsed) suggestedFix is a primitive version of this
- `getGraphFlagSummary()` in devtools-graph provides the raw data

**Implementation phase:** Phase 32 (developer tooling — logicn explain)
...
- Phase 40 (LogicN 1.0): Governance Fabric as the foundation of LogicN's market identity
...
LogicN's ExecutionGraph maps directly onto Intel's hybrid core architecture.
...
core type. LogicN does not override the OS scheduler — it provides hints.
...
- `logicn-hybrid-wasm-native-architecture-v1.md` — the canonical v1.0 hybrid WASM architecture
- `logicn-core-economics-package.md` — CostGraph package design and Intel routing
- `logicn-security-anti-abuse.md` — anti-botnet architecture and capability governance
- `logicn-explicitness-principles.md` — nothing important hidden
- `logicn-passive-execution-plans.md` — PassiveExecutionPlan (current proof foundation)
