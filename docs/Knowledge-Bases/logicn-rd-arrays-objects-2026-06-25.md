...
Completed findings below were reproduced by actually compiling + running `.lln` (not assumed).
...
Several findings cite `logicn.mjs`. There are TWO things easily confused: the **production CLI** and the **v0.1
prototype fork** (`packages-logicn/logicn-core/compiler/logicn.js`, "do-not-cross-validate" per memory). Any
finding below that rests on `logicn.mjs` host-runtime drift **must be confirmed against the canonical
...
|---|---|---|
| Missing colon `{ age 30 }` | **0 diagnostics** — silently re-parsed as a *block*; surfaces later as a confusing `LLN-NAME-001 'age' not declared` | targeted `LLN-PARSE` "expected ':' after field name" in the `{`-disambiguation (parser.ts:2103-2131) |
