...
LogicN must model photonic compute with its own IR, its own effect types, its own
...
Until real hardware backends exist, LogicN supports simulation/planning artefacts:

```bash
logicn build --target photonic-sim
```
...
| Code | Meaning |
|---|---|
| `LLN-PHOTONIC-001` | Photonic op unsupported by target |
| `LLN-PHOTONIC-002` | Wavelength band unsupported |
| `LLN-PHOTONIC-003` | Wavelength channels conflict |
| `LLN-PHOTONIC-004` | Delay-line depth unsupported |
| `LLN-PHOTONIC-005` | Optical/electrical conversion required but not declared |
| `LLN-PHOTONIC-006` | Phase information lost after detection |
| `LLN-PHOTONIC-007` | Noise budget exceeded |
| `LLN-PHOTONIC-008` | Calibration evidence required |
| `LLN-PHOTONIC-009` | Photonic fallback changed semantics |
| `LLN-PHOTONIC-010` | Photonic target cannot be treated as GPU kernel |
...
|---|---|
| `logicn-core` | Shared type/effect/report hooks |
| `logicn-core-compute` | High-level compute planning and target/fallback policy |
| `logicn-core-photonic` | Photonic types: `Wavelength`, `Phase`, `Amplitude`, `OpticalSignal`, `OpticalChannel` |
| `logicn-target-photonic` | Photonic IR, target manifests, simulation reports, compatibility checks |
| `logicn-core-reports` | Photonic report schemas |
| `logicn-tools-benchmark` | Simulation and eventual hardware benchmark evidence |
