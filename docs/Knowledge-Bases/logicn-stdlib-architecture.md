...
```
The LogicN standard library should make the safe path the fast path.
```
...
```logicn
AuditLog.write({ email: protectedEmail })  → LLN-VALUESTATE-006 ❌
...
| Code | Name | Rule |
|---|---|---|
| `LLN-STDLIB-001` | `StdlibEffectNotDeclared` | effectful stdlib function called without declaring required effect |
...
```
global mutable state          → LLN-SEC-020/021
...
prototype extension           → LLN-SEC-021
dynamic package loading       → LLN-BACKEND-001
```
...
| Protected/redacted-aware LLN-VALUESTATE-006/007 | ✅ Phase 11B |
