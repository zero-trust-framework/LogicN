...
|---|---|---|---|---|
| **P1-a** (original) | Emitter / build provenance — `manifest-generator` / `proof-graph` sign the emitted WAT/WASM | the exact emitter output bytes | a *non-faithful* emit (a legal `.lln` whose WAT the assembler rejects → minimal-encoder stub) is signed-as-correct: the signature attests the stub's bytes, not that the stub realizes the source flow | **bounded by C2's faithful-compile gate** — under `LOGICN_PROFILE=production` a non-faithful stub (`diagnostics≠0`) FAILS the build before signing. So the acute exploit (ship a signed stub) is shut; the deep gap (a *non-stub but divergent* emit) remains |
...
1. **Reference semantics.** The walker (`interpreter.ts`) is the reference; the emitted WASM must be observably
   ...
   `logicn-core-compiler/tests/`: `fidelity-differential.test.mjs`, `wat-i64-differential.test.mjs`,
...
4. **For P1-b specifically:** demote `hardwareIdentity` to documented-advisory (done — code comment +
   ...
   root (TPM/secure-element measuring the actual backend), which LogicN does not own — so P1-b's deep fix is
   **out of scope by design** (LogicN governs; it does not manufacture a hardware root of trust). Keep the
...
See also: the threat-model P1 row (`logicn-threat-model-unleashed-2026-06-25.md`), `logicn-rd-0128-cicd-native-testgen.md`
