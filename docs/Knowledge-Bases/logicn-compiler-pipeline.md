...
Applies to: LogicN v1 / Phase 7A+
```

This document defines the ordered LogicN compiler pipeline, including pass
...
|---|---|---|---|---|---|---|
| 1 | Lexer | source text + file path | token stream + lexer diagnostics | `LLN-SYNTAX-*`, `LLN-CHAR-*`, parse-adjacent lexical diagnostics | implemented | `packages-logicn/logicn-core-compiler/src/lexer.ts` |
| 2 | Parser | token stream | `AstNode`, `FlowMeta[]`, `ParseDiagnostic[]` | `LLN-PARSE-*`, `LLN-SYNTAX-*` | implemented | `packages-logicn/logicn-core-compiler/src/parser.ts` |
| 3 | Symbol Resolver | AST | resolved AST metadata + `SymbolDiagnostic[]` | `LLN-NAME-*` | Phase 7A | `packages-logicn/logicn-core-compiler/src/symbol-resolver.ts` |
...
LLN-SYNTAX-*
LLN-CHAR-*
LLN-PARSE-* for scanner-level parse-adjacent errors
```
...
```text
packages-logicn/logicn-core-compiler/src/lexer.ts
```
...
```text
LLN-TYPE-*
LLN-MATCH-* where surfaced as match aliases
```
...
```text
packages-logicn/logicn-core-compiler/src/value-state-checker.ts
```
...
`logicn-core-manifest-generation-v02.md` specifies a later manifest aggregation
