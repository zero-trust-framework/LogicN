...
```
Stage A baseline:   LLN-GOV-002..012, LLN-CONTEXT-001, profiles (dev/production/deterministic) ✅
...
| Code | Name | Rule |
|---|---|---|
| `LLN-GOV-002` | `MISSING_AUDIT_FOR_GOVERNED_SINK` | database.write/payment.charge without audit.write |
| `LLN-GOV-003` | `PROTECTED_DATA_IN_RESPONSE` | denied field in response |
| `LLN-GOV-004` | `DENIED_TARGET_SELECTED` | compute.deny conflicts with network.outbound |
| `LLN-GOV-005` | `POLICY_PURPOSE_MISMATCH` | purpose "read-only" + database.write |
| `LLN-GOV-007` | `AUTHORITY_BLOCK_MISSING_REASON` | authority block has no reason |
| `LLN-GOV-008` | `EXPERIMENTAL_CODE_IN_PRODUCTION_PROFILE` | experimental in production |
| `LLN-GOV-009` | `PRIVILEGED_FLOW_MISSING_CAPABILITY` | privileged flow with no declared capability |
| `LLN-GOV-010` | `INTENT_MISSING_ON_SECURE_FLOW` | secure flow without intent |
| `LLN-GOV-011` | `UnknownContractSet` | use SetName references undeclared contract set |
| `LLN-GOV-012` | `ContractSetRequirementNotMet` | contract set audit requirement not met |
| `LLN-CONTEXT-001` | `REQUIRED_CONTEXT_NOT_ACCESSED` | required context field never accessed |
...
Current:
- `contract.response.denies email` vs returned fields → LLN-GOV-003
...
```
deny remote.execution → enforced by LLN-GOV-004
...
| Feature | Status |
|---|---|
| LLN-GOV-002..012, LLN-CONTEXT-001 | ✅ Stage A |
