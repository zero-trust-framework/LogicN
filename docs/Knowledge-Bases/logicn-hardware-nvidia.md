...
```
LogicN treats Nvidia GPU hardware as a governed execution target.
...
**The LogicN Rule: ValueGraph governs precision, not hardware.**
...
The compiler enforces: `safety_critical` and `medical` classifications CANNOT use FP4/FP8 — the governance verifier emits `LLN-HW-004` if `allow precision_fp4` appears on a high-risk flow.
...
The most critical Blackwell security feature for LogicN: **CUDA Virtual Memory Management** (`cuMemAddressReserve`).
...
- `logicn-hardware-compute-fabric.md` — HardwareGovernanceClass, ProofLevel escalation
- `logicn-hardware-targets.md` — All target IDs
- `logicn-hardware-amd.md` — AMD comparison (ROCm vs CUDA)
- `logicn-core-economics-package.md` — CostGraph precision routing
