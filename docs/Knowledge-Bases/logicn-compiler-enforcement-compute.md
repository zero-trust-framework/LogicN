...
LogicN defines ambitious semantics for heterogeneous compute, deterministic execution
...
```text
LLN-TARGET-CAP-001: database.write is forbidden inside GPU-targeted flow
...
LogicN supports `verify cpu_reference` syntax:
...
| Code | Meaning |
|---|---|
| `LLN-TARGET-CAP-001` | Effect forbidden inside this compute target |
...
|---|---|
| `logicn-core` | Effect/capability type definitions, budget policy syntax |
| `logicn-core-compiler` | Effect propagation, capability validation, target compatibility pass |
| `logicn-core-compute` | Target capability rules, resource budget model |
| `logicn-core-runtime` | Budget enforcement, mismatch handling, reference execution |
| `logicn-core-reports` | Target compatibility report, budget utilization report, verification report |
| `logicn-target-gpu` | GPU-specific capability rules and budget enforcement |
| `logicn-target-wasm` | WASM-specific capability restrictions |
