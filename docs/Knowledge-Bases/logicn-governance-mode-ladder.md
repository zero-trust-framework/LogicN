...
ceiling**. Project `full` + flow `lean` → `full` (rejected, `LLN-CONFIG-GOV-001`). Project `auto`
+ flow `full` → `full` (opt-up, fine). An explicit `lean` on a flow that isn't EffectFree/taint-clean
→ safety override to `full` (`LLN-CONFIG-GOV-002`). Mirrors the shipped `posture.ts` fail-secure
`off|auto|on` shape (secure pole = `full`).

## The hard floors (never relaxable, even under `lean`)
Crypto-on-core (`LLN-SUBSTRATE-001`) · K3 capability gate (`(required & granted) === required`) ·
secret egress (`LLN-SECRET-002`) · privacy egress (`LLN-PRIVACY-002`) · the three-valued border
