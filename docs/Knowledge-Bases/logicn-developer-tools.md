...
Three closely related developer tooling features make LogicN's strictness helpful
rather than burdensome:

1. **LSP Implementation** — standard editor features plus LogicN-specific effect,
...
The LogicN LSP provides all standard editor capabilities:
...
```text
LLN-TYPE-001            type mismatch
LLN-EFFECT-001          undeclared effect
LLN-CAP-001             missing capability
LLN-SECURITY-001        secret leaked to log
LLN-TAINT-001           tainted value reached sink
LLN-SUPPLY-001          dependency content hash mismatch
LLN-CRYPTO-001          runtime crypto algorithm selection forbidden
LLN-MEMORY-SCOPE-001    request-scoped value escapes request
```
...
```text
LogicN: Explain Diagnostic
LogicN: Show Flow Effects
LogicN: Show Capability Path
LogicN: Show Boundary Graph
LogicN: Show Affected Flows
LogicN: Generate AI Context
LogicN: Run Check
LogicN: Open Security Report
```
...
```json
{
  ...
  "primary_span": "orders.lln:12:7",
  "related_spans": [
    { "span": "orders.lln:11:3", "label": "value moved here" }
  ],
  "fixes": [
    {
      ...
      "edits": [
        { "file": "orders.lln", "range": "11:11-11:16", "replacement": "&order" }
      ]
    }
  ]
}
...
```bash
logicn check --fix-preview      # preview all available fixes
logicn fix                       # apply all machine-applicable fixes
logicn fix --safe                # apply only machine-applicable fixes
logicn fix --review-security     # preview security-sensitive fixes
```
...
|---|---|
| `LLN-MEMORY-001` moved value | borrow, clone, reorder |
| `LLN-EFFECT-001` undeclared effect | add effect (review required), move call to allowed flow |
| `LLN-TAINT-001` tainted sink | add sanitizer call |
| `LLN-SECURITY-001` secret printed | replace with `redact(secret)` |
| `LLN-ERROR-001` unhandled Result | add `?` propagation or `match` |
| `LLN-MATCH-001` missing variant | add match arm |
...
```bash
logicn repl --pure                                               # pure flows only, no effects
logicn repl --profile local                                     # local profile
logicn repl --profile staging --capabilities database.read,orders.preview
```
...
| Code | Meaning |
|---|---|
| `LLN-REPL-001` | Capability not granted for session |
| `LLN-REPL-002` | Secret reveal denied in REPL |
| `LLN-REPL-003` | Production profile requires audited session |
| `LLN-REPL-004` | Destructive effect requires confirmation |
| `LLN-REPL-005` | Project manifest could not be loaded |
| `LLN-REPL-006` | Flow cannot be evaluated because module is invalid |
| `LLN-REPL-007` | Target fallback occurred during REPL execution |
| `LLN-REPL-008` | Expression exceeded REPL resource limit |
...
|---|---|
| `logicn-core` | Language syntax, AST, type/effect semantics, diagnostic codes |
| `logicn-core-compiler` | Parser, checker, diagnostics, source maps, incremental analysis API, fix generation |
| `logicn-core-cli` | LSP entrypoint, `logicn fix` command, `logicn repl` command |
| `logicn-core-runtime` | Governed expression execution, effect dispatch, REPL runtime |
| `logicn-core-security` | Capability checks, secret redaction, reveal policy |
| `logicn-core-config` | Profile/environment loading, production gates |
| `logicn-core-reports` | Diagnostic schema, REPL session reports, expression evidence |
| `logicn-devtools-project-graph` | Workspace graph, symbol graph, AI-readable context |
| `logicn-framework-app-kernel` | Optional request/API context simulation in REPL |
