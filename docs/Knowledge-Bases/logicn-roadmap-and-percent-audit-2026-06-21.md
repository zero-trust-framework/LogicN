...
`logicn-runtime-status-SOT.md` (last verified 2026-06-06), **this is newer** — the SOT's counts and its
...
| Axis | % | Basis |
|---|---|---|
| **A — governed decision logic in `.lln`** | **~56%** | 14 of 25 `examples/auth-service` services covered by executing tests (unchanged this session) |
...
| **App-framework layer (L2–L3)** | **~75%** | **corrects the stale "templates" claim.** Admission/fusion border **100%** (fuse-loader 3 gates + `planComposition` + revocation, 60 tests); scaffolder **100%** (B1 `logicn new app`); unified admission vocabulary **100%** (B2); governed resolver shipped (hash + sig + registry + install-deny + LLN-PKG-006 revocation). **Missing:** signed central registry index (B5a), richer worked example (B6); `framework-example-app`/`-api-server` remain template/reference |
...
| **Governance rules** | mature | 35+ LLN codes registered + enforced (incl. new **LLN-PKG-006** this session); namespace-ownership drift-guarded |
...
1. **Tri-Pipe fault-tolerance hardenings** — multi-agent re-R&D **in flight** (`wpa9c3wqk`); its ranked
   stability plan + fault model folds into [logicn-framework-plan-2026-06-21.md](logicn-framework-plan-2026-06-21.md) when it lands. *(highest leverage for "as stable as possible")*
2. **WASM codegen gaps (concrete, scoped, owner-go'd):** #200 nested-member access → walker; run-host
   string unification (`logicn run --wasm`); #171 None sentinel; #172 i32→str.
