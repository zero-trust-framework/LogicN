...
Self-hosting goal: lexer.lln must produce identical token stream
```
...
Less allocation, no regex engine overhead, easier to self-host in lexer.lln.
...
### 5. Lexer Safety Limits (LLN-LEX-*)

Already implemented: LLN-LEX-001 (nesting), LLN-LEX-002 (token length), LLN-LEX-003 (unicode escape) ✅.

Add remaining limits:
- Max file size (e.g. 10MB) → LLN-LEX-004
- Max line length (e.g. 10,000 chars) → LLN-LEX-005
- Max total token count (e.g. 1,000,000) → LLN-LEX-006
...
- `hot_reload` — runtime mutation concept, against LogicN philosophy
...
Phase 18A:
  ...
  5. Add LLN-LEX-004/005/006 (file size, line length, token count limits)
...
Stage B (ongoing):
  9. lexer.lln produces identical token stream to TypeScript lexer
  10. lexer.lln handles all 9 token kinds correctly
```

## Final Principle

```
The LogicN lexer should be deterministic, bounded, span-rich,
allocation-light, and boring.

That improves speed, security, IDE tooling, AI repair, and Stage B
self-hosting without weakening LogicN's governance model.
```

## See Also

- `logicn-lexer-lln.md` — self-hosted lexer specification
- `src/self-hosted/lexer.lln` — Stage B Milestone 1 (executing)
- `logicn-performance-roadmap.md` — broader performance roadmap
</content>
</invoke>