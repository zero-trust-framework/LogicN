...
| New | `suggestedCode` in `LLN-EFFECT-001` diagnostics |
...
|---|---|
| `logicn check` (dev) | Diagnostic severity: **warning**. `suggestedCode` is included. Compilation continues. |
| `logicn build --production` | Diagnostic severity: **error**. Build fails. No effects are silently granted. |
...
The diagnostic code for both severities is `LLN-EFFECT-001`.

---

## AI Semantic Graph Addition

Flow nodes emitted into `logicn.ai.json` are extended with effect tracking data:
...
- Diagnostic: `LLN-EFFECT-001` warning with suggested patch.
...
- `LLN-EFFECT-001` diagnostics always include `suggestedCode` containing the complete, ready-to-paste `contract.effects` block.
...
- `logicn-gradual-capability-inference` — the broader gradual typing approach that effect inference is part of.
