...
See: logicn-adaptive-runtime-profiles.md for runtime behaviour
```

## TL;DR
- Tensor shape inference belongs in the GIR, not the source language
- `ai.inference` effects automatically suggest NPU/GPU preference (LLN-HINT-COMPUTE-001)
...
in the photonic target bridge (`logicn-target-photonic`), not in LogicN source.

---

## 2. Effect → Target Affinity — Implemented (LLN-HINT-COMPUTE-001)
...
```text
LLN-HINT-COMPUTE-001 COMPUTE_TARGET_MISSING_FOR_AI_INFERENCE [info]
...
### What LogicN source never contains

```logicn
// Wrong — never add photonic-specific syntax to source
let wavelengthEmbedding: PhotonicTensor<[1, 768]> = ...   // NOT LogicN
let optical: wavelength Tensor<Float32> = ...               // NOT LogicN
```
...
| LLN-HINT-COMPUTE-001 (ai.inference without compute target) | 8A | ✅ Implemented |
...
| Adaptive runtime target learning | 8+ | See logicn-adaptive-runtime-profiles.md |

---

## See Also

- `docs/Knowledge-Bases/logicn-gir-schema.md` — full GIR schema
- `docs/Knowledge-Bases/logicn-tensor-arity-decision.md` — Tensor<T, Shape> arity
- `docs/Knowledge-Bases/logicn-adaptive-runtime-profiles.md` — runtime target learning
- `docs/Knowledge-Bases/logicn-quantum-target-bridge.md` — quantum target architecture
- `docs/Knowledge-Bases/logicn-core-photonic-v02.md` — photonic compute
