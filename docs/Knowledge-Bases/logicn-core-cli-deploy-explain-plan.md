...
The CLI is the operational governance surface for LogicN.
...
| Command | Purpose |
| --- | --- |
| `logicn build` | Compile workspace and emit manifests/reports |
| `logicn verify` | Validate artefact integrity, manifests, and policies |
| `logicn deploy` | Validate and deploy a verified artefact |
| `logicn explain` | Explain effects, boundaries, capabilities, and plans |
| `logicn plan` | Produce compute/runtime execution plans |
...
`logicn build` compiles the workspace, generates manifests, and emits all reports required for verification and deployment.
...
`logicn deploy` validates a verified artefact against deployment policy and deploys it to the selected target.
...
`logicn explain` exposes compiler and deployment reasoning.
...
`logicn plan` produces execution and compute planning metadata.
...
The LogicN CLI is not only a developer tool.
...
These contracts define the operational interface between the LogicN compiler, runtime, deployment system, governance layer, and future heterogeneous compute planning systems.
