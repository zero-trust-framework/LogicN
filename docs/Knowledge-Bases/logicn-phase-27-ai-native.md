...
Phase 27 introduces Deno Deploy as a supported runtime target for LogicN flows:

```bash
logicn build --target deno-deploy
logicn deploy --runtime deno
```
...
- TypeScript-native — LogicN emits typed interfaces compatible with Deno's type system
- Edge-distributed — AI inference can run close to the user
- Permissioned by default — aligns with LogicN's capability model (Deno --allow-* flags map to LogicN effects)
...
Fallback: cpu (always declared — LLN-GOV-014 enforces this)
```
...
## Stage B: type-checker.lln Parity Goal

Phase 27 includes a Stage B parity goal: the self-hosted `type-checker.lln` should reach functional parity with the TypeScript `type-checker.ts` for the subset of types used in AI/inference flows:

- `Tensor<Float32, [768]>` — element type and shape checking (LLN-TYPE-030, LLN-TYPE-031)
- `protected MessageText` — value-state boundary enforcement (LLN-VALUESTATE-006)
- `NativePluginManifest` — manifest schema validation
- `DataHandle` — offset/length type enforcement (no raw pointer types in source)

**Parity milestone:** `type-checker.lln` must emit equivalent diagnostics for the `classifyMessage` flow as `type-checker.ts` when run via the interpreter (Stage A execution).

The self-hosted checker is in `src/self-hosted/type-checker.lln`. Phase 28 will extend it to cover the full native plugin ABI surface.
...
All native acceleration modules in LogicN must satisfy these rules (source: hybrid-wasm-architecture.md):

**Rule 1 — Signed**
Native modules must be signed (same scheme as packages: LLN-PKG-005 equivalent).
...
Shared memory uses offsets and lengths (DataHandle), not raw pointers in LogicN source.
...
LLN-GOV-014 enforces this: missing fallback is a warning that escalates to error in production.
...
Defined in `packages-logicn/logicn-core-compiler/src/type-registry.ts`:
...
```
Phase 24: LogicN → WAT → WASM (pure flows)
...
- `docs/Knowledge-Bases/logicn-hybrid-wasm-architecture.md` — canonical architecture document
- `examples/ai-inference/classifyMessage.lln` — the complete example
