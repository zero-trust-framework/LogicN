...
The LogicN API boundary is the layer where untrusted external HTTP input enters
the governed LogicN runtime. It is not a general web framework — it is a
typed, permissioned, auditable, memory-bounded, secret-safe request gateway.

## Package

```text
logicn-framework-api-server   — HTTP transport, body limits, server, logs
logicn-framework-app-kernel   — request lifecycle, route policy, validation
```
...
New package: logicn-framework-api-server must be created.
```
...
7.  App Kernel decodes request into a typed LogicN value.
...
10. Runtime executes typed LogicN flow.
...
logicn-core-runtime
    Execute checked LogicN flows safely.
...
export interface LogicnApiManifest {
  schemaVersion: "logicn.api.manifest.v1"
  api: string
  version: string
  generatedAt: string
    ...
    try {
      ...
      if (!route) {
        return writeJson(res, 404, {
          error: { code: "LOGICN_ROUTE_NOT_FOUND", message: "Route not found" }
        })
      }
      ...
      writeJson(res, 500, {
        error: { code: "LOGICN_API_INTERNAL_ERROR", message: "Request could not be processed" }
      })
    } finally {
...
export async function readBodyWithLimit(
  ...
  for await (const chunk of req) {
    ...
    if (total > maxBytes) {
      throw new LogicnHttpError(413, "LOGICN_API_BODY_TOO_LARGE", "Request body is too large")
    }
...
export async function assertWebhookNotReplayed(input: {
  ...
  if (age > input.route.webhook!.replayWindowSeconds) {
    throw new LogicnHttpError(
      401,
      "LOGICN_WEBHOOK_REPLAY_WINDOW_EXPIRED",
  ...
  if (!inserted) {
    throw new LogicnHttpError(409, "LOGICN_WEBHOOK_DUPLICATE_EVENT", "Webhook event has already been processed")
  }
}
```
...
OpenAPI output is a publication artefact generated from the LogicN API
manifest. It is not the source of truth — the LogicN manifest is authoritative.
...
export function mapErrorToHttpResponse(
  ...
  return {
    status: 500,
    headers: { "content-type": "application/json" },
    body: { error: { code: "LOGICN_API_INTERNAL_ERROR", message: "Request could not be processed", requestId } }
  }
}
```
...
```text
1. LogicN Core defines the language.
...
10. OpenAPI is generated from LogicN, not the source of truth.
```
...
```text
logicn-core-compiler    → emits LogicnApiManifest from checked API declarations
logicn-core-network     → provides NetworkPolicy for assertNetworkAllowed()
logicn-core-security    → redaction, auth checks, capability validation
logicn-core-runtime     → executes typed LogicN flows
logicn-core-reports     → consumes route audit/security/memory reports
logicn-framework-app-kernel → owns request governance lifecycle
logicn-framework-api-server → owns HTTP transport and body limits
```

See also: `logicn-core-network-governance.md`,
