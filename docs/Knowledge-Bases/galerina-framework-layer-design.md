...
|---|---|---|
| `galerina-framework-app-kernel` | **Spec only, 0 `.ts`** | README + ARCHITECTURE + REQUIREMENTS + TODO; 5 `.fungi` fixtures run through the interpreter |
...
`galerina build <file.fungi>` → `build/<name>.wasm` + `.wat` + signed `.lmanifest` (CBOR/Ed25519→ML-DSA-65)
+ `governance-impact.json`. **Single-file → governed signed `.wasm` works** (P9). `import ./path.fungi`
...
2. Resolve + DAG-merge the entry's `import ./*.fungi` graph (#94) into one module.
...
```text
my-custom-api-rest/
  package.fungi.json     # { name, kind:"api-protocol"|"extension", provides:"rest",
                       #   entry:"src/index.fungi", seam:"protocol.inbound", capabilities:[...] }
  src/ index.fungi ...   # Galerina flows + contracts; imports merged via #94 DAG
...
### Build order
- **B1 — `galerina build --package`**: ✅ **DONE + verified (2026-06-14).** Reads `<dir>/package.fungi.json`
  ...
  `<name>.fuse.json`** (schema `fungi.fuse.v1` — kind/provides/seam/capabilities/artifacts/wasmSha256).
...
|---|---|---|
| `galerina-framework-app-kernel` | Template: the one secure App-Kernel boundary (protocol-agnostic) | Scaffold + early `.ts` (P1 keystone, #172) + 5 `.fungi` fixtures |
...
compile/run/check `.fungi`), which is **correct** — the language+core is the front door.
...
Real Galerina syntax (modelled on `examples/auth-service/createSession.fungi` and
`galerina-framework-app-kernel/examples/typed-api-boundary.fungi`). The SOAP form is
**proposed** (phase P4); the `secure flow` handler is what runs today.

```galerina
// orders/createOrder.fungi — the governed handler (real, compiler-checked)
...
```galerina
// orders/rest.fungi — REST surface (galerina-framework-api-server)
api OrdersApi {
  POST "/orders" {
...
```galerina
// orders/soap.fungi — PROPOSED galerina-api-protocol-soap (phase P4)
