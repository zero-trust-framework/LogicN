...
- "Files do not expose LogicN code. Routes expose governed entry points."
...
LogicN rejects this pattern entirely. **Files do not expose LogicN code. Routes expose governed entry points.** A flow defined in any file, in any package, is unreachable by external callers unless a specific authorised entry point declares it.
...
There is no mechanism in LogicN for an external caller to invoke a flow directly by name, path, or URL without an explicit entry point declaration. The runtime refuses unrouted execution. The compiler enforces this at the declaration level.
...
A flow declared `private` is internal to the package or module. It can be called by other LogicN flows but cannot be reached through any external entry point — even if a route attempts to trigger it.
...
**Diagnostic:** Attempts to reach ungated flows produce diagnostic `LLN-ENTRY-001 UndeclaredEntryPoint`.
...
3. `private` flows are callable only from other LogicN flows in the same package
...
- `logicn-flow-contracts.md` — full contract model
...
- `logicn-route-runtime-spec.md` — runtime handling of route entry points
- `logicn-governance-architecture.md` — overall governance model
- `compiler-diagnostics.md` — diagnostic codes including LLN-ENTRY-001
