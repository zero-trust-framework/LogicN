...
| 2 | **`.spore`** file extension | **RESOLVED 2026-06-26 (owner):** `.lln`→`.spore` (SOURCE); `.tmf` STAYS `.tmf`. branding-3's `.tmf`→`.spore` is rejected. | **SETTLED** — `.spore` = the source extension only; the compiled passport keeps `.tmf`. No longer a rebrand blocker. | 8 |
