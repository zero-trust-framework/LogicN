...
`logicn-contract-economics.md` for the canonical description of this pattern.
...
MATHEMATICAL (AST / type system)
  detects: Girard's Paradox construction (Universe stratification violation, LLN-UNIV-001)
...
status register described in `logicn-asic-cyber-physical.md` §3.
...
- **Location:** ASIC active-mesh zone (Tier 1 Sovereign Core, see `logicn-asic-cyber-physical.md`)
...
`logicn-asic-cyber-physical.md` §6). The flow is terminated cleanly (all resources released,
...
This follows the same editorial principle stated in `logicn-design-secrets-epilogue-blocks.md` §0.
...
- **Mathematical:** Compiler catches LLN-UNIV-001 (self-referential type) at compile time.
...
- **Mathematical:** The flow's output type is downgraded to `Unverified_Raw` — the output
  ...
  (LLN-GATE-* pattern). The taint monotonicity rule (see `logicn-fortified-typed-logic.md` §4)
...
- `logicn-asic-cyber-physical.md` — Physical shielding tier detail (HIV, three tiers, tamper
  response strategy, `cyber_physical_hardening {}` contract sub-block)
- `logicn-fortified-typed-logic.md` — Mathematical layer (Universe stratification, termination
  proofs, taint monotonicity) that the cross-layer matrix coordinates with
- `logicn-contract-economics.md` — Economic layer (ValueGraph, CostGraph, `economics {}`
  auto-by-default block) that forms the third axis of the resilience matrix
- `logicn-design-secrets-epilogue-blocks.md` — `secrets {}` and `epilogue {}` dual-mode
  pattern; `on_tamper_signal` contract field follows the same auto-by-default principle
- `logicn-zk-proof-plan.md` — `epilogue { generate_proof zk_snark_receipt }` produces the
