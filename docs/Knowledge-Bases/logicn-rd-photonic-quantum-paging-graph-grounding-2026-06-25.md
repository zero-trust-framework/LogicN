...
The two fences (never relaxed): **crypto + bit-exact determinism stay binary/digital (LLN-SUBSTRATE-001);
...
| The waves carry security (entanglement self-proves / QKD un-tappable / drop the MAC) | **OVERCLAIM** | Adjacent trap: a lane never carries keys/ledger. QKD *needs* the classical MAC "drop the MAC" discards; crypto stays binary (LLN-SUBSTRATE-001) |
...
governance + latency-predictability, not raw speed. **Maps to:** logicn-ext-photonic-emulator + photonic-admission.
...
| "Cancer drug structure in **seconds vs 10,000 years**" | **OVERCLAIM** | Conflates Google's random-circuit-sampling supremacy benchmark (no chemistry value) with drug design; no fault-tolerant QC does this today; LogicN governs **ffsim**, a *classical* simulator whose win is an exact subspace conservation law, not a quantum speedup |
...
logicn-ext-bridge-quantum (SHIPPED) + the decideAtBoundary/lease K3 collapse rail + LLN-SUBSTRATE-001.
...
the working set in locked RAM (logicn-ext-secrets-tmf already does this). The genuine LogicN-specific value is the
**WASM-sandbox-managed spill** (not trusting host-OS swap) + a **governed, attested crypto-erase witness** bound
to the .lmanifest — and that witness already ships (`substrate-erasure.ts`, `LLN-RETAIN-001`). So: not invented,
