...
**Phase 18A:** Token interface extended — `start`/`end` byte offsets, `endLine`/`endColumn` spans, `kindId` (TokenKindId numeric enum). Slice-based identifier scanning, direct lookahead for multi-char operators. FUNGI-LEX-004 (file > 10MB), FUNGI-LEX-005 (line > 10k chars), FUNGI-LEX-006 (> 100 diagnostics) implemented.
...
**Phase 25 target:** lexer.fungi token-by-token parity with TypeScript lexer on all CEC examples.

## What it is

`src/lexer.fungi` is the Galerina lexer written in Galerina itself. It is the first
component of the self-hosted compiler — the first proof that Galerina can describe
its own tools.

When compiled by the TypeScript bootstrapper and executed, `lexer.fungi` must
...
| `mut` bindings with reassignment | Phase 11A.4/11 | ✅ Built — FUNGI-BINDING-005/006 enforced |
...
Phase 12A blockers (while loops, mut reassignment) are resolved. The lexer.fungi itself parses with 0 errors.
...
|---|---|---|
| `FUNGI-LEX-001` | `ExcessiveNesting` | Generic type nesting depth exceeds 8 levels |
| `FUNGI-LEX-002` | `OversizedToken` | String literal or identifier exceeds 10,000 characters |
| `FUNGI-LEX-003` | `InvalidUnicodeEscape` | A `\u` or `\u{}` escape is malformed or the codepoint is out of Unicode range |
| `FUNGI-LEX-004` | `FileTooLarge` | Source file exceeds 10 MB, or token count exceeds 1,000,000 |
| `FUNGI-LEX-005` | `LineTooLong` | A single line exceeds 10,000 characters (warning severity) |

All errors carry `line`, `col`, and a human-readable `message`. The `code` field
is a `String` holding the `FUNGI-LEX-XXX` identifier so tooling can switch on it.

```galerina
record LexError {
  code: String      // e.g. "FUNGI-LEX-001"
  message: String
  line: Int
  col: Int
}
...
pure flow scanString(source: String, pos: Int, line: Int, col: Int) -> Result<Token, LexError>
{
  ...
  while i < source.charCount() {
    ...
    match ch {
      None => {
        return Err(LexError { code: "FUNGI-LEX-001", message: "Unterminated string literal", line: line, col: col })
      }
      Some(c) => {
        match c {
          '"' => {
          ...
          '\\' => {
            ...
            match next {
              None => {
                return Err(LexError { code: "FUNGI-LEX-001", message: "Incomplete escape sequence", line: line, col: col })
              }
              Some(esc) => {
                match esc {
                  'n'  => { chars = chars + "\n" }
                  ...
                  'u'  => {
                    ...
                    // Validate 4 hex digits; emit FUNGI-LEX-002 on failure
                    chars = chars + scanUnicodeEscape(source, i + 2).unwrapOrErr(
                      LexError { code: "FUNGI-LEX-002", message: "Invalid unicode escape", line: line, col: col }
                    )
                    i = i + 4
                  }
...
## Checklist for `lexer.fungi` to be valid Galerina
...
These patterns should be mirrored in `lexer.fungi` when the self-hosted lexer reaches
...
### Safety limits (FUNGI-LEX-004, FUNGI-LEX-005)

Two new guards were added:

- `FUNGI-LEX-004 FileTooLarge` — rejects source files over 10 MB before scanning begins.
  Also fires if the token stream exceeds 1,000,000 tokens during scanning.
- `FUNGI-LEX-005 LineTooLong` — emits a warning (not an error) when a completed line
...
Once `lexer.fungi` passes its own tests, the bootstrapper compiles it:

```bash
galerina build src/lexer.fungi --target node-js
```
...
```text
lexer.ts output == lexer.fungi output   => PASS
```
