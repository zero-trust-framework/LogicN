# LogicN Phase 13/14 — Design Decisions
...
```
Phase 13:  packages/logicn-devtools-graph-algorithms/src/SemanticGraph.ts
Later:     C:\laragon\www\LLN-Graph (standalone) — only after API stabilises
...
"Move LLN-Graph into the centre" means: make graph concepts central to compiler/runtime
...
## Decision 2 — logicn check is warn-only for missing effects; production fails

```
logicn check              = warn + suggest (developer feedback loop)
logicn check --strict     = missing effects fail
logicn build              = warnings allowed unless configured
logicn build --production = missing effects fail (governance boundary)
```
...
**LLN-EFFECT-001 in production mode:**
```text
...
- `logicn-semantic-graph-system.md` — SemanticGraph full proposal
- `logicn-gradual-capability-inference.md` — dev/CI/production modes
- `logicn-stage-b-root-capability-provider.md` — compiler authority isolation
- `logicn-deterministic-selfhost-verification.md` — full verification proposal
- `logicn-compiler-phase-memory-boundaries.md` — arena memory (future)
