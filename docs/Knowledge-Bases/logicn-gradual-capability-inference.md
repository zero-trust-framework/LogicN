...
### 1. Development — `logicn dev` or `logicn check --infer-effects`
...
[suggest] flow 'getUser' is missing effect declarations.
  ...
  Run: logicn fix --effects src/flows/getUser.ln
...
Governance report written to: .logicn/effect-report.json
```

### 3. Production — `logicn build --production`
...
Inference works transitively. The compiler builds an **EffectGraph** within the LLN-Graph and walks it to determine the complete effect footprint of any given flow.
...
The LogicN language server exposes inferred effect data to IDE tooling. A flow panel in the editor shows the current state of a flow's effect declarations at a glance:
...
```
logicn fix --effects src/flows/getUser.ln
```
...
LogicN separates two layers of concern:
...
This keeps LogicN learnable. A developer new to the codebase can read `contract { effects { database.read, audit.write } }` and understand exactly what the flow does. They do not need to understand the capability registry to read, write, or review application code.
...
The distinction is precise: a suggestion that the developer ignores leaves the production build failing. A silent grant that the developer never sees means authority was exercised without review. LogicN only permits the former.

This principle holds at every layer:

- Inference surfaces undeclared effects; it does not patch them away
- `logicn fix --effects` writes to files the developer commits; it does not modify the compiled output invisibly
...
|---|---|---|
| `logicn dev` | Suggest + infer | Continues |
| `logicn check` | Warn + report | Continues (configurable) |
| `logicn build --production` | Error | Fails |
...
- `logicn-semantic-graph-system` — LLN-Graph and EffectGraph architecture
- `effect-checker` — the compiler pass that validates declared effects against usage
- `logicn-roadmap` — Phase 13 and surrounding architectural proposals
