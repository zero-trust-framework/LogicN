...
**Package:** `packages-logicn/logicn-devtools-benchmarks` · **Status:** Proposed (2026-06-06) · **Post-P9, non-blocking.**
...
- **Slot-in:** `benchmarks/governance-tax/{benchmark.lln,node.mjs,python.py,bench-wasm.mjs}`; register `{ id:"governance-tax", dir:"governance-tax", logicnOpsPerRun:1, passiveCallCount:100 }`.
...
- **Slot-in:** `benchmarks/context-switch/{node.mjs,bench-wasm.mjs,benchmark.lln?}`; register `{ id:"context-switch", dir:"context-switch", logicnOpsPerRun:1000000, passiveCallCount:5 }`. `node.mjs` JS call round-trip = "no-border" baseline; delta vs WASM = border tax.
