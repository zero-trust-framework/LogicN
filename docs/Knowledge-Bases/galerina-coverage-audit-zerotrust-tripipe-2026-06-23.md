...
- **Photonic/Tri hub (fully assessed):** tower-citizen `substrate-model.ts` (FUNGI-SUBSTRATE-001..004, `effectiveVerdict=vAnd(ideal,reading)` availability-not-safety), `@galerina/substrate-math` single-source-of-truth, core-logic K3 (FUNGI-GOV-3VL-001), tri-pipe/hardware-tier fail-closed-to-binary (`npm run prove`, R&D 0009/0053/0054/0056), ext-photonic-emulator (46 tests, prove-own-maths D1/D2 25/25, degrade-only/projected-not-measured).
