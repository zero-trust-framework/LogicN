...
### 2. Compile-time governance proof is LogicN's answer to comptime

Instead of runtime checks and escape hatches, LogicN proves before execution:
...
```
logicn check                  — development: warn, do not block
logicn check --strict         — warn becomes error
logicn build --dev            — governance is checked, some warnings allowed
logicn build --production     — all governance must pass, no warnings
logicn build --deterministic  — same as production + bit-for-bit reproducibility required
logicn build --target wasm    — emit WASM module instead of JS bootstrap
```
...
Same LogicN source should produce:
...
- **Effect checker**: `LLN-STDLIB-001` catches hidden effectful calls
...
- **PackageResolver**: `LLN-PKG-003/004/005` catch hidden capabilities, install scripts, unsigned packages
...
Before shipping any LogicN feature or API, ask:
