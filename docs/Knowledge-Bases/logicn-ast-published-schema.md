...
`logicn ast --json` emits a LogicN AST conforming to a published, versioned JSON Schema.
...
```json
{
  "$schema": "https://schemas.logicn.dev/ast/v1/schema.json",
  "schemaVersion": "1.0.0",
...
```bash
logicn ast api-orders.lln --json
logicn ast src --json --out build/ast
logicn ast api-orders.lln --json --syntax-only
logicn ast api-orders.lln --json --resolved
logicn ast src --json --compact
logicn ast src --json --include-trivia
logicn schema ast --out schema/logicn-ast-v1.json
logicn ast api-orders.lln --json | logicn schema validate ast
```
...
```text
logicn ast --json           → stable public AST
logicn check --report-json  → stable diagnostics/type/effect reports
logicn graph --json         → stable project graph
logicn ir --debug           → unstable compiler-internal output
```
...
Suggested package name: `@logicn/ast-schema`
...
| Code | Meaning |
|---|---|
| `LLN-AST-001` | AST JSON emitted |
| `LLN-AST-002` | Requested AST schema version is unsupported |
| `LLN-AST-003` | Resolved AST unavailable due to type errors |
| `LLN-AST-004` | AST schema validation failed |
| `LLN-AST-005` | Unstable internal node requested in stable AST mode |
| `LLN-AST-006` | Language edition requires newer AST schema |
...
|---|---|
| `logicn-core` | Node kind definitions, span model, doc comment rules |
| `logicn-core-compiler` | AST generation, symbol resolution, `--syntax-only` / `--resolved` modes |
| `logicn-core-reports` | AST JSON schema, validation, emitter |
| `logicn-core-cli` | `logicn ast` command and mode flags |
| `logicn-devtools-project-graph` | Graph integration and symbol cross-references |
| `logicn-core-security` | Secret / privacy redaction rules for AST export |
| `@logicn/ast-schema` | Published schema package for consumer SDKs |
