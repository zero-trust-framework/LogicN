...
| C4 | TVCID = 3-set spatial coordinate ID `[X,Y,T]` (`1.md §3.B`, `entity.fungi`) | ✅ | Reasonable coordinate-ID design. "The ID **is** the memory address → O(1) route, no index" is the same A5 fiction; keep TVCID as a key, not a physical address. |
...
| # | Claim (source) | Verdict | Grounded correction |
|---|---|---|---|
| D1 | Three-valued authorization allow/deny/unknown, `unknown → deny` (`3.md §3`, governance .fungi) | ✅ | Real and **proved** in Galerina (`galerina-three-valued-governance.md`): Kleene K3, `collapse(0)=deny`, No-Coercion theorem. This is the genuine "tri-logic" win. |
