<!-- ABSORBED R&D SOURCE — verbatim mirror. LogicN is the main library; the R&D repo is upstream/authoring.
     Source: LogicN-R-AND-D/tri-encription/FINDINGS-AND-ROADMAP.md  ·  Pinned: R&D fb68d06 (2026-06-16)
...
> photonic-LogicN ecosystem. This is R&D scratch (`C:\wwwprojects\LogicN-R-AND-D\tri-encription\`), not a
> production repo and not the LogicN/TritMesh monorepos.
...
The question driving this work: *given the LogicN governance lessons, what can encryption actually look like
on a photonic/ternary substrate for `.tmf`, and is the ecosystem's design honest?*

The through-line across every finding is **crypto-on-core** (`LLN-SUBSTRATE-001`): cryptography must run
...
| # | Finding | Verdict | Doc |
|---|---|---|---|
| 1 | **Confidentiality is genuinely missing** — LogicN/`.tmf` sign + integrity-check but do **not** encrypt. | **ADD** KEM-DEM/HPKE: ML-KEM-768 (hybrid X25519+ML-KEM during transition) → SHAKE/HKDF → AES-256-GCM / streaming AEAD, layered **under** the TMX-256 + ML-DSA-65 gate, verify-before-decrypt, fail-closed. | [research/quantum-resilient-tri-encryption.md](research/quantum-resilient-tri-encryption.md) |
...
| 4 | **"Self-healing" under zero-trust** | **Erasure coding (Reed-Solomon) outside the gate + re-verify against the signed root, fail-closed** — never the notes' in-cache convolution (which fabricates unsigned data and is an in-gate attacker target). | [research/LLN-AMD-024-tmf-confidentiality.md](research/LLN-AMD-024-tmf-confidentiality.md) |
...
([`bench/`](bench)) over the real `@noble` PQ primitives, and a **runnable LogicN clone** of the governance
gate ([`lln/`](lln)) that type-checks and executes on the real compile→WASM path.
...
| `research/LLN-AMD-024-tmf-confidentiality.md` | The ratified `.tmf` confidentiality blueprint (container, pipeline, STREAM, erasure self-heal) | **Done; now promoted to the byte-precise oracle spec `../tmf/spec/tmf-encryption-v0.md`** |
...
| `lln/k3-gate.lln` (+ `lln/README.md`) | The K3 governance + verify-before-decrypt **decision** layer in real LogicN | **Done; `logicn check` clean, runs on WASM** |
...
LogicN ships **hashing (SHA-256/512) + signing (Ed25519 today, ML-DSA-65 planned)** and a strong key-custody
...
- **Governance/key-release (the earned home):** LogicN's strong-Kleene K3 calculus is *proven* fail-closed
...
integrity/authenticity metadata; filter at trusted endpoints.** (Resolves LLN-AMD-024 Open Question #2.)

---

## 4. Engineering validation (`bench/`) — measured

Audited pure-JS `@noble` stack (the same `@noble/post-quantum` the LogicN compiler depends on), on **Intel
...
≈ 159 / 248 MB/s. **Byte constants validated at runtime** (corroborating LLN-AMD-024): ML-KEM-768 ct = 1088 B,
...
## 5. LogicN dogfooding (`lln/`) — runs on WASM

The **K3 governance gate + verify-before-decrypt decision** is cloned in real `.lln`, **`logicn check` →
...
median). The **crypto math is honestly Blocked** in `.lln` (no byte buffers / bitwise / crypto — logicn-issues
0002/0003); it stays the engine layer. *LogicN governs; the engine computes.*

**Three dogfooding findings** (candidate `logicn-issues`, staged in `lln/logicn-gaps-candidate-issues.md`, not
...
- **Crypto-on-core (`LLN-SUBSTRATE-001`) independently re-derived** from both the photonic-hashing literature
...
- **Honest-no-overreach maintained throughout.** Flagged and rejected: "12.5M RPS / sub-0.01 ms / single
  ...
  fixed-width contradiction, "photonic SHA-256," and `logicn-substrate-mytri` (a rejected/fictional package).
- **One KB contradiction found — now corrected (2026-06-16):** `logicn-hardware-future-substrates.md`'s
  "Encryption → Photonic matrix operations" line contradicted `LLN-SUBSTRATE-001`; it now reads "Always CPU
  (deterministic core)" with a note crediting this track's verdict 2 (verified at the primary source). I did
  not edit the LogicN repo myself.
...
- **Fold the metadata amendment into LLN-AMD-024** — mark §2's attribute layer *encrypted-in-payload*, add the
  option matrix (a/b/c/d), and the cleartext-metadata-minimization note. *(Resolves Open Question #2.)*
- ✅ **DONE — `logicn-hardware-future-substrates.md` corrected** (2026-06-16): the "Encryption → Photonic"
  line is now "Always CPU (deterministic core)," crediting this track's verdict 2. Verified at the primary source.
- **File the remaining LogicN dogfooding finding** (§5) as a `logicn-issue` — GAP-1 + GAP-3 fixed (re-verified
...
- ✅ **DONE — Confidentiality layer promoted to a byte-precise oracle spec** (`../tmf/spec/tmf-encryption-v0.md`
  ...
  — closes the LLN-AMD-024 §4 key-commitment gap), 36-B AAD context (resolves Q5), STREAM nonce; adversarially
...
- ✅ **DONE — Expanded the `.lln` governance clone** (`lln/k3-policy.lln`): `allOf`/`anyOf` Kleene policy
  composition (empty → unknown, no vacuous allow) + the `authorizeRead`/`egressRedact` egress seam encoding
  verdict 5 as runnable `.lln`; `logicn check` clean, every flow `--invoke`-verified on WASM. Surfaced GAP-4
  (CLI marshals `List<Int>` as a scalar) and confirmed GAP-1/GAP-3 fixed, GAP-2 improved.
- **Hybrid-KEM packaging** in the fixed-width `.tmf` layout (LLN-AMD-024 open question).
...
2. **Apply the LLN-AMD-024 metadata amendment now**, or hold it pending the routing decision above.
3. **Where to file** the LogicN dogfooding findings and the `future-substrates:63` correction (both touch
...
- Blueprint: [`research/LLN-AMD-024-tmf-confidentiality.md`](research/LLN-AMD-024-tmf-confidentiality.md)
- Engineering: [`bench/`](bench) (`npm install && npm test && npm run bench`) · [`bench/README.md`](bench/README.md)
- LogicN clone: [`lln/k3-gate.lln`](lln/k3-gate.lln) · [`lln/README.md`](lln/README.md)
- Adjacent context (not in this folder): the companion `encryption-on-photonic-substrates.md` in the
  LogicN-TritMesh repo; the FFSM workstream (`LogicN-R-AND-D/FFSM/`, task #199) whose "sign `sha256(output)` on
