<!-- ABSORBED R&D SOURCE — verbatim mirror. LogicN is the main library; the R&D repo is upstream/authoring.
     Source: LogicN-R-AND-D/CROSSOVER-encryption-rnd.md  ·  Pinned: R&D fb68d06 (2026-06-16)
...
> encryption for the `.tmf` / photonic-LogicN ecosystem) from the **same source notes**. This document
...
> **Off-limits (unchanged):** the `C:\wwwprojects\LogicN-TritMesh` product repo and
...
|---|---|---|---|
| **`.tmf` spec track** | `C:\wwwprojects\LogicN-R-AND-D\tmf\` | Byte-precise **format** + **integrity/authenticity** + reference vectors | spec-complete (signing Blocked on a vetted lib) |
| **tri-encryption track** | `C:\wwwprojects\LogicN-R-AND-D\tri-encription\` | **Confidentiality** design + **measured benchmark** + **runnable `.lln`** governance + threat literature | done; verified 10/10 + `logicn check` clean |
...
**Adjacent R&D at the parent** (`C:\wwwprojects\LogicN-R-AND-D\`), referenced but not merged here:
`FFSM\` (governed `ffsim` quantum-sim bridge, task #199 — same attestation idiom), and
`photonic-tri-governance\` (the three-valued K3 / Direction-A governance the tri-encryption `.lln` clone
exercises).
...
- **tri-encryption `.lln`**: `logicn check k3-gate.lln` → **0 errors / 0 warnings**; K3 + verify-before-decrypt
...
- **One claim corrected by primary-source check:** the tri-encryption doc's "KB contradiction at
  `logicn-hardware-future-substrates.md:63`" was **real when written** and has **already been fixed**
...
| F3 | **Confidentiality** is genuinely missing in LogicN/`.tmf` → **add KEM-DEM/HPKE**: hybrid X25519+ML-KEM-768 → KDF → AEAD, **under** the TMX+ML-DSA gate, verify-before-decrypt, fail-closed | ✅ designed + benched | tri `LLN-AMD-024-tmf-confidentiality.md`, `bench/` |
| F4 | **Crypto-on-core** (`LLN-SUBSTRATE-001`): hashing/keying/signing stay **bit-exact on a deterministic digital core**; photonics confined to QRNG / optical-PUF / optical-LSH, outside the gate, re-verified | ✅ re-derived twice | both tracks; KB `logicn-hardware-future-substrates.md` |
...
| F6 | **Tri-logic belongs in the key-release *gate*** (proven fail-closed, `collapse(0)=deny`), **not the cipher**; the only honest "trit in the crypto" is NTRU/ML-KEM's exact **digital** ternary/small-signed polynomials | ✅ proven runnable | tri `lln/k3-gate.lln` · tmf `research/ternary-in-cryptography.md` |
| F7 | **No in-gate self-heal** — reconstruction inside the trust gate is forbidden; use **Reed-Solomon erasure on ciphertext, outside the gate**, re-verify vs. the signed root, fail-closed | ✅ both, aligned | tri `LLN-AMD-024…` · tmf `encryption-architecture.md` §5 |
...
| F10 | **No fictional perf / hardware** — "12.5M RPS / sub-0.01 ms / single clock cycle", in-cache self-heal, "store a 0 in 0 bits", "photonic SHA-256", `logicn-substrate-mytri` — all rejected | ✅ both ledgers | tmf `real-vs-aspirational-ledger.md` · tri §6 |
| F11 | **LogicN dogfooding gaps** (from the `.lln` clone): `governance` reserved word (no hint) **✅ FIXED**; `secure flow main` absent from `--invoke` **⏳ OPEN**; CLI Bool args silently mis-marshal **✅ FIXED** | ✅ reproduced; 2/3 fixed (re-verified 2026-06-16) | tri `lln/logicn-gaps-candidate-issues.md` |
...
**Entry / status**
- `C:\wwwprojects\LogicN-R-AND-D\tmf\README.md` — index of the `.tmf` track.
- `C:\wwwprojects\LogicN-R-AND-D\tmf\STATUS-AND-NEXT-STEPS.md` — decision brief (status snapshot + options + build forks).

**Research**
- `C:\wwwprojects\LogicN-R-AND-D\tmf\research\encryption-architecture.md` — the grounded zero-trust + PQC design (3 jobs, KEM-DEM, threat model, crypto-on-core).
- `C:\wwwprojects\LogicN-R-AND-D\tmf\research\real-vs-aspirational-ledger.md` — claim triage (REAL/ASPIRATIONAL/FICTIONAL).
- `C:\wwwprojects\LogicN-R-AND-D\tmf\research\ternary-in-cryptography.md` — the three "ternaries"; NTRU is the only ternary↔lattice link.
- `C:\wwwprojects\LogicN-R-AND-D\tmf\research\external-repos-analysis.md` — the 8 `C:\wwwprojects\x` repos (NVFP4 verified; photonic noisy).
- `C:\wwwprojects\LogicN-R-AND-D\tmf\research\findings-and-next-intentions.md` — narrative write-up + phased roadmap.
- `C:\wwwprojects\LogicN-R-AND-D\tmf\research\open-questions.md` — decision forks (ratified items resolved at top).

**Spec (byte-precise, verified)**
- `C:\wwwprojects\LogicN-R-AND-D\tmf\spec\tmx-256-construction-v0.md` — TMX-256 integrity core. **SHIP.**
- `C:\wwwprojects\LogicN-R-AND-D\tmf\spec\tmf-container-v0.md` — `.tmf` byte layout + fail-closed reader.
- `C:\wwwprojects\LogicN-R-AND-D\tmf\spec\nvfp4-codec-v0.md` — opt-in `Vector`-modality NVFP4 codec.
- `C:\wwwprojects\LogicN-R-AND-D\tmf\spec\signature-custody-v0.md` — hybrid sig block + key custody (impl Blocked); **TASK 2, #34-ready.**
- `C:\wwwprojects\LogicN-R-AND-D\tmf\spec\inclusion-proof-v0.md` — TMX-256 Merkle-path proof (selective disclosure; Phase 1c).
- `C:\wwwprojects\LogicN-R-AND-D\tmf\spec\tmf-encryption-v0.md` — **the v1 confidentiality layer** (promoted from `LLN-AMD-024`): 3 orthogonal selectors (`kem_profile`/`aead_suite`/`dem_mode`), SHAKE256 DEM KDF, AAD-committing AEAD, STREAM nonce, fail-closed verify-before-decrypt; resolves the AAD-binding question (Q5).
- `C:\wwwprojects\LogicN-R-AND-D\tmf\spec\_vectors\gen_tmx_vectors.py` · `gen_tmf_container.py` · `gen_nvfp4_block.py` · `gen_sig_block.py` · `gen_inclusion_proof.py` · `gen_tmf_encryption.py` (+ `*.txt` outputs) — **5+ stdlib reference generators** + golden vectors (the cross-language conformance oracle).

**Source**
- `C:\wwwprojects\LogicN-R-AND-D\tmf\notes\1.md · 2.md · 3.md` — original `.tmf` material (discussion-only; ≡ tri-encryption notes).

### tri-encryption track — `C:\wwwprojects\LogicN-R-AND-D\tri-encription\`
**Entry**
- `C:\wwwprojects\LogicN-R-AND-D\tri-encription\FINDINGS-AND-ROADMAP.md` — the track's consolidated checkpoint (5 verdicts + roadmap).

**Research**
- `C:\wwwprojects\LogicN-R-AND-D\tri-encription\research\quantum-resilient-tri-encryption.md` — confidentiality design + KEM-DEM + tri-logic in both layers.
- `C:\wwwprojects\LogicN-R-AND-D\tri-encription\research\photonic-sha256-integrity.md` — "photonic SHA-256?" verdict (no) + honest photonic integrity architecture.
- `C:\wwwprojects\LogicN-R-AND-D\tri-encription\research\metadata-confidentiality.md` — attribute-layer leakage + metadata-minimization.
- `C:\wwwprojects\LogicN-R-AND-D\tri-encription\research\LLN-AMD-024-tmf-confidentiality.md` — the `.tmf` confidentiality blueprint (container, STREAM, RS self-heal).

**Engineering (runnable, verified)**
- `C:\wwwprojects\LogicN-R-AND-D\tri-encription\bench\` — `bench.mjs`, `tmf-crypto.test.mjs`, `lib\{k3,kemdem,rs,stream,tmx}.mjs`, `README.md` (`npm test` → 10/10; `npm run bench`).
- `C:\wwwprojects\LogicN-R-AND-D\tri-encription\lln\k3-gate.lln` + `lln\README.md` — K3 governance + verify-before-decrypt, `logicn check` clean, runs on WASM.

**Source**
- `C:\wwwprojects\LogicN-R-AND-D\tri-encription\notes\1.md · 2.md · 3.md` — ≡ the `.tmf` notes (identical).

### Adjacent (not merged here)
- `C:\wwwprojects\LogicN-R-AND-D\FFSM\` — governed `ffsim` quantum-sim bridge (task #199).
- `C:\wwwprojects\LogicN-R-AND-D\photonic-tri-governance\` — three-valued K3 / Direction-A governance R&D.
- KB read for grounding: `C:\wwwprojects\LogicN\docs\Knowledge-Bases\logicn-hardware-future-substrates.md` (crypto-on-core; corrected 2026-06-16).
...
- **D-A. AEAD cipher (the one real divergence): AES-256-GCM vs Ascon-AEAD128.** *(Corrected 2026-06-16 on
  ...
  §4 and `LLN-AMD-024`); the `.tmf` spec track is **Ascon-AEAD128** (`encryption-architecture.md`). Both are
...
- Confidentiality design + validation — KEM-DEM, bench 10/10, runnable K3 `.lln`.
...
- ✅ **1d DONE** — **Confidentiality layer promoted to a byte-precise, oracle-backed spec**
  ...
  Disentangles the `LLN-AMD-024` `0x01/0x02` overload into 3 orthogonal selector bytes; pins the SHAKE256 DEM
...
- **Cleanup:** mark the KB `:63` item **resolved** in the tri-encryption docs; flag the AES/Ascon divergence
  in `FINDINGS-AND-ROADMAP.md`; close the "crypto Blocked in `.lln`" claim with a probe `.lln`.
...
Blocked piece — real signing); KEM-DEM confidentiality + RS erasure outside the gate; `.lln` governance
contract in `packages-logicn`. **The Python + `@noble` vectors become the cross-language conformance oracle.**
...
- ✅ **Expanded the `.lln` governance clone** (`lln/k3-policy.lln`: `allOf`/`anyOf` + `authorizeRead`/`egressRedact`
  egress seam, `logicn check` clean, `--invoke`-verified; surfaced GAP-4).
...
> and Q8 (where to file the 3 LogicN gaps) remain owner calls. The items below are kept for rationale.
...
8. **Where to file** the remaining LogicN dogfooding finding — `governance` reserved word **✅ FIXED** and Bool
...
"firewalls filter on cleartext meaning" (killed by inversion attacks) · `logicn-substrate-mytri` (fictional pkg).
...
4. Add a one-line probe `.lln` to **prove "crypto is Blocked in `.lln`"** (close the unverified claim).
...
`tri-encription\research\LLN-AMD-024…` §2.3/§4, and the `tmf\spec\tmf-container-v0.md` §10 v1 note; the
"crypto-Blocked-in-.lln" claim closed with `tri-encription\lln\probe-no-bitwise.lln` (LLN-PARSE-001 on `^`).
Action 5 (folder consolidation): kept both folders, this doc canonical; physical merge deferred. The 3 LogicN
dogfooding gaps are staged as filing-ready issues in `tri-encription\lln\logicn-gaps-candidate-issues.md`
...
promoted from the `LLN-AMD-024` sketch into the byte-precise, oracle-backed spec track** —
...
lift R&D-only; file the 3 LogicN gaps).*
