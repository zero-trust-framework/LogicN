<!-- ABSORBED R&D SOURCE — verbatim mirror. Galerina is the main library; the R&D repo is upstream/authoring.
     Source: Galerina-R-AND-D/tri-encription/fungi/README.md  ·  Pinned: R&D fb68d06 (2026-06-16)
...
[`../research/FUNGI-AMD-024-tmf-confidentiality.md`](../research/FUNGI-AMD-024-tmf-confidentiality.md)
...
`.fungi`, get real results, and record every gap honestly.

## The honest split: what `.fungi` can and can't do here

| Layer | Home | Why |
|---|---|---|
| **K3 governance / key-release gate** (this file) | **`.fungi`, runs on WASM** | three-valued fail-closed decision logic is exactly Galerina's domain |
| **Crypto math** (ML-KEM, ML-DSA, SHA-256/SHAKE, AES-GCM, Reed-Solomon GF(256)) | **engine layer (the TS bench)** | **Blocked in `.fungi`**: no byte buffers, no bitwise operators, no crypto primitives (galerina-issues 0002/0003). Galerina governs; the engine computes. |

## Run it

```sh
GALERINA=C:/wwwprojects/Galerina/galerina.mjs
node $GALERINA check k3-gate.fungi                       # -> 0 errors, 0 governance warnings
node $GALERINA run   k3-gate.fungi --invoke collapse 0   # -> -1   (unknown collapses to deny)
node $GALERINA run   k3-gate.fungi --invoke keyRelease 1 1 1   # -> 1   (allow)
```

## Real results (compiled to WASM and executed)

`galerina check k3-gate.fungi` → **`✅ 0 errors, 0 governance warnings`**. Then, invoking the
...
## Policy composition + egress seam (`k3-policy.fungi`)

[`k3-policy.fungi`](k3-policy.fungi) extends the clone with the Phase-3 governance pieces, as executable
...
This encodes the metadata-minimization verdict (FUNGI-AMD-024 verdict 5 = CROSSOVER F9) as runnable governance: a
...
4. 🆕 **NEW (major) — `for … in` is silently not compiled to WASM.** A `List<Int>` param + a
   ...
   [`probe-list-allof.fungi`](probe-list-allof.fungi). Workaround: `k3-policy.fungi` uses fixed-arity `allOf2/allOf3`.
