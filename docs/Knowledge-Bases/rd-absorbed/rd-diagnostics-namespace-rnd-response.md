<!-- ABSORBED R&D SOURCE — verbatim mirror. LogicN is the main library; the R&D repo is upstream/authoring.
     Source: LogicN-R-AND-D/diagnostics-namespace-rnd-response.md  ·  Pinned: R&D fb68d06 (2026-06-16)
     Integrated LogicN view: logicn-diagnostic-namespace-ownership.md  ·  Catalog: logicn-rd-absorption-catalog.md
     Rule: edit the upstream source then re-vendor; do not fork this copy (feedback-auto-import-rd-docs). -->

> **Absorbed R&D source (verbatim).** This is the archived upstream document. Curated/integrated view: `logicn-diagnostic-namespace-ownership.md`. See `logicn-rd-absorption-catalog.md` for the full ledger. Internal links below point at the upstream R&D tree.

---
# Diagnostic-Namespace Ownership — R&D response (3 test-harness patches)

**Status:** R&D response (2026-06-16). **Subject:** `LogicN/docs/Knowledge-Bases/logicn-diagnostic-namespace-ownership.md`
(the checked-invariant KB) + its harness `logicn-core-compiler/tests/diagnostic-namespace.test.mjs`.
**Boundary:** the LogicN repo is **off-limits to edit** from this R&D session — this doc is a *recommendation*
...
`LLN-DP-*` / `LLN-PRIVACY-*` codes that must register cleanly under exactly this invariant.
...
**Gap.** The invariant asserts **Source ──▶ Registry** (every emitted `LLN-*` is registered). It does **not**
assert **Registry ──▶ Source**. So if a refactor deletes the code that emitted `LLN-SECRET-005`, the registry
entry becomes **ghost documentation** — a documented code nothing emits, silently rotting.

**Fix.** Add the reverse assertion. Every `LLN-*` in `compiler-diagnostics.md` must either be **found emitted in
...
const emitted   = scanSourceForCodes();          // set of LLN-* in `code: "LLN-..."` literals
...
**Gap.** The scan relies on `code: "LLN-..."` literals (AST/regex). It is blind to dynamic construction:
```js
const errorCode = `LLN-PRIVACY-00${level}`;   // invisible to the conformance scan -> unregisterable, untrackable
```
...
interpolation that produces an `LLN-*` prefix is a **fatal compiler error** (itself a registered diagnostic,
e.g. `LLN-META-001` *type-check/lint*). The AST check:
```js
// lint pass: forbid non-literal LLN-* code construction
// FAIL on: TemplateLiteral / BinaryExpression('+') whose (static prefix) starts with "LLN-"
...
if (node.type !== 'Literal' && reachesDiagnosticEmitter(node) && startsWithLLN(staticPrefixOf(node)))
  fatal('LLN-META-001', 'diagnostic code must be a static string literal; dynamic LLN-* construction is forbidden');
```
...
**Gap.** `compiler-diagnostics.md` is canonical; `logicn-governance-rules.md` is the governance subset. The same
...
const gov = parseGovernanceRules();                                     // logicn-governance-rules.md
for (const e of registered.values())
...
```
**Better (eliminate the human sync entirely):** **generate** `logicn-governance-rules.md` from the canonical
...
The new `LLN-DP-*` / `LLN-PRIVACY-*` codes (privacy-governance §3) are introduced **with mechanism tags**
...
2. Static-literal lint: a `\`LLN-...${x}\`` anywhere on the emit path is a fatal compile error.
...
## Sources / refs
`logicn-diagnostic-namespace-ownership.md` (the reviewed KB), `logicn-design-stability-and-forward-planning.md`
§5, `logicn-governance-rules.md`, `compiler-diagnostics.md` (all in `LogicN/docs/Knowledge-Bases/`, read-only
