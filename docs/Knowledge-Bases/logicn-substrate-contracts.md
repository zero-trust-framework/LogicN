...
`logicn-substrate-failure-model.md` §8.
**Parent agenda:** `logicn-photonic-tri-substrate-rd-agenda.md` §5 (Direction B); follow-up list
`logicn-substrate-failure-model.md` §8.
**Provenance.** Direction A made the governance verdict three-valued and proved it cannot fail open
(`logicn-three-valued-governance.md`). Direction C shipped the *noise math* and the
`LLN-SUBSTRATE-001..004` diagnostic family in `logicn-tower-citizen/src/substrate-model.ts` — a
...
*forbidden*, not re-engineered. No `.tmf` / TritMesh coupling. LogicN stays TS-like flow+contract — no
...
but only if someone hands it parameters. Today nothing does: a `.lln` author has **no way to declare**
...
## 2. Prior art / mapping onto LogicN
...
- **Kleene-K3 fail-closed composition** — Direction A's `vAnd` + No-Coercion; the safety half of B3
  ...
  *not* re-proved here.
- **Static manifest clamping** (`logicn-domain-guard-policies.md`) — the prior art for "a contract
...
if ((tok.kind === "keyword" || tok.kind === "identifier") && tok.value === "substrate") {
  if (seenBlocks.has("substrate")) {
    this.emit("LLN-SYNTAX-009", "DUPLICATE_CONTRACT_SECTION",
...
(`LLN-SYNTAX-009`, defined in `compiler-diagnostics.md:369`, currently un-enforced) is opportunistically
...
`access`, `@`-attributes). Reference emit/continue templates: `LLN-SYNTAX-LEGACY-001` (`parser.ts:646–654`),
`LLN-SYNTAX-LEGACY-002` (`parser.ts:441–476`).
...
`substrate-model.ts:273–278` (`SUBSTRATE_DIAGNOSTICS`) and `compiler-diagnostics.md` §`LLN-SUBSTRATE-*`.
...
|---|---|---|---|---|
| **B1** | **crypto-on-core.** Integrity is never tolerance-bounded. | flow has a Hash/Sign/crypto effect **and** `lane` is `photonic`/`noisy` | `LLN-SUBSTRATE-001` · `CRYPTO_ON_NOISY_LANE` | **error** (always, every profile) |
| **B2** | **redundancy sufficiency.** Declared tolerance must be provable at the declared `N`. | `epsilonModeled > tolerance` at the declared `redundancy` | `LLN-SUBSTRATE-002` (raisable by more `N`) / `LLN-SUBSTRATE-003` (`N>1` still short, **or** `pBad ≥ 0.5` so voting cannot converge) | `002`: **error** in `production`/`deterministic`, **warning** in `dev`. `003`: **error** (always) |
| **B3** | **determinism preservation.** An un-voted noisy result must not feed a deterministic context. | `lane` noisy **and** `redundancy == 1` **and** sink requires determinism | `LLN-SUBSTRATE-004` · `UNVOTED_ANALOG_INTO_DETERMINISTIC` | **error** (always) |

### 4.1 B1 — crypto-on-core (`LLN-SUBSTRATE-001`)
...
### 4.2 B2 — redundancy sufficiency (`LLN-SUBSTRATE-002` / `-003`)
...
- `check.met` ⇒ pass.
- `!met && !check.redundancyHelps` (i.e. `pBad ≥ 0.5`) ⇒ `LLN-SUBSTRATE-003` **error** — voting
  fundamentally does not converge; raising `N` is futile. Fix: reduce noise / move to digital.
- `!met && redundancyN > 1` ⇒ `LLN-SUBSTRATE-003` **error** — declared `N` is short of what the model
  needs. Fix: raise `N` (the `check.trace` over `N=1,3,5,7` shows the smallest sufficient `N`).
- `!met && redundancyN == 1` ⇒ `LLN-SUBSTRATE-002` — tolerance unmet with no voting; raising `N`
...
### 4.3 B3 — determinism preservation (`LLN-SUBSTRATE-004`)
...
if (laneIsNoisy && isUnvoted && sinkRequiresDeterminism) { /* LLN-SUBSTRATE-004, error */ }
```
...
A new sibling inference module **`logicn-core-compiler/src/substrate-inference.ts`**, mirroring
...
(`LLN-INV-001/002`, ~`governance-verifier.ts:1820–1827`) and **before** the trap declarations
(`LLN-TRAP-001/002`, ~`1829–1832`) — the same neighbourhood where `checkResilienceViolations` /
...
unrecognised `lane` keyword is itself a diagnostic (`LLN-SUBSTRATE-002`/`-003` family or a syntax-level
...
**Verified fact (re-read in the live tree, not assumed):** `logicn-core-compiler`'s `package.json`
depends only on `@logicn/devtools-graph-algorithms` and `@noble/post-quantum` (plus `argon2`,
`bcryptjs`, `wabt`, `wat-wasm`). It does **not** depend on `logicn-tower-citizen`, and `tower-citizen`
depends only on `@logicn/inference-bridge-contract`. `governance-verifier.ts` (3263 lines) contains
...
been executed: a new **zero-dependency workspace package `@logicn/substrate-math`** (`packages-logicn/
logicn-substrate-math`) now holds the pure compute as the **single source of truth**:
...
- Both consumers depend on it (`file:../logicn-substrate-math`): the compiler's `substrate-math.ts` is now
...
- `NMR_GOLDEN_VALUES` — `(pBad, N, expectedEpsilon)` tuples pinning `nmrFailureProbability`. Anchors,
  hand-verified against `logicn-substrate-failure-model.md` §3.3: `nmr(0,N)=0` ∀N; `nmr(1,N)=1` ∀N;
...
`logicn-tower-citizen/tests/substrate-model.test.mjs` and `logicn-core-compiler/tests/substrate-contracts.test.mjs`.
...
runtime cost. Extracting `@logicn/substrate-math` (§6 follow-up) would let both suites import one fixture;
...
```js
it("LLN-SUBSTRATE-001: crypto effect on a photonic lane is denied (every profile)", () => {
...
`, "production");
  assert.ok(hasDiag(result, "LLN-SUBSTRATE-001"));
  const d = result.diagnostics.find(x => x.code === "LLN-SUBSTRATE-001");
  assert.equal(d.severity, "error");                 // never tolerated, even with redundancy: 3
  assert.match(d.message, /digital lane|integrity/i);
});
it("LLN-SUBSTRATE-001 still fires in dev (integrity is profile-independent)", () => {
  const result = parseAndVerify(/* same source */, "dev");
  assert.equal(result.diagnostics.find(x => x.code === "LLN-SUBSTRATE-001").severity, "error");
});
...
```js
it("LLN-SUBSTRATE-002: tight tolerance at N=1 is rejected; redundancy: tmr clears it", () => {
  const tight = `
...
{ return mean(xs) }`;
  ...
  assert.ok(hasDiag(r1, "LLN-SUBSTRATE-002"));
  assert.equal(r1.diagnostics.find(x => x.code === "LLN-SUBSTRATE-002").severity, "error");

  const voted = tight.replace("redundancy: 1", "redundancy: tmr");   // tmr → N=3 (consensusTrit)
  const r2 = parseAndVerify(voted, "production");
  assert.ok(!hasDiag(r2, "LLN-SUBSTRATE-002"));    // monotone: raising N clears it (substrate-model.ts:222–236)
  assert.ok(!hasDiag(r2, "LLN-SUBSTRATE-003"));
});
it("LLN-SUBSTRATE-002 is a warning in dev, error in production", () => {
...
contract { substrate { lane: photonic; tolerance: 1e-6; redundancy: 1 } } { return mean(xs) }`;
  assert.equal(parseAndVerify(src, "dev").diagnostics
    .find(x => x.code === "LLN-SUBSTRATE-002").severity, "warning");
  assert.equal(parseAndVerify(src, "production").diagnostics
    .find(x => x.code === "LLN-SUBSTRATE-002").severity, "error");
});
it("LLN-SUBSTRATE-003: when pBad ≥ 0.5 voting cannot converge — error, no false promise", () => {
  ...
  assert.ok(hasDiag(r, "LLN-SUBSTRATE-003"));
  assert.ok(!hasDiag(r, "LLN-SUBSTRATE-002"));
});
```
...
```js
it("LLN-SUBSTRATE-004: un-voted noisy result into a deterministic sink is rejected", () => {
...
`, "deterministic");
  assert.ok(hasDiag(r, "LLN-SUBSTRATE-004"));
  assert.equal(r.diagnostics.find(x => x.code === "LLN-SUBSTRATE-004").severity, "error");
});
...
{ return analogScore(req) }
`, "deterministic");
  assert.ok(!hasDiag(r, "LLN-SUBSTRATE-004"));      // a vote restores determinism at the boundary
});
```
...
```js
it("a flow with NO substrate block emits zero LLN-SUBSTRATE-* diagnostics", () => {
  const r = parseAndVerify(`
secure flow createOrder(req: Request) -> Result<Response, ApiError>
contract { effects { database.write audit.write crypto.sign } }
{ return Ok(Response.ok({})) }
`, "production");
  for (const code of ["LLN-SUBSTRATE-001","LLN-SUBSTRATE-002","LLN-SUBSTRATE-003","LLN-SUBSTRATE-004"])
...
{ return x }
`, "production");
  assert.equal(r.diagnostics.filter(d => d.code.startsWith("LLN-SUBSTRATE-")).length, 0);
});
```

Plus the **oracle** tests (§6.1) in both suites and a **constant-identity** check
(`assert.equal(SUBSTRATE_DIAGNOSTICS.CRYPTO_ON_NOISY_LANE, "LLN-SUBSTRATE-001")`, mirroring the
`LLN_RES_001.code` style at `governance-verifier.test.mjs`). The full package suite + the graph check
...
compiler the four codes are registered as exported `LLN_SUBSTRATE_001..004` const objects in
`governance-verifier.ts` (the diagnostic-constant section, alongside `LLN_RES_001`/`LLN_OBS_001`), and
...
|---|---|
| `tolerance:` non-numeric / `< 0` / `> 1` | reject — emit a parse/`LLN-SUBSTRATE-002`-family diagnostic; never coerce to default |
...
| `lane:` unrecognised keyword | `malformed` → `LLN-SUBSTRATE-002` error; do **not** silently treat as `digital` |
| `tolerance:` split/incomplete numeric (`1e-`) | `malformed` → `LLN-SUBSTRATE-002` error; **never** truncate to the leading digit |
...
which `checkSubstrateViolations` reports as `LLN-SUBSTRATE-002` (error) — it does **not** mint a new code
...
the LogicN **lexer did not tokenize scientific notation** — `1e-6` lexed as `[1, e, -, 6]`, which a naive
...
follows, so `1e-6` is **one** numeric token everywhere in LogicN. This is a small, guarded change (a
...
or incomplete numeric (`1e-`, `1 e - 6`) fails closed as `LLN-SUBSTRATE-002`, never a silent default.
...
- ✅ **DONE: `@logicn/substrate-math` package extraction** (§6) — the NMR compute is now a shared
...
3. ~~`@logicn/substrate-math` package now, or copy-with-oracle for the spike?~~ **RESOLVED (§6): copy +
...
`logicn-substrate-failure-model.md` (Direction C — the model, the `LLN-SUBSTRATE-*` registration, §8
follow-ups that scope this doc) · `logicn-three-valued-governance.md` (Direction A — `Verdict`, `vAnd`,
...
(constant export block) · `compiler-diagnostics.md` (`LLN-SUBSTRATE-*`, `LLN-SYNTAX-009`) ·
`logicn-domain-guard-policies.md` (static-manifest-clamping prior art) ·
`logicn-photonic-tri-substrate-rd-agenda.md` §5 (parent agenda, `#58`).
