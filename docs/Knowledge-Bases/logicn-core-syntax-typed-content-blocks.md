...
**Status:** Stage 1 — parser state tracking live in compiler; `LLN-BLOCK-001..004` constants defined; `validateTypedContentBlock()` stub in place  
...
LogicN provides four typed content block forms for embedding structured content inside flows:
...
`script` blocks are subject to strict content scanning. `ProtectedSecret` values must not be interpolated into script blocks (see `LLN-BLOCK-004`).
...
The `{` and `}` characters in CSS, DOM, HTML, and script content do **not** affect LogicN brace depth tracking — the compiler suspends scope counting while inside a content block.
...
LogicN tracks brace depth to determine flow scope exits. Content blocks suspend this tracking:

```logicn
flow renderTable() -> Html {
  html <<TABLE
    <table>
      <tr class="{ active }">  <!-- these braces are NOT LogicN scope braces -->
        <td>Row</td>
      </tr>
    </table>
  TABLE
...
The `{` and `}` inside the `TABLE` block are invisible to the LogicN brace counter.
...
- **`ProtectedSecret` values** must not be interpolated into `script` or `html` blocks — use `LLN-BLOCK-004` check
...
### Content block diagnostics (LLN-BLOCK-*)

| Code | Name | Trigger |
|---|---|---|
| `LLN-BLOCK-001` | `UNKNOWN_CONTENT_BLOCK_TYPE` | Block type is not `html`, `dom`, `script`, or `css` |
| `LLN-BLOCK-002` | `UNCLOSED_CONTENT_BLOCK` | Opening marker found but closing marker never appears |
| `LLN-BLOCK-003` | `MISMATCHED_CONTENT_BLOCK_MARKER` | Closing marker does not match the opening marker |
| `LLN-BLOCK-004` | `SECRET_IN_CONTENT_BLOCK` | `ProtectedSecret` interpolated into a `script` or `html` block |

---

## 9. Type Contracts (`@logicn/core`)
...
## 10. Compiler API (`@logicn/core-compiler`)
...
- If end-of-file is reached while still inside a block, `LLN-BLOCK-002` is emitted

Unknown block types (e.g. `xml <<XML`) immediately emit `LLN-BLOCK-001` without entering block mode.
...
| Area | Status | Notes |
|---|---|---|
| `ContentBlockType`, `CONTENT_BLOCK_TYPES` | ✅ | `@logicn/core/src/index.ts` |
| `TypedContentBlockExpression` interface | ✅ | `@logicn/core/src/index.ts` |
| `typedContentBlockExpr` AstNodeKind | ✅ | `@logicn/core/src/index.ts` |
| `LLN-BLOCK-001..004` constants | ✅ | `@logicn/core-compiler` |
| `LLN_BLOCK_DIAGNOSTICS` array | ✅ | `@logicn/core-compiler` |
| Content block state tracking in `validateCoreSyntaxSafety()` | ✅ | Real check; brace depth suspended inside blocks |
| `LLN-BLOCK-001` unknown type detection | ✅ | `parseContentBlockOpen()` |
| `LLN-BLOCK-002` unclosed block detection | ✅ | Post-loop check |
...
| `LLN-BLOCK-003` mismatched marker (Stage 2) | ⏳ | Blocked on multi-block nesting parser |
| `LLN-BLOCK-004` secret in script block (Stage 2) | ⏳ | Blocked on interpolation AST |
