...
2. **Tri-Pipe Substrate Routing (physics-aware scheduling).** A deterministic routing function `R(o)` assigns each
   ...
   `FUNGI-SUBSTRATE-001` compile error before `.wasm` is generated.
...
| §3 crypto invariant + contamination | `substrate-inference.ts`, `substrate-math.ts`, `FUNGI-SUBSTRATE-001..004`, `verifySubstrate` (governance-verifier) | mostly **BUILT**; confirm the `S_P→S_B` key-contamination case is explicitly a hard error |
...
R&D-0036 `ifStmt` elision at wat-emitter.ts:1623-1651), static-truth proving (`tryStaticEval` → FUNGI-INV-001), the
...
- **Emission:** `(unreachable) ;; FUNGI-GDCE-001 TRAP→ERASE` (REUSE) + a non-silent verifier diagnostic **`FUNGI-GDCE-001`** (analogous to FUNGI-INV-001). Both tiers take the same K3 branch → preserves 0014 parity.
...
(FUNGI-SUBSTRATE-001) is **effect-declaration-level, not value-level taint** — it doesn't track a photonic *value*
...
1. **§2 Governance DCE pass** (`FUNGI-GDCE-001`) — the headline net-new.
