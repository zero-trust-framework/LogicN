...
- The Stage B self-hosted compiler is a LogicN application — it needs its own
...
Stage B introduces a LogicN compiler. Without explicit isolation, user code
...
build is compromised. LogicN must close this gap structurally, not by policy
convention alone.
...
The compiler must declare its own capabilities as a governed LogicN application.
...
LogicN Stage B defines five authority domains. No domain inherits authority from
...
## Compiler Manifest in LogicN

If the language is governance-first, the compiler must be the first program that
lives under its own rules. The compiler declares its capabilities in LogicN
...
contract {
  intent {
    purpose "Transform LogicN source into Governed IR artefacts"
    domain  compiler
    actor   COMPILER
  }
...
COMPILER capability -> CompilerHost internal operation only
  Example: filesystem.read used to open a .lln source file
...
See `logicn-deterministic-selfhost-verification.md` for the full verification
protocol.
...
event sequence number anchored to the LLN-Graph EventDAG.
...
The compiler declares its own capabilities in LogicN source — using the same
contract syntax as any other governed LogicN program. This is the first
self-governing artefact in the LogicN toolchain. If the compiler cannot declare
...
4. The compiler declares its own capabilities in LogicN source.
...
logicn-semantic-graph-system.md                  — LLN-Graph EventDAG
logicn-deterministic-selfhost-verification.md    — verification protocol
logicn-governed-memory-blocks.md                 — governed object identity
