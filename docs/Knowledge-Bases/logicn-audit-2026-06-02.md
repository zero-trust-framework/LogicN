...
|---|---|---|---|
| logicn-core-compiler | 2,810 | 2,810 | 0 |
...
| 26B | ImmutableInputSeal, HardwareGovernanceClass, ProofLevel, LLN-HW-001/002/003 | ✅ |
...
| 29 | @logicn/core-economics — CostGraph, ValueGraph, IBM matrix, RouteDecision | ✅ |
...
| 34 | verifyPasswordService.lln — live governed HTTP service, POST /auth/verify | ✅ **Runtime=25%** |
...
- Phase 41 syntax additions documented in `logicn-grammar.ebnf`, `logicn-spec-manifest.yaml`, `AI_INDEX.md`
...
| `else if` → LLN-SYNTAX-010 hard error | ✅ implemented + documented |
...
8 Stage B self-hosted files (all in `packages-logicn/logicn-core-compiler/src/stage-b/` or equivalent):
...
- Phase 55: ML-DSA-65 (NIST FIPS 204) hybrid signing — ⏳ planned
  ...
  - Diagnostics: LLN-HW-101..104 (MissingRequiredAttestation, UnsupportedAttestationAlg, HybridAttestationIncomplete, AttestationEvidenceStale)

---

## Runtime in LogicN Progress
...
| Route dispatcher in LogicN | 54 | **50%** 🎯 | ⏳ Planned |
...
| File | Changes |
|---|---|
| `logicn-glossary.md` | Added `when` guard arm, `match`, `inline contract`, `with effects` (REMOVED) entries; fixed `fn`/`flow` examples to use v1 syntax |
| `compiler-diagnostics.md` | Added LLN-SYNTAX-006..010, LLN-SYNTAX-LEGACY-001/002, LLN-STYLE-001/002, LLN-TAINT-001..007, LLN-PROFILE-001..007 (incl. 005B), LLN-HW-001..003/101..104 |
| `logicn-roadmap-next10-phases.md` | Updated test counts to 2,952; Phase 41 row corrected in progress table; Phase 41 entry includes LLN-SYNTAX-010 note |
| `logicn-audit-2026-06-02.md` | This file — successor to 2026-06-01 audit |

Files verified as current (no changes needed):

| File | Verdict |
|---|---|
| `AI_INDEX.md` | ✅ Current — Phase 41 syntax, ML-DSA, LLN-SYNTAX-010, `when` guard arms all documented |
| `logicn-spec-manifest.yaml` | ✅ Current — grammar_version v1.1, all Phase 41 additions listed |
| `logicn-grammar.ebnf` | ✅ Current — `when_guard_arm`, integer/string literal arms, `else if` hard error (LLN-SYNTAX-010), `:` return type, inline contract all present |
