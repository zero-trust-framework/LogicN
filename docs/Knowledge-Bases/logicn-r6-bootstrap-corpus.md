...
execute** entirely in LogicN (Stage B) AND produce **full parity** with the Stage-A
TypeScript reference, Runtime-in-LogicN is complete for the supported subset.

Files: `packages-logicn/logicn-core-compiler/tests/r6-corpus/r6-00N-*.lln` (each carries
...
|---|---|---|---|
| **R6-001** `classify` | `r6-001-classify.lln` | **R1** | String literals; string-returning `Result<String,String>` (Ok/Err); `contract.types` alias (`Verdict`); Int param; `if`/comparison/`return` |
| **R6-002** `distanceSq` | `r6-002-distance.lln` | **R2** | `record` declaration; record **value** as a param; **field access** (`p.x`); arithmetic over fields |
| **R6-003** `listLen` | `r6-003-listlen.lln` | **R2** | `Array<Int>` **array literal** `[1,2,3,4]`; list method call `.count()`; typed `let` |
| **R6-004** `recordAmount` | `r6-004-record-amount.lln` | **R4** | `secure` flow; `effects { audit.write }` block; **member call** `AuditLog.write(...)`; `Result<Int,String>` return |
| **R6-005** `nameOf` | `r6-005-name-of.lln` | **R5** | `match` over integer patterns + `_`; **`Option<String>`** (`Some`/`None`); String payloads |
...
- **Record literals don't unify with a nominal `record` type** in Stage A (`{x,y}` types as
  generic `Record`, not `Point` → LLN-TYPE-002/008). The corpus therefore passes a record
...
widening of the Stage-B pipeline (see `logicn-selfhosting-roadmap-axisB.md` → "Path to 100%").
