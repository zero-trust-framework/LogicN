...
Phase 9B: self-hosting foundations — lexer and parser written in LogicN
```

## TL;DR
- Phase 9A completes the TypeScript runtime (async interpreter, full type inference, stdlib finalization)
- Phase 9B proves self-hosting by rewriting the lexer and parser in LogicN
...
**Goal:** LLN-TYPE-003 (branded type enforcement) + complete expression propagation

**Branded type enforcement:**
- Track `type Email = Brand<String, "EmailAddress">` in collectDeclarations
- In assignment checking: `let email: Email = rawString` → LLN-TYPE-003
...
- LLN-GOV-003 (protected data without authority) — requires expression type inference
- LLN-GOV-005 (policy purpose mismatch) — requires intent/behavior analysis
...
**Goal:** Prove LogicN can describe its own syntax by rewriting lexer and parser in LogicN.

**Decision (recorded):** Stage B targets lexer + parser first, not the entire compiler.

### Stage B1: Lexer in LogicN

Rewrite `src/lexer.ts` as LogicN source.
...
**Success criterion:** `lex(source, file)` in LogicN produces identical tokens to the TypeScript lexer for the same input.

### Stage B2: Parser in LogicN

Rewrite `src/parser.ts` as LogicN source.

More complex than the lexer but bounded:
- Takes `Array<Token>`, returns `AstNode`
- No network, no I/O
- Well-specified (grammar in `logicn-grammar.ebnf`)

**Success criterion:** Same `.lln` input → Stage A AST === Stage B AST.
...
2. Stage B (LogicN): parse same examples, record ASTs
3. Compare: every AST must match exactly

### Stage B4: Downstream (later)

After lexer + parser self-host:
- Type checker in LogicN
- Effect checker in LogicN
...
- LLN-TYPE-003 branded type enforcement implemented
...
- LogicN lexer produces identical tokens to TypeScript lexer
- LogicN parser produces identical AST to TypeScript parser
...
- `docs/Knowledge-Bases/logicn-adaptive-runtime-profiles.md` — async runtime model
- `docs/Knowledge-Bases/logicn-intent-guided-optimisation.md` — IGO runtime concept
- `docs/Knowledge-Bases/logicn-language-lessons.md` — lessons from other languages
- `docs/Knowledge-Bases/logicn-type-improvements-phase-8.md` — LLN-TYPE-003 spec
