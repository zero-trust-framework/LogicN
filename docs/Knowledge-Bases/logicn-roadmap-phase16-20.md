...
LogicN compiler output. Its purpose is to enable the `logicn verify-selfhost`
...
### logicn verify-selfhost (Full Implementation)

The `logicn verify-selfhost` command provides a formal proof that the compiler
...
4. On mismatch → emit diagnostic `LLN-BUILD-001` with a diff of the diverging
   output sections
...
- 1720+ tests passing
- `logicn verify-selfhost` passes against the compiler source tree (B1 == B2)
...
LogicN currently resolves names within a single source file. Phase 17 extends
resolution to the package level.

- `package.logicn.yaml` files are scanned from disk at compile time
- `import Email from "@logicn/healthcare-types"` resolves by looking up the
  manifest for `@logicn/healthcare-types` and locating the `Email` export
- Cross-module type sharing: a type declared in one package is usable in
  another with full type-checker support
- `LLN-NAME-003`: cross-module shadow detection — a local name that shadows an
...
## Phase 18 — type-checker.lln (Stage B Milestone 4)

### Writing the Type Checker in LogicN

Phase 18 is the fourth Stage B milestone: the LogicN type checker is written
in LogicN.

Implementation path:

- `type-checker.lln` v0 consumes the token stream and AST produced by
  `lexer.lln` and `parser.lln`
...
- The output of `type-checker.lln` is compared against the TypeScript reference
  implementation on the same corpus; any divergence is a bug in the LogicN
  implementation

This milestone proves that the LogicN type system is expressive enough to
describe itself.
...
- `LLN-PARSE-DEPTH-001` — new diagnostic emitted when nesting exceeds the
  declared limit

### Language Server Protocol (LSP) Skeleton

The `logicn lsp` CLI command starts a JSON-RPC server implementing a minimal
subset of LSP:

| LSP capability | Description |
|---|---|
| `textDocument/hover` | Returns type and effect information for the token under the cursor |
| `textDocument/diagnostics` | Streams all LLN-* diagnostics for the open file |
...
|---|---|
| `lexer.lln` full parity | All token types produced by the TypeScript lexer are also produced by `lexer.lln` on the same input |
| `parser.lln` completeness | Handles expressions, statements, and match arms — not just flow headers |
| `type-checker.lln` completeness | Handles all LLN-TYPE-001 through LLN-TYPE-022 error codes |
| `logicn build src/ --self-hosted` | The TypeScript bootstrap layer is not used; the LogicN compiler compiles itself end-to-end |
| `verify-selfhost PASS` | Three independent builds B1, B2, B3 all produce the same canonical hash |

### What Stage B Completion Means

When Stage B is complete, LogicN is self-hosted: the language proves its own
...
remains the primary runtime for users until the LogicN-native runtime has
...
- Expand `package.logicn.yaml` registry to support versioned package graphs
- Add `logicn-compliance` reporting layer (enterprise, post-v1)
- Stable benchmarks comparing TypeScript runtime vs LogicN-native runtime
...
- Tensor shape and dtype declared in LogicN source; the backend validates
...
Phase 23 maps LogicN's software capability model onto CHERI hardware
capabilities.
...
- The LogicN compiler emits CHERI-annotated lowering IR when targeting a CHERI
  platform
- `LLN-CHERI-001`: diagnostic emitted when a LogicN source pattern cannot be
...
- `logicn verify-attestation --pq` verifies ML-DSA signatures
