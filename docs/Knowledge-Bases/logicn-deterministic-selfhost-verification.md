...
Three specific failure modes matter for LogicN:
...
**Governance violation propagation.** LogicN's governance model — effect checking, capability enforcement, boundary verification — is enforced by the compiler. If the compiler itself has a governance bug, it may emit output that would fail its own rules, and then compile that output without noticing, because the checker has the same bug.
...
Stage A Compiler (bootstrap, written in JS/TS)
    |
    | compiles LogicN compiler source
    v
Build B1
    |
    | compiles LogicN compiler source (same source, same config)
    v
Build B2
    |
    | compiles LogicN compiler source (same source, same config)
...
**B1** is the first self-hosted build. It is produced by Stage A compiling the LogicN compiler source.

**B2** is produced by B1 compiling the same LogicN compiler source with the same configuration.

**B3** is produced by B2 compiling the same LogicN compiler source with the same configuration.
...
The verification procedure extends the `ExecutionProofChain` from `LLN-Graph` with compiler-specific fields. Each build produces a proof record:
...
When `logicn verify-selfhost` runs, it produces three `CompilerBuildProof` records (one per build) and asserts that all hash fields match across the three records. The comparison result is written to the governance log.
...
## LLN-BUILD-001: NON_DETERMINISTIC_BUILD
...
**Not triggered by:** Builds that differ because source files differ, configuration differs, or the compiler version differs. `LLN-BUILD-001` is specifically about same-input → different-output.
...
1. Build B1
   ...
   - Compile LogicN compiler source at HEAD
   - Record CompilerBuildProof for B1

2. Build B2
   - Use B1 as the compiler
   - Compile the same LogicN compiler source (same commit, same config)
...
3. Compare B1 and B2
   ...
   - If any assertion fails: emit LLN-BUILD-001, stop

4. Build B3
   - Use B2 as the compiler
   - Compile the same LogicN compiler source
...
- `1` — `LLN-BUILD-001` triggered, proof not issued
...
- Implement `logicn verify-selfhost` command with the pipeline described above
- Implement `LLN-BUILD-001` diagnostic
...
The self-hosted compiler's output is only deterministic if the compiler's execution environment is also deterministic. The `CompilerRootBoundary` (see `logicn-stage-b-root-capability-provider`) must provide identical authority on every run:
...
If the root capability provider supplies different authority on different runs, the compiler may make different decisions, and the output will diverge. `LLN-BUILD-001` will fire, but the root cause will be in the capability provider, not in the compiler logic itself.
...
6. `LLN-BUILD-001` fires when same-input → different-output is detected.
...
- `logicn-stage-b-root-capability-provider` — root capability ownership and authority model
- `logicn-compiler-phase-memory-boundaries` — phase arena model (memory determinism per phase)
- `logicn-proof-chain-spec` — ExecutionProofChain specification (`proof-chain`)
- `logicn-roadmap` — stage sequencing and priorities
