...
## TL;DR
- Branded types must be nominal — `let id: CustomerId = rawString` must emit LLN-TYPE-003
...
## 1. Branded Type Enforcement (LLN-TYPE-003)
...
### Invalid (must emit LLN-TYPE-003)

```logicn
let id: CustomerId = rawString    // LLN-TYPE-003: use validate.customerId()
let email: Email = someString     // LLN-TYPE-003: use validate.email()
```
...
- Emit LLN-TYPE-003 with suggestedFix pointing to validate gate
...
- Emit LLN-MATCH-003 when a pattern variant doesn't belong to the subject enum
...
- In `moneyBinary()`, compare currency tags — emit LLN-TYPE-004 if they differ
