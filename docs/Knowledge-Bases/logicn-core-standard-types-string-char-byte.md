...
**Scope:** `String`, `Char`, `Byte`, `Bytes`, `SecureString` — safe use in LogicN source, compiler contracts, runtime checks, and reports.  
**Source:** NOTES TO COVER / b.txt (2026-05-26)  
**Related KB:** `arrays-and-string-operations.md`, `logicn-core-security-secret-reference-model.md`

---

## 1. Why These Types Exist

LogicN treats text and raw binary data as separate concepts.
...
Many security bugs come from mixing text, bytes, encodings, secrets, file contents, network packets, and user input without clear boundaries. LogicN makes those boundaries visible and compiler-enforced.
...
`String` is LogicN's standard text type — valid Unicode, immutable by default.
...
|---|---|---|
| `LLN-STRING-001` | `INVALID_UTF8_DECODE` | Attempted decode produced invalid UTF-8 |
| `LLN-STRING-002` | `SECRET_STORED_AS_STRING` | Secret value assigned to plain `String` |
| `LLN-STRING-003` | `IMPLICIT_STRING_BYTE_CONVERSION` | `Bytes` assigned to `String` without explicit decode |
| `LLN-STRING-004` | `AMBIGUOUS_STRING_LENGTH` | `.length` used without specifying chars vs bytes |
...
let text: String = raw
// ^ LLN-STRING-003: Bytes cannot become String without explicit decode.
```
...
let wrong: Byte = c
// ^ LLN-CHAR-001: Char cannot be assigned to Byte.
...
`charLiteral` is an `AstNodeKind` in `@logicn/core` — single-quoted character literals parse to this node kind.

### Compiler Diagnostics

| Code | Name | Description |
|---|---|---|
| `LLN-CHAR-001` | `CHAR_BYTE_CONFUSION` | `Char` assigned to or compared with `Byte` without conversion |
| `LLN-CHAR-002` | `INVALID_CHAR_LITERAL` | Character literal contains an invalid Unicode scalar value |
| `LLN-CHAR-003` | `MULTI_CHAR_LITERAL` | Character literal contains more than one character unit |
...
```logicn
let c: Char = 'AB'
// ^ LLN-CHAR-003: Char literal must contain exactly one character unit.
```
...
`Byte` is LogicN's standard unsigned 8-bit raw value. Its primary purpose is binary data, not general arithmetic.
...
`byteLiteral` is an `AstNodeKind` in `@logicn/core` — hex or decimal byte literals (e.g. `0xFF`, `255`) parse to this node kind when the declared type is `Byte`.
...
| Code | Name | Description |
|---|---|---|
| `LLN-BYTE-001` | `BYTE_OUT_OF_RANGE` | Byte literal value is outside 0–255 |
| `LLN-BYTE-002` | `BYTE_OVERFLOW` | Byte arithmetic result exceeds 255 without explicit handling |
| `LLN-BYTE-003` | `IMPLICIT_BYTE_STRING_CONVERSION` | `Bytes` assigned to `String` without explicit decode |
| `LLN-BYTE-004` | `RAW_BYTES_LOGGED` | Raw `Bytes` value passed to a log sink without redaction |
| `LLN-BYTE-005` | `UNBOUNDED_BYTES_READ` | `Bytes` read without a declared memory limit or streaming path |

Example:

```logicn
let b: Byte = 300
// ^ LLN-BYTE-001: Byte value must be between 0 and 255.
```
...
## 8. AstNodeKind Additions (logicn-core/src/index.ts)

Two new node kinds were added to `AstNodeKind` in `@logicn/core` on 2026-05-26:
