...
```bash
logicn build --emit-intent-graph
logicn graph intent --out build/intent-graph.json
```
...
Instead of asking an AI or compliance tool to infer behavior from source text, LogicN can
directly state:
...
```json
{
  ...
  "project": {
    "name": "orders-service",
    "workspace": "logicn.workspace.json"
  },
  "build": {
    "id": "build_abc",
    "commit": "abc123",
  ...
  "edges": [
    ...
    },
    {
      ...
      "span": { "file": "api-orders.lln", "start": { "line": 24, "column": 1 }, "end": { "line": 24, "column": 25 } }
    }
  ],
  "indexes": {}
}
...
logicn build --emit-intent-graph
logicn graph intent --out build/intent-graph.json
logicn graph intent --format json
logicn graph intent --format graphml
logicn graph intent --only capabilities
logicn graph intent --only secrets
logicn graph intent --flow createOrder
logicn graph intent --redact
logicn ai-context --include intent-graph
```
...
| Code | Meaning |
|---|---|
| `LLN-INTENT-GRAPH-001` | Intent graph emitted |
| `LLN-INTENT-GRAPH-002` | Unresolved edge omitted due to type errors |
| `LLN-INTENT-GRAPH-003` | Secret value redacted |
| `LLN-INTENT-GRAPH-004` | Schema validation failed |
| `LLN-INTENT-GRAPH-005` | Graph contains experimental node kind |
| `LLN-INTENT-GRAPH-006` | Transitive summary omitted due to cycle |
...
|---|---|
| `logicn-core` | Semantic definitions: flows, effects, capabilities, resources, boundaries |
| `logicn-core-compiler` | Graph extraction from AST, type/effect/capability analysis |
| `logicn-core-reports` | Intent graph schema, report writer, validation |
| `logicn-devtools-project-graph` | Graph visualization, indexes, AI/project context integration |
| `logicn-core-security` | Redaction, secret/resource/security metadata |
| `logicn-core-cli` | Graph/build commands and export options |
| `logicn-framework-app-kernel` | API/webhook/runtime boundary nodes |
| `logicn-core-compute` | Compute-target/resource planning nodes |
