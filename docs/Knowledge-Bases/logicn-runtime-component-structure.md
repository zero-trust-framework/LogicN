...
graph TD
    ...
    subgraph TARGETS["Execution Targets"]
        TC["logicn-target-cpu\nx86-64 · ARM · RISC-V"]
        TW["logicn-target-wasm\nbrowser · edge · serverless"]
        TG["logicn-target-gpu\nCUDA · ROCm (post-v1)"]
        TN["logicn-target-native\nnative binary output"]
        TP["logicn-target-photonic\noptical compute (research)"]
        TAI["logicn-target-ai-accelerator\nTPU · Gaudi · Trainium (post-v1)"]
    end

    subgraph FRAMEWORK["Framework Layer"]
        FA["logicn-framework-api-server\nHTTP transport · route manifests"]
        FK["logicn-framework-app-kernel\nauth · rate limits · jobs"]
        FW["logicn-web-*\nrouter · render · state · events"]
    end

    subgraph DEVTOOLS["DevTools"]
        DS["logicn-devtools-security\nrunSecurityAudit · PCI DSS 4.0.1"]
        DN["logicn-devtools-naming\nLLN-NAMING-001..005"]
        DC["logicn-devtools-context\ncontext receipts · --summary"]
        DI["logicn-devtools-intelligence\nBM25 hybrid search"]
        DP["logicn-devtools-provenance\ndata lineage · W3C PROV-JSON"]
        DPCI["logicn-devtools-pci\nPCI DSS 4.0.1 audit"]
        DB2["logicn-devtools-benchmarks\n23 benchmarks · compare"]
    end

    subgraph EXT["Extension Packages"]
        EV["logicn-ext-secrets-vault\nHashiCorp Vault · dual-token rotation"]
        EP["logicn-ext-proof-snarkjs\nGroth16 Phase 1 prover"]
    end
...
graph LR
    ...
    subgraph ENFORCED["Compiler-enforced at build time"]
        E1["effects → LLN-EFFECT-001..005\ndeny-by-default: omit = pure"]
        E2["intent → LLN-GOV-010\nrequired for secure flows"]
        E3["secrets → LLN-SECRET-001/002/003\nsink guards: log/serialize/network"]
        E4["invariant → LLN-TERM-001\ndecreases annotation"]
        E5["cyber_physical_hardening → LLN-GOV-017\nvalidates values + warns if low-risk"]
    end
...
sequenceDiagram
    ...
    DEV->>RT: logicn run program.lln --invoke main
...
```mermaid
graph TD
    subgraph INPUT["Input: auth-service corpus (31 .lln files)"]
        F["examples/auth-service/*.lln"]
    end

    subgraph TOOLS["DevTools packages"]
        SEC["logicn-devtools-security\nrunSecurityAudit\nLLN-TAINT/GATE/SECRET/GOV/PCI\n34 tests"]
        NAM["logicn-devtools-naming\nLLN-NAMING-001..005\nabbreviation detection\n15 tests"]
        CTX["logicn-devtools-context\nContext receipts\n51-97% token reduction\n35 tests"]
        INT["logicn-devtools-intelligence\nBM25 hybrid search\n81 flows indexed\n16 tests"]
        PRV["logicn-devtools-provenance\ndata lineage graph\nW3C PROV-JSON output\n20 tests"]
        PCI["logicn-devtools-pci\nPCI DSS 4.0.1\nLLN-PCI-001..010\n12 tests"]
        BEN["logicn-devtools-benchmarks\n23 benchmarks\ngoverned vs Rust/WASM/Node/Python"]
    end
