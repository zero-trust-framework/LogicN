...
entropy capability in LogicN. **No hardware in scope, no engine code yet** — this defines the boundary, the
governance semantics, and the acceptance bar so the capability can be wired when vetted hardware exists.
Source lane: `logicn-quantum-resilience-roadmap.md` §Q1 + `rd-absorbed/rd-qrng-entropy-source-for-hybrid-signing.md`
(Lane D). The web-cited standards/products grounding is R&D bridge task `0005` (feeds this design; refine on its return).

> **Crypto-on-core (`LLN-SUBSTRATE-001`):** a QRNG is a **source of randomness, not a cryptographic primitive.**
...
QRNG availability is a **capability behind the LogicN governance boundary**, modelled like the Lane B/E quantum
...
| **Health → verdict** | a failed RCT/APT health test flips the capability to **unavailable** mid-run; the verdict for any QRNG-requiring policy collapses to **`unknown → deny`** (K3, `LLN-GOV-3VL-001`). |
...
- **`LLN-ENTROPY-001`** — a policy/contract that **requires** `entropy.qrng` but the capability is unavailable (no
  vetted source, or a failed health test) → **deny** (fail-closed; audited, never silent).
- **`LLN-ENTROPY-002`** — an attempt to consume **raw** entropy-source output as key material (bypassing the
  SP 800-90B health gate / 90A DRBG) → **error** ("never raw bits → key").

## 4. How it meets the engine
The QRNG capability is **upstream of** `logicn-ext-tmf`'s key schedule — it supplies the bytes that seed:
...
4. the `entropy.qrng` capability + `LLN-ENTROPY-001/002` enforced in the governance verifier.
...
## See also
`logicn-quantum-resilience-roadmap.md` (§Q1, the lane this implements) · `rd-absorbed/rd-qrng-entropy-source-for-hybrid-signing.md` (Lane D research) · `logicn-tmf-engine.md` (the consumer) · `logicn-three-valued-governance.md` (the K3 fail-closed gate) · `logicn-quantum-resistance-posture.md`. Grounding survey: R&D bridge task `0005`.
