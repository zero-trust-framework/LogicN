...
|-----|------|-------------|----------|----|--------|
| 0100 | Pre-build fail-closed threat model of web-* stubs | Threat model of the 6 web-* stub packages → deny-by-default invariants (escape/sanitise/redirect/gesture) mapped to OWASP/CWE/NIST + the FUNGI-WEB-* series + SPEC acceptance skeletons | SHIPPED | 93 | **NO** |
...
| 0104 | env.tmf sealed secrets (`@galerina/ext-secrets-tmf`) | Secrets as encrypted-at-rest sections in the .tmf container (KEM-DEM X25519+ML-KEM-768 / AES-256-GCM), decrypt-in-zeroed-arena, fail-closed K3 loader | SHIPPED | 74 | defensive-pub |
| 0105 | Flow-kind tier floor (FUNGI-TIER-001) | Compile-time pass flooring a flow's governance tier to "secure" when its effects touch any of 21 secure-required effects — closes an under-tiered fail-open | SHIPPED | 93 | defensive-pub |
...
| 0111 | Affine consume-once typestate passport | FUNGI-AFFINE-001 (consumed passport can't be re-used) + FUNGI-PASSPORT-002 (state-skip) on the value-state-checker; Raw→Verified→Authorized→Sealed | SHIPPED | 88 | defensive-pub |
