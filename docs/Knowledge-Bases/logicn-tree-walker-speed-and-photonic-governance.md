...
> Companion (speculative levers, now backed by these measurements): [logicn-interpreter-speedup-and-json-rd.md](logicn-interpreter-speedup-and-json-rd.md).
...
1. **The async/await-per-node tax is ~80% of the cost (7.4×)** — it dominates everything. This empirically
   validates LogicN's `SyncInterpreter` (Phase 27B) as the **#1 lever**; since its node coverage is partial,
...
LogicN has **no user-facing async/await** — `async`/`await`/`yield` are in `V1_FUTURE_RESERVED` (lexer.ts:187-191),
...
(every `.lln` program pays it, even a 100%-sync no-effect flow). **Variant G proves the converse:** an
...
**Verdict:** *as a language feature* — don't add a colored one (LogicN is already on the correct colorless/effects
...
— LogicN's "Io-as-a-parameter" analogue). Biggest measured win (7–9×), 0014-determinism-safe (G computed the
...
parameter — LogicN already took that fork at the language level (capabilities are parameters, not a color), so
...
- **The win is mathematical, not a photonic speed claim:** governance becomes associative-semiring linear algebra
  ...
  substrate's strength), **never crypto** (its weakness). See [logicn-three-valued-governance.md](logicn-three-valued-governance.md).
...
## See also
[logicn-interpreter-speedup-and-json-rd.md](logicn-interpreter-speedup-and-json-rd.md) (speculative levers, now measured here) ·
[logicn-three-valued-governance.md](logicn-three-valued-governance.md) (the K3 algebra governance-as-T-MAC reduces) ·
[logicn-substrate-failure-model.md](logicn-substrate-failure-model.md) · [logicn-formal-verification-direction.md](logicn-formal-verification-direction.md) (the SMT/exact-equality point) ·
