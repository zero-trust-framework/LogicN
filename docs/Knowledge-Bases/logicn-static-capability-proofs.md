...
LogicN's capability system answers the question before execution begins: *does this flow have permission to do what it wants to do?* If the answer is no, the build fails.
...
```text
LLN-EFFECT-001: Flow getPatient calls AuditLog.write which requires effect audit.write,
...
In `logicn check` mode this is a warning with a suggested fix. In `logicn build --production` this is a hard error (see Decision 2).
...
LogicN's static capability proofs shift this work to compile time:
...
The effect checker compares required effects against the contract's `effects` block. Any required effect not declared produces `LLN-EFFECT-001`. Any transitive effect not declared (inherited from a callee) produces `LLN-EFFECT-002`.
...
flow body scan
  ...
  → any missing: LLN-EFFECT-001 / LLN-EFFECT-002 emitted
```
...
```text
LLN-EFFECT-002: Flow processOrder inherits effect network.outbound from fetchRate
...
| --- | --- | --- |
| `logicn check` | Warning + suggested fix | Always error |
| `logicn check --strict` | Error | Always error |
| `logicn build` | Warning (configurable) | Always error |
| `logicn build --production` | Hard error | Always error |
...
When `LLN-EFFECT-001` or `LLN-EFFECT-002` fires, the compiler produces a machine-readable fix suggestion:
...
The `suggestedCode` field enables IDE quick-fix and `logicn fix` CLI application without manual editing.
...
RULE 3   Missing effects emit LLN-EFFECT-001 (direct) or LLN-EFFECT-002 (transitive).
...
- `logicn-passive-execution-plans.md` — plans that record approved_capabilities
- `logicn-metadata-erasure.md` — what survives into the plan vs what is erased
- `logicn-phase-13-decisions.md` — Decision 2 (warn vs error modes), Decision 5 (canonical hashing)
- `logicn-gradual-capability-inference.md` — inference modes for developer workflow
- `logicn-stage-b-root-capability-provider.md` — how capabilityHost is bootstrapped
- `compiler-diagnostics.md` — full LLN-EFFECT-* and LLN-BOUNDARY-* code table
