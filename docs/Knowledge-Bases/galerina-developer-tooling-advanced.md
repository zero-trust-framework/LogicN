...
1. **Intent-Driven Test Skeleton Generation** — `galerina generate tests` derives the test
   ...
   a `--tap` TAP plan; the `--out <dir>` file-emission and the `.fungi` test-skeleton output shown
...
galerina generate tests api-orders.fungi
galerina generate tests api-orders.fungi --tap
# PLANNED (file emission to a directory — not yet built; see R&D 0016/0119):
galerina generate tests api-orders.fungi --out tests/generated/
```
...
```text
Warning: FUNGI-CAPABILITY-IMPORT-001
...
| `FUNGI-CONSTRAINT-004` | Transitive constraint not covered by caller declaration |
