...
```
record-allocation LogicN WASM: ~35M/s  (vs 216K current = 162× improvement)
...
private evalExprSync(node: AstNode): LogicNValue {
  switch (node.kind) {
    ...
    case "identifier":    return this.scope.get(node.value ?? "") ?? LLN_VOID;
    case "binaryExpr": {
      ...
      return fn ? fn(left, right) : LLN_VOID;
    }
    case "ifStmt":   return this.evalIfSync(node);
    case "letDecl":  { this.scope.set(node.value!, this.evalExprSync(node.children![0]!)); return LLN_VOID; }
...
**Implementation:** A dedicated `evalIntBinary(left: number, op: string, right: number): LogicNValue` that skips all tag checking.
...
export async function runWasmBenchmark() {
  // compile benchmark.lln → WAT → binary → WebAssembly.instantiate
...
**Target:** By Phase 33, LogicN pure flows should reach Python-class throughput (1-10M ops/s) via the bytecode VM + WASM path.
...
```
LogicN governed (tree-walker): 292K/s   ← current
LogicN WASM:                   ~500M/s  ← Phase 27 enabled
Node.js:                       795M/s

LogicN WASM is 63% of Node.js speed for pure integer flows.
```

That single number changes the entire narrative from "LogicN is 2,700× slower than Node.js" to "LogicN WASM is within 2× of Node.js with full governance at compile time."

---

## See Also

- `logicn-roadmap-phase26-50.md` — phase ordering
