...
The `wasm-standalone` target compiles a LogicN flow to a self-contained WASI
module. The module:
...
```
logicn build --target=wasm-standalone examples/healthcare/
```
...
The `getPatient` flow (`examples/healthcare/getPatient.lln`) demonstrates the
...
Phase 26 sets the goal of **Stage B parser parity**: the self-hosted `parser.lln`
(written in LogicN) should produce identical parse results to the TypeScript
