...
```
LogicN should make botnet behaviour impossible to hide.
```
...
|---|---|---|
| Deny network by default | ✅ | LLN-EFFECT-001 fires if network.outbound undeclared |
| No dynamic code execution | ✅ | LLN-SOURCE-ESCAPE-001 (eval, Function, dynamic import) |
| No monkey patching | ✅ | LLN-SEC-020/021 (Runtime.patch, prototype mutation) |
| Signed packages | ✅ | LLN-PKG-005, runtime policy: require signature |
...
| Stdlib capability enforcement | ✅ | LLN-STDLIB-001 (File.readText needs filesystem.read) |
...
**New diagnostic:** `LLN-NET-001` (NetworkDestinationDenied) — called host not in allowlist
**New diagnostic:** `LLN-NET-002` (PrivateRangeAccess) — resolved IP is in private range
...
**Enforcement gap:** `LLN-RUNTIME-006` (RateLimitExceeded) — for when limits fire at runtime.
...
- No dynamic code (LLN-SOURCE-ESCAPE-001)
- No monkey patching (LLN-SEC-020/021)
...
- `logicn-hybrid-wasm-architecture.md` — 8 native governance rules
- `logicn-explicitness-principles.md` — nothing important hidden
- `logicn-static-capability-proofs.md` — capability enforcement architecture
