...
Package `logicn-ext-bridge-cpp` does not yet exist. This document records the intended design so
...
not developer obligations. See `logicn-contract-economics.md` and
`logicn-design-secrets-epilogue-blocks.md` for the canonical description of auto-by-default
governance blocks.
...
`logicn-ext-bridge-cpp` is a **non-core devtools package** whose goal is to validate that
LogicN can express the same low-level semantics as C++ without sacrificing safety or governance.
It does this in two directions:

- **Inward (transpile C++ → LogicN):** parse C++ source and emit equivalent governed `.lln` flows,
  injecting appropriate contracts automatically.
- **Outward (verify equivalence):** run side-by-side benchmarks between clang -O3 output and LogicN
...
runner would bloat the compiler runtime for the majority of LogicN workloads that never touch C++.
It sits at the same tier as `logicn-ext-secrets-vault` and `logicn-ext-proof-snarkjs`: optional,
injected at the project level, invisible to pure LogicN consumers.
...
logicn-ext-bridge-cpp/
  src/
    ...
    transformer/    # Maps C++ constructs to LogicN IR nodes
    emitter/        # Serializes IR nodes to .lln source text
  ...
  tests/
    ...
    equivalence/    # Side-by-side C++ vs LogicN output comparisons
  benchmarks/
    runner.ts       # Orchestrates clang -O3 compilation + LogicN WASM Phase 27 emission
...
## 3. Core C++ → LogicN Mappings

### 3.1 RAII / Stack Allocation → Linear Scoped `let` Binding

C++ RAII ties object lifetime to lexical scope via constructor/destructor pairs. LogicN expresses
...
takes the pointer out of the allocated region. LogicN replaces this with **typed array access
...
buf[42] = 99    // compile-time or runtime bounds check; LLN-BOUND-* error on violation
```
...
LogicN's equivalent claim is that **WASM Phase 27 output** (the highest optimization tier in
LogicN's emission pipeline, targeting the WASM Component Model + SIMD extensions) produces
machine code of comparable density to clang `-O3` for the same algorithm expressed in LogicN.

The equivalence test runner (see §5) validates this claim empirically. The transformer does not
attempt to replicate C++-specific optimizations; it emits semantically correct LogicN and trusts
the WASM Phase 27 pipeline to optimize. Where a specific C++ template produces a known hot path
(e.g. `std::sort` → introsort), the transformer may emit a hint annotation that the LogicN
runtime can use to select an equivalent primitive.

### 3.4 Manual Heap Management → LogicN SoA Arena + Automatic GIR Lowering

C++ manual heap management (`new`/`delete`, allocators, placement new) requires the programmer
to track ownership explicitly. LogicN replaces this with **Structure-of-Arrays (SoA) arenas** —
...
```logicn
// LogicN equivalent — arena allocated, GIR-lowered to SoA layout
...
When the transformer emits a `.lln` file from C++ source, it **auto-injects a `contract {}`
...
the developer may override by editing the emitted `.lln`.
...
2. Transpiles the same algorithm via the bridge transformer to a `.lln` file.
3. Compiles the `.lln` file at WASM Phase 27.
4. Runs both WASM modules on an identical input vector and asserts:
   ...
   - **Performance parity:** LogicN execution time is within a configurable threshold (default:
...
Phase 1 can begin immediately using the existing LogicN Stage-A compiler and a simple regex-based
...
`logicn-ext-bridge-cpp` is **non-core**. It must not be imported by:
- `logicn-core-compiler` (no Clang dep in the core compiler)
- `logicn-core-runtime`
- `logicn-core-stdlib`

It may import from `logicn-core-compiler` (to emit GIR nodes) and from `logicn-core-stdlib`
(for `Array<T>`, `FileHandle`, etc. type references). The package registers itself via the same
`ProverBackend`-style plug-in injection pattern used by `logicn-ext-proof-snarkjs` — the core
...
- `logicn-contract-economics.md` — auto-by-default dual-mode contract blocks (same pattern used
  for transpilation-synthesized contracts)
- `logicn-design-secrets-epilogue-blocks.md` — `secrets {}` and `epilogue {}` blocks; dual-mode
  governance pattern this package inherits
- `neutral-governed-ir.md` — GIR lowering that the transformer emits into
- `logicn-zk-proof-plan.md` — `ProverBackend` plug-in injection pattern (same architecture used
  for bridge package registration)
- `logicn-asic-cyber-physical.md` — hardware target contracts synthesized for C++ hardware-access
  patterns (§4)
