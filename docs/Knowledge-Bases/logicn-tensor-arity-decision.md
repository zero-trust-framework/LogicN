...
Tensor                       // invalid — LLN-TYPE-009
Tensor<Float32>              // invalid — LLN-TYPE-009 (expected 2 args)
```
...
LogicN's governance-first design.

---

## Why Not Backend Parameters

LogicN does **not** include device, layout, or backend in the tensor type:
...
| Source | Diagnostic |
|---|---|
| `Tensor` (bare, no args) | `LLN-TYPE-009` — expects 2 type arguments |
| `Tensor<Float32>` (one arg) | `LLN-TYPE-009` — expects 2 type arguments |
| `Tensor<Float32, [1,128], Gpu>` (three args) | `LLN-TYPE-009` — expects 2 type arguments |
| `Tensor<Float32, [1, 128]>` | valid |
...
```
LLN-TYPE-009: InvalidGenericInstantiation
...
LogicN maximises compatibility by separating:
