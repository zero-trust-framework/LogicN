...
Vendor-specific support belongs in target adapters and device profiles, not LogicN language syntax.
...
export interface GpuPlan {
  ...
  /** Explanation strings for logicn explain. */
  explanation: string[]
}
```
...
LogicN distinguishes optical I/O from photonic compute.
...
`logicn-core-compute` must not claim that photonic execution exists until a real backend exists.
...
```text
packages-logicn/logicn-core-compute/src/
...
```text
LogicN source declares intent; compute planning recommends safe targets; runtime/deployment policy decides execution.
```
