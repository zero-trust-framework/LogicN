...
**Parent agenda:** `logicn-photonic-tri-substrate-rd-agenda.md` §5 (Direction C).
...
(`logicn-three-valued-governance.md`). Direction C models the substrate's *failure modes in software*
so the verifier can reason against them **before any silicon exists**.
**Module:** `packages-logicn/logicn-tower-citizen/src/substrate-model.ts` — a **new sibling file**.
...
- **von Neumann N-modular redundancy (NMR) / TMR.** Majority voting of `N` independent replicas fails
  ...
  decreasing in odd `N`**. The theorem behind acceptance test #2. LogicN already ships the 3-input
...
**not** help — the check sets `redundancyHelps=false` and emits `LLN-SUBSTRATE-003` rather than
...
## 5. Diagnostics — `LLN-SUBSTRATE-*` (reconciled against `LLN-PHOTONIC-*`)

`LLN-PHOTONIC-001…010` already exist — **hardware-capability** codes (static device properties, e.g.
...
strictly complementary, no collision. They sit after `LLN-PHOTONIC-*`, before `LLN-RUNTIME-*`.

| Code | name | meaning | severity |
|---|---|---|---|
| `LLN-SUBSTRATE-001` | `CRYPTO_ON_NOISY_LANE` | a `Hash`/`Sign`/crypto effect declared on a noisy lane. Integrity is **never tolerated** — forbidden outright (the durable B1 insight, registered now). | **error** (always) |
| `LLN-SUBSTRATE-002` | `TOLERANCE_UNACHIEVABLE_UNDER_NOISE` | `epsilonModeled > epsilonDeclared` at the declared `N`. | **error** in `production`/`deterministic`; **warning** in `dev` |
| `LLN-SUBSTRATE-003` | `REDUNDANCY_INSUFFICIENT` | redundancy declared but cannot meet tolerance under the model (incl. the `pBad ≥ 0.5` "voting won't help" case). | **error** (always) |
| `LLN-SUBSTRATE-004` | `UNVOTED_ANALOG_INTO_DETERMINISTIC` | an un-voted (`N=1`) noisy result feeds a context requiring determinism (strict profile / crypto boundary). | **error** (always) |

`LLN-SUBSTRATE-*` are a **higher** severity category than Direction A's `LLN-GOV-3VL-001` (`warning`,
...
1. **Tolerance-unachievable flagged (agenda #1).** Non-trivial noise + tight `epsilonDeclared` at `N=1`
   ⇒ `met=false` and `verifyToleranceUnderNoise` emits exactly one `LLN-SUBSTRATE-002` whose
...
2. **Raising TMR clears it, monotonically (agenda #2).** Same `(params, epsilonDeclared)`, `pBad<0.5`:
   ...
   + `LLN-SUBSTRATE-003` and that the trace never clears.
...
7. **Crypto-on-noisy + unvoted-sink.** `hasCrypto` on a noisy lane ⇒ `LLN-SUBSTRATE-001` (error)
   regardless of tolerance; `N=1` + deterministic sink + noise ⇒ `LLN-SUBSTRATE-004`.
...
```ts
// substrate-model.ts  (logicn-tower-citizen) — sibling to tpl-simulator.ts; imports its gates UNCHANGED
...
- **Direction B — substrate/tolerance contracts.** The `substrate { lane, tolerance, redundancy }`
  ...
  pass in `logicn-core-compiler/src/governance-verifier.ts` consuming the parsed block + `effectResult`
  and calling this module; `LLN_SUBSTRATE_001..004` code constants registered in
  `logicn-core-compiler/src/index.ts`; the B1 crypto-on-core invariant pulled forward; `routePrecision()`
...
1. **Numeric codes canonical** (`LLN-SUBSTRATE-001..004`) with `screaming_snake` names — matches the
   `LLN-PHOTONIC-NNN` neighbourhood.
...
`logicn-photonic-tri-substrate-rd-agenda.md` (parent §5) · `logicn-three-valued-governance.md`
(Direction A — `Verdict`, `vAnd`, `decideAtBoundary`, No-Coercion) · `tpl-simulator.ts`
(`#173/#196` gates + `consensusTrit`, frozen) · `compiler-diagnostics.md` (`LLN-PHOTONIC-*` neighbourhood
+ new `LLN-SUBSTRATE-*` subsection) · `hybrid-engine.ts` (seeded-PRNG idiom) · `sentinel-power`/`-time`
