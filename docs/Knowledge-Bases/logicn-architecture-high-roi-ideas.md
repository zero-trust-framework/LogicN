...
**Status:** `capabilityHost.ts`, `contractEnforcer.ts`, `LLN-EFFECT-*` series implemented.
...
**Status:** Not yet implemented. Phase 21A. See `logicn-tensor-numeric-performance.md`.
...
Bootstrap determinism tests prove planHash stability. See `logicn-passive-execution-plans.md`.
...
`@logicn/devtools-graph-algorithms` package provides BFS, DFS, topoSort, fixpoint.
...
Generate `logicn.ai.json` containing flows, contracts, types, effects, capabilities, diagnostics, entry points.
...
| — | LLN-STDLIB-001 enforcement | ✅ Phase 19A |
...
```
The LogicN architecture separates concerns cleanly:
