...
**Auto-by-default:** The `strict` security profile (LLN-PROFILE-002/003) already bans unbounded
...
LogicN's ProofGraph is the architectural guarantee that governed execution was correctly
...
The `strict` security profile bans unbounded recursion (LLN-PROFILE-002) but does so by
...
LogicN's taint system assigns `TaintType.Unverified_Raw` to any value that arrived from an
...
Types in LogicN are organized into a **strict ascending hierarchy of universes**. A type at
...
//                                         ^^^^
// LLN-UNIV-001: Self-referential type at Universe<1> — use an opaque handle or
...
The existing `type` keyword and all current LogicN structural types (`record`, `Array<T>`,
...
relevant only when writing the type-checker itself in LogicN (Stage B self-hosting work).
...
`LLN-TERM-001` (unproven termination) and the flow is rejected under the `strict` profile.
...
The `strict` security profile currently bans unbounded recursion via LLN-PROFILE-002 and
LLN-PROFILE-003. With the termination engine:
...
Taint status in LogicN is a **permanent mathematical property** of a value, not a mutable flag.
The formal rule:
...
> (an LLN-GATE-* flow), which takes the tainted value as input and produces a new,
...
The `checkValueStates` function (Stage-A compiler) and the `LLN-GATE-*` error codes implement
...
The `once` qualifier is related to the linear type semantics already present in LogicN's scoped
`let` bindings (see `logicn-cpp-bridge.md` §3.1) and the `secrets {}` vault handle model
described in `logicn-design-secrets-epilogue-blocks.md`. Taint monotonicity via `once` is a
formal extension of those existing patterns into the type system.

### 4.4 Relationship to ProofGraph

Taint consumption events (a `once Unverified_Raw` value passing through an LLN-GATE-* flow)
...
type-checker is stable enough to express the stratification rules in LogicN itself.
...
| Code | Meaning |
|------|---------|
| `LLN-UNIV-001` | Self-referential type declaration — Universe stratification violation |
| `LLN-UNIV-002` | Type at level N references type at level > N-1 |
| `LLN-TERM-001` | Recursive flow with no `decreases` annotation (under `strict` profile) |
| `LLN-TERM-002` | `decreases` metric not strictly decreasing at a call site |
| `LLN-TERM-003` | `decreases` metric lower bound not established by `contract.invariant` |
| `LLN-TAINT-001` | Taint stripped implicitly through generic instantiation |
| `LLN-TAINT-002` | `once` value used more than once (linearity violation) |

---

## 7. Related Documents

- `logicn-design-secrets-epilogue-blocks.md` — `once` qualifier semantics connect to vault
  handle linearity; `secrets {}` grammar pattern used for `decreases` keyword implementation
- `logicn-contract-economics.md` — `contract.invariant {}` block fits within the same
  dual-mode governance pattern as `economics {}`
- `logicn-zk-proof-plan.md` — termination proofs as first-class ProofGraph nodes, auditable
...
- `trust-conversion-and-data-safety.md` — LLN-GATE-* gateway transformation patterns
- `runtime-profiles.md` — `strict` profile (LLN-PROFILE-002/003) that termination proofs
...
- `logicn-cross-layer-resilience.md` — Universe stratification contributes to the mathematical
