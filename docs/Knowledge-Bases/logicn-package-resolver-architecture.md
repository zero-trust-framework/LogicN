...
Phase 18B: hash, signature, targets, compute, installScript, LLN-PKG-* diagnostics, resolver reports
...
```
LogicN packages should be resolved like security-critical infrastructure,
...
Capability manifest (what it declares: effects, capabilities, targets)
  ↓
Deterministic dependency graph (package.logicn.lock)
  ↓
...
## Package Manifest Schema (package.logicn.yaml)
...
registry: "https://registry.logicn.dev"
...
targets:
  cpu:
    path: dist/cpu.logicn
  wasm:
    path: dist/wasm.logicn
  npu:
    path: dist/npu.logicn
...
## Diagnostic Codes — LLN-PKG-*

| Code | Name | Severity | Trigger |
|---|---|---|---|
| `LLN-PKG-001` | `CapabilityExpanded` | error | Package declares more capabilities than the lockfile snapshot |
| `LLN-PKG-002` | `UntrustedRegistry` | error | Package comes from an unregistered or unverified registry |
| `LLN-PKG-003` | `MissingHash` | warning | Package manifest has no content-addressable hash |
| `LLN-PKG-004` | `InstallScriptDenied` | error | Package declares or attempts an install script; default policy denies |
| `LLN-PKG-005` | `MissingSignature` | warning | Package has no signature; origin cannot be verified |
...
- Monkey-patch runtime objects (LLN-SEC-020/021)
...
That is a `LLN-PKG-001` (CapabilityExpanded) — a breaking security change that must be explicitly reviewed and re-approved.
...
The lockfile (`package.logicn.lock`) stores:
...
runtime package mutation            — LLN-SEC-020/021
dynamic dependency loading          — ungoverned
unverified registries               — supply chain risk
hidden transitive capabilities      — LLN-PKG-001
...
| `LLN-PKG-001..005` diagnostic constants | ✅ Phase 18B |
| `checkPackageCapabilityExpansion()` | ✅ Phase 18B |
| `getResolverReport()` | ✅ Phase 18B |
| Full lockfile (`package.logicn.lock`) | 📋 Phase 19 |
