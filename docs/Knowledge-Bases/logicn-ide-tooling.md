...
**Depends on:** [logicn-semantic-graph-system](logicn-semantic-graph-system.md), [logicn-ai-semantic-graph-output](logicn-ai-semantic-graph-output.md), `logicn-lsp`
...
Most language IDEs help developers write code. The LogicN IDE helps developers understand consequences.
...
The `logicn-lsp` language server implements the Language Server Protocol and targets:

| Editor | Delivery |
|---|---|
| VS Code | Official `logicn` extension (VS Code Marketplace) |
...
The LSP server is a single binary (`logicn-lsp`) that all editors connect to. Editor-specific extensions handle installation and startup; the semantic features are implemented once in the server.

---

## Syntax Highlighting

The LogicN TextMate grammar and Treesitter grammar cover:
...
effects [database.read] {          // ← only database.read declared
    ...
    // ⚠ LLN-EFFECT-002: flow uses effect payment.charge not declared in contract
...
LogicN diagnostics follow a consistent format:
...
```
⚠ LLN-PRIVACY-005: protected Email passed to audit.write — redact() required first.
```
...
## logicn explain Command

```bash
logicn explain getUser
logicn explain processPayment --detail effects
logicn explain users.lln
```
...
The developer sees deployment consequences without running `logicn plan` manually. The plan is computed from the SemanticGraph and the project's target configuration.
...
- The assistant receives `logicn.ai.json` for the current project
...
The IDE is the delivery mechanism. The AI graph is the context. Together, an AI assistant working in a LogicN project generates code that passes governance checks on the first attempt, not after multiple correction cycles.

---

## Excellent Error Messages

LogicN error messages are designed with the same care as Rust and Elm. Every error message answers four questions:
...
```bash
logicn find effect database.write
logicn find capability payment.charge
logicn find protected Email
logicn find intent UserLookup
logicn find boundary users-service
```
...
LogicN IDE tooling helps developers understand consequences. It shows what a flow will do, what it will not do, what data it touches, what authority it requires, and what governance it must satisfy — before the code is deployed, before a security review, before an audit.

The goal is that a developer looking at any flow in any LogicN project can answer the question "what will this actually do, and is it safe?" in under ten seconds, without reading anything other than what the IDE shows them.
...
| logicn explain is always accurate | Generated from compiler truth, not from comments |
...
- [logicn-semantic-graph-system](logicn-semantic-graph-system.md) — SemanticGraph architecture
- [logicn-ai-semantic-graph-output](logicn-ai-semantic-graph-output.md) — AI graph export (logicn.ai.json)
- [logicn-one-click-governance-fixes](logicn-one-click-governance-fixes.md) — quick fix implementations
- [logicn-developer-tools](logicn-developer-tools.md) — developer tooling overview
- [logicn-developer-tooling-advanced](logicn-developer-tooling-advanced.md) — test generation and signatures
