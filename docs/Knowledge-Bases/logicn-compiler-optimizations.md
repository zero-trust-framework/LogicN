...
performance foundation for LogicN. Each builds on the language's existing pure-flow
...
Not all pure flows should auto-evaluate at compile time. LogicN distinguishes:
...
Rather than lowering to chained runtime calls, LogicN should first represent pipelines as
a single IR node:
...
LogicN AST
  → LogicN HIR
  → LogicN MIR  (typed, carries effects/ownership/boundaries/source spans)
  → LogicN MLIR dialect  (preserves high-level semantics for multi-target lowering)
...
MLIR is the better bridge for LogicN's multi-target ambition. LLVM is the final native backend.
...
**Stage 4 — MLIR dialect**: introduce `logicn.effect`, `logicn.boundary`, `logicn.secret`,
`logicn.taint`, `logicn.compute`, `logicn.vector` operations.
...
|---|---|
| `logicn-core` | Purity semantics, comptime syntax, pipeline semantics |
| `logicn-core-compiler` | Comptime evaluator, pipeline IR, fusion pass, range analysis, MIR generation |
| `logicn-core-runtime` | Unfused fallback operations, runtime arenas |
| `logicn-core-compute` | Target capability rules, workload fingerprints, PGO trust policy |
| `logicn-core-security` | Sandboxing rules for comptime, secret restrictions |
| `logicn-target-cpu` | CPU feature detection, LLVM target policy |
| `logicn-target-wasm` | WASM module metadata |
| `logicn-target-native` | Native artefact metadata |
| `logicn-target-gpu` | GPU kernel launch parameters, PGO evidence |
| `logicn-core-vector` | Vector/matrix/tensor operations feeding IR |
| `logicn-core-reports` | Optimisation reports, PGO evidence, comptime reports, backend reports |
| `logicn-tools-benchmark` | Reproducible benchmarks, PGO generation, regression detection |
