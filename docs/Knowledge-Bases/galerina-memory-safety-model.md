...
- **AMENDMENT 2 (scoping):** the WAT bump heap has **no free** (monotonic `$__spore_heap`; records never freed within a flow; host GC reclaims) ⇒ it is **spatially**-gapped (needs MSWasm/WasmGC) but **NOT temporally**-gapped — gen tags would be dead weight there. Keep the two surfaces separate (gen tags ⇒ the free-list pool only).
