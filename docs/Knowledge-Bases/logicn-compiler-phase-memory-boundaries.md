...
For LogicN this matters for three reasons:
...
**Better self-hosting demonstration.** The self-hosted LogicN compiler should itself be a well-governed program. Explicit phase arenas demonstrate that LogicN programs can manage structured resource lifetimes without garbage-collection magic.
...
| Code | Name | Condition |
|---|---|---|
| `LLN-MEMORY-001` | `BoundaryLeakDetected` | Memory attributed to a phase arena is still reachable after that phase completed |
| `LLN-MEMORY-002` | `CrossBoundaryReference` | A phase arena holds a reference into another phase arena (not into CompilerRootBoundary) |
| `LLN-MEMORY-003` | `InvalidBoundaryOwnership` | An allocation was made outside any declared boundary |
...
- Implement `LLN-MEMORY-001/002/003` diagnostics — these require allocator instrumentation
...
6. `LLN-MEMORY-001/002/003` are internal diagnostics, not user errors.
...
- `logicn-stage-b-root-capability-provider` — root capability ownership model
- `logicn-governed-memory-blocks` — user-facing governed memory in LogicN programs
