# LogicN Canonical Example Corpus — Index
...
Each example lives at `docs/Examples/<level-dir>/<id>/example.lln`.
...
| `020-invalid-fn-top-level` | fn cannot be declared at top level `LLN-SYNTAX-005` | stable | Money, fn |
| `021-flow-contract-basic` | flow contract block with types, intent, and events | stable | Array, Result, events, match, protected |
| `022-no-toplevel-let` | top-level let bindings are rejected `LLN-SYNTAX-006` | stable | contract, flow |
...
| `060-invalid-email-assignment` | direct assignment to protected Email is forbidden `LLN-TYPE-003` | stable | protected |
| `061-redacted-email` | redacted Email label | stable | redacted |
| `062-invalid-redacted-email` | protected Email cannot be assigned to redacted Email directly `LLN-TYPE-002` | draft | protected, redacted |
| `063-option-some` | Option<T> with Some value | stable | Option |
| `064-option-none` | Option<T> with None value | stable | Option |
| `065-option-invalid-arity` | Option<T> requires exactly one type parameter `LLN-TYPE-009` | stable | Option |
| `066-result-success` | Result<T,E> Ok variant | stable | Result |
| `067-result-error` | Result<T,E> Err variant | stable | Result |
| `068-result-invalid-arity` | Result<T,E> requires exactly two type parameters `LLN-TYPE-009` | stable | Result |
| `069-auto-inference` | Auto type inference | stable | — |
| `070-auto-invalid` | Auto requires an initializer `LLN-TYPE-002` | draft | — |
| `071-money-gbp` | Money<GBP> type | stable | Money |
| `072-money-add-same-currency` | Money<GBP> addition with same currency | stable | Money |
| `073-money-cross-currency-invalid` | cross-currency Money addition is forbidden `LLN-TYPE-004` | stable | Money |
...
| `076-money-times-money-invalid` | Money multiplied by Money is forbidden `LLN-TYPE-004` | stable | Money |
| `077-money-ratio` | Money<GBP> / Money<GBP> yields Decimal | stable | Money |
| `078-money-ratio-cross-currency-invalid` | cross-currency Money ratio is forbidden `LLN-TYPE-004` | stable | Money |
...
| `081-tensor-invalid-arity` | Tensor requires exactly two type parameters `LLN-TYPE-009` | stable | Tensor |
| `082-readonly-view` | ReadOnlyView<T> generic type | stable | — |
| `083-readonly-view-invalid` | ReadOnlyView requires exactly one type parameter `LLN-TYPE-009` | stable | flow |
| `084-unknown-type` | unknown type name produces LLN-TYPE-001 `LLN-TYPE-001` | stable | flow |
| `085-type-mismatch` | type mismatch produces LLN-TYPE-002 `LLN-TYPE-002` | stable | flow |
| `086-protected-not-redacted` | protected value cannot be directly assigned to redacted binding `LLN-TYPE-002` | draft | protected, redacted |
...
| `105-missing-database-effect` | undeclared database.write effect `LLN-EFFECT-001` | stable | Result |
| `106-missing-network-effect` | undeclared network.outbound effect `LLN-EFFECT-001` | stable | Result, unsafe |
| `107-pure-flow-calls-effectful-flow` | pure flow cannot call an effectful flow `LLN-EFFECT-003` | draft | Money |
...
| `111-local-fn-effect-not-covered-by-parent` | local fn cannot use effects not declared by the containing flow `LLN-EFFECT-001` | stable | Result, fn, unsafe |
| `112-local-fn-cannot-declare-effects` | local fn cannot declare its own effects `LLN-SEC-014` | stable | Result, fn |
| `113-secure-flow-with-effects` | secure flow with full effects pattern | stable | Result, audit, protected, record, redacted, unsafe |
| `114-secure-flow-missing-audit-effect` | secure flow calls AuditLog.write but does not declare audit.write `LLN-EFFECT-001` | stable | Result, audit, protected, unsafe |
| `115-effect-propagation-through-call` | effect propagation through a flow call chain | draft | Result |
| `116-effect-propagation-missing-parent` | caller missing effect required by callee `LLN-EFFECT-002` | stable | Result |
| `117-effectful-operation-in-pure-flow` | filesystem.read in a pure flow is forbidden `LLN-EFFECT-003` | stable | Result, unsafe |
| `118-filesystem-read-guarded` | guarded flow with filesystem.read effect | stable | Result, unsafe |
| `119-effect-name-invalid` | non-canonical effect name is rejected `LLN-EFFECT-004` | stable | Result, unsafe |
...
| `157-invalid-email-assignment` | assigning String directly to protected Email is forbidden `LLN-TYPE-003` | stable | protected |
...
| `160-protected-not-redacted` | protected value cannot be directly assigned to redacted binding `LLN-TYPE-002` | draft | protected, redacted |
| `161-safe-audit-log` | safe audit log write with redacted value | stable | audit, protected, redacted |
| `162-invalid-audit-log` | unsafe value reaches audit sink without validation `LLN-VALUESTATE-003` | stable | audit, unsafe |
| `163-unsafe-audit-log` | passing unsafe value to audit sink `LLN-VALUESTATE-003` | stable | audit, unsafe |
| `164-safe-database-write` | protected value is safe to write to database | stable | Result, protected, record, unsafe |
| `165-unsafe-database-write` | unsafe value cannot be written to a database sink `LLN-VALUESTATE-003` | stable | unsafe |
| `166-safe-network-send` | protected value may be sent over network (policy dependent) | stable | Result, protected |
| `167-unsafe-network-send` | unsafe value crossing a trust boundary via network `LLN-VALUESTATE-003` | stable | unsafe |
| `168-redacted-network-send` | redacted value is safe to send to audit endpoints | stable | Result, audit, protected, redacted |
| `169-secret-comparison` | direct equality comparison on secrets is forbidden `LLN-SECRET-002` | stable | flow |
| `170-constant-time-comparison` | constantTimeEquals for safe secret comparison | stable | flow, for |
| `171-protected-console-log` | secret value sent to console sink is forbidden `LLN-SECRET-001` | stable | flow |
| `172-secret-console-log` | secret value sent to console sink is forbidden `LLN-SECRET-001` | stable | flow |
...
| `176-sql-injection-concat` | string taint propagation (SQL injection pattern) `LLN-VALUESTATE-004` | stable | Result, unsafe |
| `177-secret-serialization` | SecureString serialization denied `LLN-SECRET-003` | draft | Result |
...
| `203-intent-mismatch-invalid` | intent mismatch when flow behaviour contradicts declared intent `LLN-INTENT-001` | draft | Result, events, protected |
| `204-remote-execution-denied` | compute target with remote.execution denied | stable | Result, audit |
| `205-remote-execution-violation` | compute target remote violates governance policy `LLN-GOV-004` | draft | Result, audit |
| `206-protected-data-sharing-authority` | authority block for sharing protected data externally | stable | Result, audit, protected |
| `207-protected-data-sharing-missing-authority` | sending protected data externally without authority block `LLN-GOV-003` | draft | Result, audit, protected |
| `208-audit-proof-required` | delete flow with full audit evidence | stable | Result, audit, protected, record, redacted |
| `209-audit-proof-missing` | intent requires audit but no audit.write sink used `LLN-GOV-002` | stable | Result, audit, protected, record |
...
| `212-policy-purpose-mismatch` | declared purpose does not match observed template use `LLN-GOV-005` | draft | Result, audit, match, protected |
...
| `216-event-declaration` | global event declaration + flow emission + LLN-EVENT-001 `LLN-EVENT-001` | stable | Result, audit, events, record |
| `217-contract-set-validation` | contract set use resolution — LLN-GOV-011 and LLN-GOV-012 `LLN-GOV-011` | stable | Result, Set, audit, protected, record, unsafe |
| `218-errors-contract` | contract errors block — returns, map, expose, redact, audit | stable | Map, Result, audit, errors, record |
| `219-response-denies` | contract response.denies — LLN-GOV-003 `LLN-GOV-003` | stable | Result, audit, protected |
...
| `225-context-missing` | LLN-CONTEXT-001 — context require actor declared but never accessed `LLN-CONTEXT-001` | stable | Result, audit |
...
| `306-tensor-arity-invalid` | Tensor requires exactly two type parameters: element type and shape `LLN-TYPE-009` | stable | Tensor |
...
| `312-money-invalid-cross-currency` | adding Money values of different currencies is a type error `LLN-TYPE-004` | stable | Money |
...
| `316-invalid-tensor-add` | Tensor arithmetic requires shape compatibility `LLN-TYPE-004` | stable | Tensor |
| `317-money-cross-currency-compute` | Cross-currency arithmetic is denied `LLN-TYPE-004` | stable | Money |
| `318-compute-hint-missing` | ai.inference without compute target preference `LLN-HINT-COMPUTE-001` | stable | Result |
...
| `352-ai-inference-effect` | calling an AI model without declaring the required ai.inference effect `LLN-EFFECT-001` | draft | Result |
...
| `356-ai-missing-audit` | secure AI flow that declares audit.write but omits the AuditLog.write call `LLN-GOV-002` | draft | Result, audit |
...
| `358-ai-secure-string-in-ai` | passing a SecureString to an AI model is a secret-exposure error `LLN-SECRET-001` | draft | Result |
...
| `362-ai-invalid-tensor-arity` | Tensor type with missing shape argument raises LLN-TYPE-009 `LLN-TYPE-009` | stable | Result, Tensor |
| `363-local-model-only` | local-only AI classification denying both remote execution and network outbound | draft | Result, unsafe |
| `364-ai-effect-propagation` | a pure flow calling an ai.inference flow inherits that effect and must declare it `LLN-EFFECT-002` | draft | and, effects, flow |
...
| `366-ai-unsafe-input-to-model` | Unsafe input cannot flow directly to AI model `LLN-VALUESTATE-003` | draft | Result, unsafe |
| `367-ai-inference-without-effect` | ai.inference effect must be declared `LLN-EFFECT-001` | draft | flow |
| `368-contract-ai-flow` | flow contract with AI intent for IGO optimisation `LLN-HINT-COMPUTE-001` | stable | Result, events |
...
| `414-target-no-fallback-invalid` | omitting the fallback clause from a non-cpu compute target is an error `LLN-TARGET-001` | draft | Result, Tensor |
...
| `416-target-fallback-missing` | compute target without fallback `LLN-HINT-COMPUTE-001` | stable | Tensor |
...
| `452-healthcare-invalid-pii-log` | passing a protected PatientId directly to a log call without redaction `LLN-SECRET-001` | draft | Result, audit, protected, record, unsafe |
...
| `454-financial-cross-currency-invalid` | adding Money<GBP> and Money<USD> without currency conversion is a type error `LLN-TYPE-004` | stable | Money |
| `455-financial-money-calculation` | financial tax calculation using Money and Decimal | stable | Money |
| `456-compliance-pii-effect` | accessing PII without declaring the pii.read effect `LLN-PII-001` | draft | Result, protected, record, unsafe |
| `457-compliance-phi-effect` | handling HIPAA PHI without declaring the phi.read effect `LLN-PHI-001` | draft | Result, protected, record, unsafe |
| `458-compliance-audit-required` | secure flow that declares audit.write but never calls AuditLog.write `LLN-AUDIT-001` | draft | Result, audit, protected, record, unsafe |
...
| `461-authority-missing` | sharing protected data across a trust boundary without an authority block `LLN-GOV-003` | draft | Result, audit, protected, unsafe |
...
| `463-policy-mismatch` | declared purpose does not match the template used in the flow body `LLN-GOV-005` | draft | Result, audit, match, protected, unsafe |
| `464-enterprise-supply-chain` | flow requiring a module capability that has not been accepted in the supply-chain manifest `LLN-MODULE-005` | draft | Result, audit |
...
| `466-pii-without-pii-effect` | PII access requires pii.read effect `LLN-PII-001` | draft | Result |
| `467-protected-response-body` | Protected value in response body requires policy `LLN-GOV-003` | draft | Result, protected |
