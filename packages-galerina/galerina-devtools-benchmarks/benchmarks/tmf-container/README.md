...
|---|---|
| **Node.js** | **Galerina's shipped `@galerinaa/ext-tmf` engine** (`dist/index.js`, `writeTmf`). The engine is pure TypeScript-on-Node — there is **no `.spore` execution path** for `.tmf` creation — so the engine's throughput *is* the Node.js row. There is deliberately no Galerina-interpreter column here. |
