 ...
 *  (no regression) a gated flow and the canonical verifyPassword.fungi stay clean.
 */
...
const VERIFY_PASSWORD_PATH = resolve(__dirname, "../../../examples/auth-service/verifyPassword.fungi");
...
describe("0098-prov-parsefail Fix B: blind analysis denies by default", () => {
  it("a broken source sets analyzerBlind=true", () => {
    const result = analyzeFile(BROKEN_SRC, "broken.fungi");
  ...
  it("buildProvenanceGraph counts a blind file toward flowsWithUngatedSinks (CLI exit 2)", () => {
    ...
    assert.ok(graph.riskFlows.some(r => r.flowName === "<parse-failure>"),
      "a blind file must produce an explicit FUNGI-PROV-001 <parse-failure> risk entry");
  });
});
...
describe("0098-prov-parsefail: clean inputs still pass (no false positive)", () => {
  ...
  it("verifyPassword.fungi (canonical clean) has 0 ungated sinks and is not blind", () => {
    ...
    assert.equal(graph.summary.flowsWithUngatedSinks, 0,
      "verifyPassword.fungi must remain the canonical clean example (0 ungated sinks)");
  });
});
