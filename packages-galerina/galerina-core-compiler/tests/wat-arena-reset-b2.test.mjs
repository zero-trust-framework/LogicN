// B2 (R&D 0055) — per-flow arena reset. The bump pointer $__fungi_heap was monotone (0 resets ⇒ a
// process-lifetime leak that traps at maxPages). Now a LEAF entry-point (exported ∧ not called by any
// other flow) resets $__fungi_heap to WAT_HEAP_BASE at entry, reclaiming the previous invocation's arena.
//
...
async function emit(src) {
  const prog = L.parseProgram(src, "b2.fungi");
...
test("B2: a leaf entry-point that allocates emits the per-flow heap reset", async () => {
  ...
  assert.ok(wat.includes("(global.set $__fungi_heap (i32.const 1024))"), "the reset to WAT_HEAP_BASE must be emitted");
});
...
test("B2: leaf-guard — a callee flow (referenced by another) does NOT self-reset; only the leaf entry does", async () => {
  ...
  const resetCount = (wat.match(/\(global\.set \$__fungi_heap \(i32\.const 1024\)\)/g) || []).length;
